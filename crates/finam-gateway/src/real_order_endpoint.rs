...
pub struct M3j7TinyLiveOrderPathSkeletonInput {
    ...
    pub m3j6_design_package_accepted: bool,
    pub compile_time_gate_closed: bool,
    pub feature_gate_disabled: bool,
    pub endpoint_gate_closed: bool,
    pub operator_go_artifact_absent: bool,
    pub immediate_pre_run_readonly_absent: bool,
...
pub struct M3j7TinyLiveOrderPathSkeletonReport {
    pub m3j_step: String,
    pub implementation_skeleton_only: bool,
    pub skeleton_ok: bool,
    pub reachability: M3j7TinyLiveOrderPathSkeletonReachability,
    pub default_unreachable_ok: bool,
    pub gates_closed_by_default: bool,
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
pub enum M3j10OperatorGoReadonlyRefreshDecision {
    Blocked,
    RefreshPackageReady,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M3j10OperatorGoReadonlyRefreshInput {
    pub generated_at_label: String,
    pub m3j9_pre_authorization_accepted: bool,
    pub operator_go_artifact_present: bool,
    pub operator_go_artifact_redacted: bool,
    pub operator_go_artifact_fresh: bool,
    pub timestamp_bound: bool,
    pub account_bound: bool,
    pub symbol_bound: bool,
    pub timeframe_bound: bool,
    ...
    pub config_digest_bound: bool,
    pub session_digest_bound: bool,
    pub immediate_finam_readonly_refresh_completed: bool,
    pub immediate_finam_readonly_refresh_fresh: bool,
    pub immediate_finam_readonly_refresh_clean: bool,
    pub active_orders_zero: bool,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M3j10OperatorGoReadonlyRefreshReport {
    pub m3j_step: String,
    pub operator_go_readonly_refresh_only: bool,
    pub decision: M3j10OperatorGoReadonlyRefreshDecision,
    pub refresh_package_ready: bool,
    pub operator_go_ok: bool,
    pub binding_ok: bool,
    pub readonly_refresh_ok: bool,
    pub broker_truth_ok: bool,
    pub position_ok: bool,
    pub one_shot_arm_ok: bool,
    pub kill_switch_ok: bool,
...
pub struct M3j14OneShotMicroExecutionGateInput {
    pub generated_at_label: String,
    pub m3j13_no_send_runbook_accepted: bool,
    pub explicit_operator_approval_artifact_referenced: bool,
    pub fresh_immediate_readonly_evidence_referenced: bool,
    pub readonly_token_scope_used_for_gate: bool,
    pub full_trade_token_loaded: bool,
    pub trade_token_required_for_actual_invocation: bool,
    ...
    pub orphan_active_orders_zero: bool,
    pub flat_or_expected_position: bool,
    pub kill_switch_armed_and_tested: bool,
    pub one_shot_ttl_arm_present_and_fresh: bool,
    pub begin_submit_durable_audit_before_boundary_required: bool,
    pub post_run_reconciliation_required: bool,
    pub eod_report_required: bool,
...
pub struct M3j14OneShotMicroExecutionGateReport {
    pub m3j_step: String,
    pub one_shot_micro_execution_gate_only: bool,
    pub decision: M3j14OneShotMicroExecutionGateDecision,
    pub gate_ready_no_send: bool,
    pub operator_and_readonly_evidence_ok: bool,
    pub token_scope_ok: bool,
    pub scope_ok: bool,
    pub broker_truth_ok: bool,
    pub position_ok: bool,
...
pub fn m3j7_tiny_live_order_path_skeleton_report(
    input: M3j7TinyLiveOrderPathSkeletonInput,
) -> M3j7TinyLiveOrderPathSkeletonReport {
    let default_unreachable_ok = input.compile_time_gate_closed
        && input.feature_gate_disabled
        && input.endpoint_gate_closed
        && input.operator_go_artifact_absent
        && input.immediate_pre_run_readonly_absent
        && !input.real_boundary_call_requested;
    let gates_closed_by_default = input.m3j6_design_package_accepted
        && input.compile_time_gate_closed
        && input.feature_gate_disabled
        && input.endpoint_gate_closed;
    ...
    let audit_rollback_reconciliation_ok = input.durable_audit_before_boundary_required
        && input.rollback_plan_required
        && input.post_run_reconciliation_required
        && input.eod_report_required;
    let scanner_and_redaction_ok =
        input.scanner_blocks_unguarded_boundary_methods && input.redaction_required;
    ...
    if !gates_closed_by_default {
        skeleton_blockers
            .push("compile/feature/endpoint gates are not closed by default".to_string());
    }
    if !kill_switch_coverage_ok {
        skeleton_blockers.push("kill switch coverage is incomplete".to_string());
    }
    if !first_micro_scope_ok {
        skeleton_blockers.push("first micro scope is not constrained".to_string());
    }
    if !audit_rollback_reconciliation_ok {
        skeleton_blockers
            .push("audit, rollback, reconciliation, or EOD requirement is missing".to_string());
    }
    if !scanner_and_redaction_ok {
        skeleton_blockers.push("scanner or redaction requirement is incomplete".to_string());
    }
    if !no_live_boundary {
        skeleton_blockers.push("live boundary is open in skeleton package".to_string());
    }
    let skeleton_ok = default_unreachable_ok
        && gates_closed_by_default
        && kill_switch_coverage_ok
        && first_micro_scope_ok
        && audit_rollback_reconciliation_ok
        && scanner_and_redaction_ok
        && no_live_boundary;
    let reachability = if skeleton_ok {
        M3j7TinyLiveOrderPathSkeletonReachability::CandidateOnly
    } else {
        M3j7TinyLiveOrderPathSkeletonReachability::NotReachable
    };
    M3j7TinyLiveOrderPathSkeletonReport {
        m3j_step: "M3j-7".to_string(),
        implementation_skeleton_only: true,
        skeleton_ok,
        reachability,
        default_unreachable_ok,
        gates_closed_by_default,
        kill_switch_coverage_ok,
        first_micro_scope_ok,
        audit_rollback_reconciliation_ok,
...
pub fn m3j10_operator_go_readonly_refresh_report(
    input: M3j10OperatorGoReadonlyRefreshInput,
) -> M3j10OperatorGoReadonlyRefreshReport {
    let operator_go_ok = input.operator_go_artifact_present
        && input.operator_go_artifact_redacted
        && input.operator_go_artifact_fresh;
    let binding_ok = input.timestamp_bound
        && input.account_bound
        && input.symbol_bound
        && input.timeframe_bound
        ...
        && input.config_digest_bound
        && input.session_digest_bound;
    let readonly_refresh_ok = input.immediate_finam_readonly_refresh_completed
        && input.immediate_finam_readonly_refresh_fresh
        && input.immediate_finam_readonly_refresh_clean;
    let broker_truth_ok = input.active_orders_zero
    ...
    let refresh_package_ready = input.m3j9_pre_authorization_accepted
        && operator_go_ok
        && binding_ok
        && readonly_refresh_ok
        && broker_truth_ok
    ...
    let mut blockers = Vec::new();
    if !input.m3j9_pre_authorization_accepted {
        blockers.push("M3j-9 pre-authorization package is not accepted".to_string());
    }
    if !operator_go_ok {
        blockers.push("fresh redacted operator GO artifact is missing".to_string());
    }
    if !binding_ok {
        blockers.push(
            "timestamp/account/symbol/timeframe/strategy/config/session binding is incomplete"
    ...
    if !readonly_refresh_ok {
        blockers
            .push("immediate FINAM read-only refresh is missing, stale, or not clean".to_string());
    }
    if !broker_truth_ok {
    ...
    if !micro_limits_ok {
        blockers.push("max orders, quantity, or micro notional/loss limit is missing".to_string());
    }
    if !audit_ok {
        blockers.push("durable audit-before-boundary proof is missing".to_string());
    }
    ...
    if !redaction_ok {
        blockers.push("raw operator GO, account, or broker payload would be exported".to_string());
    }
    if unsafe_live_input_detected {
        blockers.push("live boundary input is present in readonly refresh package".to_string());
    }

    M3j10OperatorGoReadonlyRefreshReport {
        m3j_step: "M3j-10".to_string(),
        operator_go_readonly_refresh_only: true,
        decision: if refresh_package_ready {
            M3j10OperatorGoReadonlyRefreshDecision::RefreshPackageReady
        } else {
            M3j10OperatorGoReadonlyRefreshDecision::Blocked
        },
        refresh_package_ready,
        operator_go_ok,
        binding_ok,
        readonly_refresh_ok,
        broker_truth_ok,
...
pub fn m3j14_one_shot_micro_execution_gate_report(
    input: M3j14OneShotMicroExecutionGateInput,
) -> M3j14OneShotMicroExecutionGateReport {
    let operator_and_readonly_evidence_ok = input.explicit_operator_approval_artifact_referenced
        && input.fresh_immediate_readonly_evidence_referenced;
    let token_scope_ok = input.readonly_token_scope_used_for_gate
        && !input.full_trade_token_loaded
        && input.trade_token_required_for_actual_invocation;
    ...
    let broker_truth_ok = input.active_orders_zero
        ...
        && input.orphan_active_orders_zero;
    let position_ok = input.flat_or_expected_position;
    let risk_controls_ok =
        input.kill_switch_armed_and_tested && input.one_shot_ttl_arm_present_and_fresh;
    let audit_and_post_run_ok = input.begin_submit_durable_audit_before_boundary_required
        && input.post_run_reconciliation_required
        && input.eod_report_required;
    let scanner_and_redaction_ok =
        input.redacted_evidence_only && input.scanner_blocks_unguarded_post_delete_send;
    let unsafe_execution_input_detected = input.boundary_invocation_requested
        || input.real_finam_order_endpoint_requested
        || input.runtime_live_attachment_requested
    ...
    let gate_ready_no_send = input.m3j13_no_send_runbook_accepted
        && operator_and_readonly_evidence_ok
        && token_scope_ok
        && scope_ok
        && broker_truth_ok
        && position_ok
    ...
    let mut blockers = Vec::new();
    if !input.m3j13_no_send_runbook_accepted {
        blockers.push("M3j-13 no-send runbook is not accepted".to_string());
    }
    if !operator_and_readonly_evidence_ok {
        blockers
            .push("operator approval artifact or fresh readonly evidence is missing".to_string());
    }
    if !token_scope_ok {
        blockers.push(
            "token scope is unsafe for no-send gate or trade-token requirement is not explicit"
                .to_string(),
        );
    }
    if !scope_ok {
        blockers.push("one-account/symbol/timeframe/strategy tiny max-one market-or-limit scope is incomplete".to_string());
    }
    if !broker_truth_ok {
        blockers.push("active, unknown active, or orphan active orders are not zero".to_string());
    }
    ...
    if !risk_controls_ok {
        blockers.push("kill switch or one-shot TTL arm is missing or stale".to_string());
    }
    if !audit_and_post_run_ok {
        blockers.push(
            "audit-before-boundary, reconciliation, or EOD requirement is missing".to_string(),
        );
    }
    if !scanner_and_redaction_ok {
        blockers.push("scanner coverage or redaction requirement is incomplete".to_string());
    ...
    if unsafe_execution_input_detected {
        blockers.push("real invocation, live attach, continuous consumer, advanced order semantics, or full trade token is present in no-send gate".to_string());
    }

    M3j14OneShotMicroExecutionGateReport {
        m3j_step: "M3j-14".to_string(),
        one_shot_micro_execution_gate_only: true,
        decision: if gate_ready_no_send {
            M3j14OneShotMicroExecutionGateDecision::GateReadyNoSend
        } else {
            M3j14OneShotMicroExecutionGateDecision::Blocked
        },
        gate_ready_no_send,
        operator_and_readonly_evidence_ok,
        token_scope_ok,
...
mod tests {
    ...
    #[test]
    fn m3j7_tiny_live_order_path_skeleton_is_candidate_only_and_no_live() {
        let report = m3j7_tiny_live_order_path_skeleton_report(sample_m3j7_skeleton_input());
        assert_eq!(report.m3j_step, "M3j-7");
        assert!(report.implementation_skeleton_only);
        ...
        assert!(report.gates_closed_by_default);
        ...
        assert!(report.skeleton_blockers.is_empty());
    }

    #[test]
    fn m3j7_requested_boundary_call_missing_gate_or_open_boundary_blocks_skeleton() {
        let mut requested = sample_m3j7_skeleton_input();
        requested.real_boundary_call_requested = true;
        let report = m3j7_tiny_live_order_path_skeleton_report(requested);
        assert!(!report.skeleton_ok);
        assert_eq!(
            report.reachability,
            M3j7TinyLiveOrderPathSkeletonReachability::NotReachable
        ...
        let mut missing_gate = sample_m3j7_skeleton_input();
        missing_gate.endpoint_gate_closed = false;
        let report = m3j7_tiny_live_order_path_skeleton_report(missing_gate);
        assert!(!report.skeleton_ok);
        assert!(!report.gates_closed_by_default);

        let mut open_boundary = sample_m3j7_skeleton_input();
        open_boundary.external_finam_order_calls_allowed = true;
    ...
    #[test]
    fn m3j10_operator_go_readonly_refresh_ready_but_live_micro_still_blocked() {
        let report =
            m3j10_operator_go_readonly_refresh_report(sample_m3j10_readonly_refresh_input());
        assert_eq!(report.m3j_step, "M3j-10");
        assert!(report.operator_go_readonly_refresh_only);
        assert_eq!(
            report.decision,
            M3j10OperatorGoReadonlyRefreshDecision::RefreshPackageReady
        );
        assert!(report.refresh_package_ready);
        assert!(report.operator_go_ok);
        assert!(report.binding_ok);
        assert!(report.readonly_refresh_ok);
        assert!(report.broker_truth_ok);
        assert!(report.position_ok);
        assert!(report.one_shot_arm_ok);
        assert!(report.kill_switch_ok);
    ...
    #[test]
    fn m3j10_stale_readonly_active_orders_expired_arm_or_live_input_blocks_refresh() {
        let mut stale_readonly = sample_m3j10_readonly_refresh_input();
        stale_readonly.immediate_finam_readonly_refresh_fresh = false;
        let report = m3j10_operator_go_readonly_refresh_report(stale_readonly);
        assert_eq!(
            report.decision,
            M3j10OperatorGoReadonlyRefreshDecision::Blocked
        );
        ...
        let mut active_orders = sample_m3j10_readonly_refresh_input();
        active_orders.active_orders_zero = false;
        active_orders.unknown_active_orders_zero = false;
        let report = m3j10_operator_go_readonly_refresh_report(active_orders);
        assert_eq!(
            report.decision,
            M3j10OperatorGoReadonlyRefreshDecision::Blocked
        );
        ...
        let mut expired_arm = sample_m3j10_readonly_refresh_input();
        expired_arm.one_shot_ttl_not_expired = false;
        let report = m3j10_operator_go_readonly_refresh_report(expired_arm);
        assert_eq!(
            report.decision,
            M3j10OperatorGoReadonlyRefreshDecision::Blocked
        );
        ...
        let mut live_input = sample_m3j10_readonly_refresh_input();
        live_input.live_micro_go_requested = true;
        live_input.non_loopback_order_endpoint_allowed = true;
        let report = m3j10_operator_go_readonly_refresh_report(live_input);
        assert_eq!(
            report.decision,
            M3j10OperatorGoReadonlyRefreshDecision::Blocked
        );
    ...
    #[test]
    fn m3j14_one_shot_micro_gate_ready_with_readonly_scope_but_no_send() {
        let report = m3j14_one_shot_micro_execution_gate_report(sample_m3j14_micro_gate_input());
        assert_eq!(report.m3j_step, "M3j-14");
        assert!(report.one_shot_micro_execution_gate_only);
        assert_eq!(
            report.decision,
            M3j14OneShotMicroExecutionGateDecision::GateReadyNoSend
        );
        assert!(report.gate_ready_no_send);
        assert!(report.operator_and_readonly_evidence_ok);
        assert!(report.token_scope_ok);
        ...
        assert!(!report.stop_sltp_bracket_replace_multileg_allowed);
        assert!(report.trade_token_required_for_actual_invocation);
        assert!(!report.unsafe_execution_input_detected);
        assert!(report.blockers.is_empty());
    }

    #[test]
    fn m3j14_missing_approval_trade_token_or_boundary_request_blocks_gate() {
        let mut missing_approval = sample_m3j14_micro_gate_input();
        missing_approval.explicit_operator_approval_artifact_referenced = false;
        let report = m3j14_one_shot_micro_execution_gate_report(missing_approval);
        assert_eq!(
            report.decision,
            M3j14OneShotMicroExecutionGateDecision::Blocked
        );
        ...
        let mut trade_token_loaded = sample_m3j14_micro_gate_input();
        trade_token_loaded.full_trade_token_loaded = true;
        let report = m3j14_one_shot_micro_execution_gate_report(trade_token_loaded);
        assert_eq!(
            report.decision,
            M3j14OneShotMicroExecutionGateDecision::Blocked
        );
        ...
        let mut boundary_request = sample_m3j14_micro_gate_input();
        boundary_request.boundary_invocation_requested = true;
        boundary_request.runtime_live_attachment_requested = true;
        boundary_request.stop_sltp_bracket_replace_multileg_requested = true;
        let report = m3j14_one_shot_micro_execution_gate_report(boundary_request);
        assert_eq!(
            report.decision,
            M3j14OneShotMicroExecutionGateDecision::Blocked
        );
    ...
    fn sample_m3j7_skeleton_input() -> M3j7TinyLiveOrderPathSkeletonInput {
        M3j7TinyLiveOrderPathSkeletonInput {
            ...
            m3j6_design_package_accepted: true,
            compile_time_gate_closed: true,
            feature_gate_disabled: true,
            endpoint_gate_closed: true,
            operator_go_artifact_absent: true,
            immediate_pre_run_readonly_absent: true,
            ...
            durable_audit_before_boundary_required: true,
            rollback_plan_required: true,
            post_run_reconciliation_required: true,
            eod_report_required: true,
            scanner_blocks_unguarded_boundary_methods: true,
            redaction_required: true,
            real_boundary_call_requested: false,
            live_ready_allowed: false,
            runtime_live_attachment_allowed: false,
            external_finam_order_calls_allowed: false,
            command_consumer_to_real_finam_allowed: false,
    ...
    fn sample_m3j10_readonly_refresh_input() -> M3j10OperatorGoReadonlyRefreshInput {
        M3j10OperatorGoReadonlyRefreshInput {
            generated_at_label: "synthetic-m3j10-test".to_string(),
            m3j9_pre_authorization_accepted: true,
            operator_go_artifact_present: true,
            operator_go_artifact_redacted: true,
            operator_go_artifact_fresh: true,
            timestamp_bound: true,
            account_bound: true,
            symbol_bound: true,
            timeframe_bound: true,
            ...
            config_digest_bound: true,
            session_digest_bound: true,
            immediate_finam_readonly_refresh_completed: true,
            immediate_finam_readonly_refresh_fresh: true,
            immediate_finam_readonly_refresh_clean: true,
            active_orders_zero: true,
    ...
    fn sample_m3j14_micro_gate_input() -> M3j14OneShotMicroExecutionGateInput {
        M3j14OneShotMicroExecutionGateInput {
            generated_at_label: "synthetic-m3j14-test".to_string(),
            m3j13_no_send_runbook_accepted: true,
            explicit_operator_approval_artifact_referenced: true,
            fresh_immediate_readonly_evidence_referenced: true,
            readonly_token_scope_used_for_gate: true,
            full_trade_token_loaded: false,
            ...
            orphan_active_orders_zero: true,
            flat_or_expected_position: true,
            kill_switch_armed_and_tested: true,
            one_shot_ttl_arm_present_and_fresh: true,
            begin_submit_durable_audit_before_boundary_required: true,
            post_run_reconciliation_required: true,
            eod_report_required: true,
