...
pub enum Stage5dBootstrapBlockReason {
    ...
    ExtraWorkingOrder,
    WorkingOrderIdentityMissing,
    WorkingOrderKindMismatch,
    ExpectedWorkingStopUnsupported,
    ExpectedWorkingStopMissing,
    ExtraWorkingStop,
    WorkingStopKindMismatch,
    WorkingSetDuplicateId,
    SupplementalStopTruthBindingMismatch,
    SemanticStateInvalid,
}

#[allow(dead_code)]
const STAGE5D_SUPPLEMENTAL_WORKING_STOP_TRUTH_SCHEMA_VERSION: u16 = 1;

#[derive(Clone, Debug, PartialEq, Serialize, Deserialize)]
#[allow(dead_code)]
pub(crate) struct Stage5dSupplementalWorkingStopTruth {
    pub schema_version: u16,
    pub snapshot_id: String,
    pub account_id: BrokerAccountId,
    pub instrument: InstrumentId,
    pub received_ts: DateTime<Utc>,
    pub working_stops: Vec<Stage5dSupplementalWorkingStopOrderTruth>,
    pub fingerprint_sha256: String,
}

#[derive(Clone, Debug, PartialEq, Serialize, Deserialize)]
#[allow(dead_code)]
pub(crate) struct Stage5dSupplementalWorkingStopOrderTruth {
    pub account_id: BrokerAccountId,
    pub instrument: InstrumentId,
...
> {
    ...
    let expected_working_order_ids = injected
        ...
        .clone();
    let expected_working_stop_order_ids = injected
        .envelope
        .runtime_private_extension
        .expected_working_sets
...
fn validate_stage5d_runtime_restore_broker_truth(
    ...
    if !broker_owned_order_ids_are_frozen {
...
pub(crate) fn stage5d_notify_working_set_broker_truth_bootstrap_at(
    ...
    match crate::stage5c_paper_host::stage5d_bootstrap_preserving_loaded_with_validated_working_sets_at(
        loaded,
        notification_now,
        working_orders,
        working_stops,
...
fn stage5d_validate_working_set_truth(
    ...
    let expected_order_set = expected_orders.iter().collect::<HashSet<_>>();
    let expected_stop_as_order_set = expected_stops
        .iter()
        .map(BrokerStopOrderId::as_str)
        .collect::<HashSet<_>>();
    if expected_orders
        .iter()
        .any(|id| expected_stop_as_order_set.contains(id.as_str()))
    {
        return Err(Stage5dBootstrapBlockReason::WorkingSetDuplicateId);
    }

    let mut working_orders = HashMap::new();
    for order in &snapshot.target_active_orders {
        if order.account_id != snapshot.account_id || order.instrument != snapshot.instrument {
            return Err(Stage5dBootstrapBlockReason::SnapshotInstrumentMismatch);
        }
        let Some(order_id) = order.broker_order_id.clone() else {
            return Err(Stage5dBootstrapBlockReason::WorkingOrderIdentityMissing);
        };
        if !expected_order_set.contains(&order_id) {
            if expected_stop_as_order_set.contains(order_id.as_str()) {
                return Err(Stage5dBootstrapBlockReason::WorkingOrderKindMismatch);
            }
            return Err(Stage5dBootstrapBlockReason::ExtraWorkingOrder);
        }
        if order.lifecycle != broker_core::BrokerOrderLifecycle::Active
            || !matches!(
                order.status,
                broker_core::OrderStatus::New
                    | broker_core::OrderStatus::Working
                    | broker_core::OrderStatus::PartiallyFilled
            )
            || !matches!(order.order_type, broker_core::OrderType::Limit)
        {
            return Err(Stage5dBootstrapBlockReason::WorkingOrderKindMismatch);
        }
        working_orders.insert(
            order_id.clone(),
            stage5d_order_event_from_active_order(order, loaded.stage5d_strategy(), admission)?,
        );
    }
    if working_orders.len() != expected_orders.len() {
        return Err(Stage5dBootstrapBlockReason::ExpectedWorkingOrderMissing);
    }
    for expected in expected_orders {
        if !working_orders.contains_key(expected) {
            return Err(Stage5dBootstrapBlockReason::ExpectedWorkingOrderMissing);
        }
    }
...
#[allow(dead_code)]
fn validate_stage5d_broker_truth_bootstrap_common(
    loaded: &crate::stage5c_paper_host::Stage5cRuntimeStateLoadedPaperStrategy,
    envelope: &Stage5dPersistenceEnvelope,
) -> Result<(), Stage5dBootstrapBlockReason> {
    let admission = loaded.stage5d_admission();
...
#[allow(dead_code)]
fn stage5d_order_event_from_active_order(
    order: &broker_core::BrokerOrderSnapshot,
    strategy: &crate::hybrid_intraday_runtime::HybridIntradayRuntimeStrategy,
    admission: &crate::stage5c_paper_host::Stage5cPaperHostAdmission,
) -> Result<crate::runtime_compat::OrderEvent, Stage5dBootstrapBlockReason> {
    let Some(order_id) = order.broker_order_id.clone() else {
        return Err(Stage5dBootstrapBlockReason::WorkingOrderIdentityMissing);
    };
    Ok(crate::runtime_compat::OrderEvent {
        order_id,
        request_id: None,
        symbol: order.instrument.symbol.clone(),
        status: "working".to_string(),
        side: stage5d_order_side_name(order.side).to_string(),
        order_type: stage5d_order_type_name(order.order_type).to_string(),
        ...
        price: order
            ...
            .unwrap_or_default(),
        existing: true,
        comment: stage5d_runtime_protective_comment(strategy, admission, "TP"),
        ts_utc: order.source_ts.unwrap_or(order.received_ts).timestamp(),
    })
}
...
pub(crate) fn stage5d_validate_supplemental_working_stop_truth(
    ...
    let expected_stops = &envelope
        ...
        .expected_working_sets
        .expected_working_stop_order_ids;
    let expected_stop_set = expected_stops.iter().collect::<HashSet<_>>();
    let expected_orders_as_stop_set = envelope
        .runtime_private_extension
        .expected_working_sets
        .expected_working_order_ids
        .iter()
    ...
    for stop in &truth.working_stops {
        ...
        if expected_orders_as_stop_set.contains(stop.stop_order_id.as_str()) {
            return Err(Stage5dBootstrapBlockReason::WorkingStopKindMismatch);
        }
        if !expected_stop_set.contains(&stop.stop_order_id) {
            return Err(Stage5dBootstrapBlockReason::ExtraWorkingStop);
        }
        if !matches!(
            stop.status.as_str(),
            "new" | "working" | "active" | "accepted"
        ) {
            return Err(Stage5dBootstrapBlockReason::WorkingStopKindMismatch);
        }
        working_stops.insert(
            stop.stop_order_id.clone(),
            crate::runtime_compat::StopOrderEvent {
                stop_order_id: stop.stop_order_id.clone(),
                exchange_order_id: stop.exchange_order_id.clone(),
                symbol: stop.instrument.symbol.clone(),
                status: "working".to_string(),
                side: stop.side.clone(),
                qty: parse_finite_decimal_string(&stop.qty).map_err(|_| {
                    Stage5dBootstrapBlockReason::SupplementalStopTruthBindingMismatch
                })?,
                filled: 0.0,
                stop_price: parse_finite_decimal_string(&stop.stop_price).map_err(|_| {
                    Stage5dBootstrapBlockReason::SupplementalStopTruthBindingMismatch
                })?,
                price: stop
                    .limit_price
                    ...
                    .map(parse_finite_decimal_string)
                    .transpose()
                    .map_err(|_| Stage5dBootstrapBlockReason::SupplementalStopTruthBindingMismatch)?
                    .unwrap_or_default(),
                existing: true,
                comment: stop.comment.clone(),
                end_time: None,
                ts_utc: stop.received_ts.timestamp(),
            },
        );
    }
    if working_stops.len() != expected_stops.len() {
        return Err(Stage5dBootstrapBlockReason::ExpectedWorkingStopMissing);
    }
    for expected in expected_stops {
        if !working_stops.contains_key(expected) {
            return Err(Stage5dBootstrapBlockReason::ExpectedWorkingStopMissing);
        }
    }
...
#[allow(dead_code)]
fn stage5d_compute_supplemental_working_stop_truth_fingerprint(
    truth: &Stage5dSupplementalWorkingStopTruth,
) -> Result<String, Stage5dBootstrapBlockReason> {
    let payload = serde_json::to_vec(&serde_json::json!({
        "schema": "stage5d_supplemental_working_stop_truth_v1",
        "schema_version": truth.schema_version,
        "snapshot_id": truth.snapshot_id,
        "account_id": truth.account_id,
        "instrument": truth.instrument,
        "received_ts": truth.received_ts,
        "working_stops": truth.working_stops,
    }))
    .map_err(|_| Stage5dBootstrapBlockReason::SupplementalStopTruthBindingMismatch)?;
    Ok(format!(
        "stage5d_supplemental_working_stop_truth_sha256:{:x}",
        Sha256::digest(payload)
    ))
}

#[allow(dead_code)]
fn stage5d_order_side_name(side: broker_core::OrderSide) -> &'static str {
    match side {
...
#[allow(dead_code)]
fn stage5d_order_type_name(order_type: broker_core::OrderType) -> &'static str {
    match order_type {
...
#[allow(dead_code)]
fn stage5d_runtime_protective_comment(
    strategy: &crate::hybrid_intraday_runtime::HybridIntradayRuntimeStrategy,
    admission: &crate::stage5c_paper_host::Stage5cPaperHostAdmission,
    role: &str,
) -> Option<String> {
    match Strategy::state(strategy) {
        StrategyState::HybridIntradayRuntime {
            ...
            let owner = match current_owner {
                ...
                Some(crate::hybrid_intraday::Owner::IntradayBreakout) | None => "BO",
            };
            format!(
                "HYB|sid={}|c={cycle}|o={owner}|r={role}",
...
mod tests {
    ...
    fn stage5d_test_assert_recovery_index_pending_entry_field_level(
        label: &str,
        state: &Value,
        extension: &Stage5dRuntimePrivateExtension,
        expected_entry: &Stage5dPendingEntryExtension,
        expected_cycle_id: &str,
        expected_created_ts_utc: i64,
    ) {
        // recovery_index_r1r1_pending_request_field_level_assertions
        let actual_entry = extension
            .pending_entry
            .as_ref()
            .unwrap_or_else(|| panic!("{label}: pending-entry private DTO must be present"));
        assert_eq!(
            actual_entry.request_id, expected_entry.request_id,
            "{label}: request ID"
        );
        assert_eq!(actual_entry.owner, expected_entry.owner, "{label}: owner");
        assert_eq!(actual_entry.side, expected_entry.side, "{label}: side");
        assert_eq!(
            actual_entry.reason, expected_entry.reason,
            "{label}: lifecycle reason"
        );
        assert_eq!(
            actual_entry.entry_style, expected_entry.entry_style,
            "{label}: entry style"
        );
        assert_eq!(
            actual_entry.target_qty, expected_entry.target_qty,
            "{label}: target qty"
        );
        assert_eq!(
            actual_entry.stop_price, expected_entry.stop_price,
            "{label}: stop price"
        );
        assert_eq!(
            actual_entry.take_price, expected_entry.take_price,
            "{label}: take price"
        );
        let runtime = stage5d_runtime_object(state, label);
        let expected_request_id = expected_entry
            .request_id
            .expect("field-level pending entry must carry request id")
            .to_string();
        assert_eq!(
            runtime
            ...
            "{label}: semantic request ID"
        );
        assert_eq!(
            runtime
                .get("pending_entry_cycle_id")
                .and_then(Value::as_str),
            Some(expected_cycle_id),
            "{label}: semantic cycle ID"
        );
        assert_eq!(
            runtime
                .get("pending_entry_created_ts_utc")
                .and_then(Value::as_i64),
            Some(expected_created_ts_utc),
            "{label}: semantic created timestamp"
        );
        assert_eq!(
            runtime.get("pending_entry_owner"),
            Some(&serde_json::to_value(expected_entry.owner).expect("owner serializes")),
            "{label}: semantic owner"
        );
        assert_eq!(
            runtime.get("pending_entry_side"),
            Some(&serde_json::to_value(expected_entry.side).expect("side serializes")),
            "{label}: semantic side"
        );
    }
    ...
    #[derive(Debug, Clone, Copy, PartialEq, Eq)]
    enum Stage5dR3OperationalStateCase {
        PartialEntry,
        PendingExit,
        DeferredEntry,
        DeferredExit,
    ...
    impl Stage5dR3OperationalStateCase {
        fn snapshot_id(self) -> &'static str {
            match self {
                Self::PartialEntry => "stage5d-final-r3-operational-partial-entry",
                Self::PendingExit => "stage5d-final-r3-operational-pending-exit",
                Self::DeferredEntry => "stage5d-final-r3-operational-deferred-entry",
                Self::DeferredExit => "stage5d-final-r3-operational-deferred-exit",
                Self::SafeModeCloseOnly => "stage5d-final-r3-operational-safe-mode-close-only",
            }
        }

        fn expected_position_qty(self) -> f64 {
            match self {
                Self::PartialEntry => -1.0,
                Self::PendingExit => 1.0,
                Self::DeferredEntry => 0.0,
                Self::DeferredExit => 1.0,
                Self::SafeModeCloseOnly => -1.0,
            }
        }

        fn expected_pending_requests(self) -> usize {
            match self {
                Self::PartialEntry => 1,
                Self::PendingExit => 1,
                Self::DeferredEntry => 1,
                Self::DeferredExit => 1,
                Self::SafeModeCloseOnly => 1,
            }
        }
    }

    fn stage5d_test_source_operational_ctx_with_position(
        position_qty: f64,
    ) -> crate::runtime_compat::StrategyCtx {
        let mut ctx = stage5d_test_source_paper_ctx();
        ctx.position_qty = Some(position_qty);
        ctx.gateway_phase = crate::live_guard::GatewayPhase::LiveReady;
        ctx
    }

    fn stage5d_test_source_operational_state_strategy(
        case: Stage5dR3OperationalStateCase,
    ) -> crate::hybrid_intraday_runtime::HybridIntradayRuntimeStrategy {
        match case {
            Stage5dR3OperationalStateCase::PartialEntry => {
                let mut strategy = stage5d_test_source_pending_entry_strategy(
                    Stage5dR3aPendingSourceCase::MrShort,
                );
                let req = match Strategy::state(&strategy) {
                    StrategyState::HybridIntradayRuntime {
                        pending_entry_request_id,
                        ..
                    } => pending_entry_request_id.expect("partial entry request id"),
                    StrategyState::Idle => panic!("partial entry source must be hybrid"),
                };
                let ack_ctx = stage5d_test_source_operational_ctx_with_position(0.0);
                let _ =
                    strategy.on_ack(&ack_ctx, &crate::runtime_compat::CommandAck::accepted(req));
                let fill_ctx = stage5d_test_source_operational_ctx_with_position(-1.0);
                let intents = strategy.on_position(
                    ...
                    &crate::runtime_compat::PositionEvent {
                        ...
                        existing: false,
                        avg_price: 100.0,
                        ts_utc: stage5d_test_source_ts_local(2026, 1, 6, 9, 11, 0),
                    },
                );
                assert!(
                    intents.is_empty(),
                    "operational partial-entry source lifecycle must not create bracket before target"
                );
                let exported = strategy
                    .stage5d_export_runtime_private_extension()
                    .expect("partial entry extension exports");
                assert!(exported.pending_entry.is_some());
                assert!(exported.partial_entry_timer.is_some());
                assert!(exported.pending_exit.is_none());
                stage5d_test_apply_source_bootstrap_position(&mut strategy, -1.0);
                strategy
            }
            Stage5dR3OperationalStateCase::PendingExit => {
                let mut strategy = stage5d_test_source_broker_open_strategy(Stage5dSide::Long);
                let exit_ctx = stage5d_test_source_operational_ctx_with_position(1.0);
                let intents = strategy.on_bar(
                    &exit_ctx,
                    &stage5d_test_source_bar_ohlc(
                        stage5d_test_source_ts_local(2026, 1, 6, 23, 40, 0),
                        100.0,
                        100.5,
                        99.5,
                        100.0,
                ...
                assert_eq!(
                    intents.len(),
                    1,
                    "operational pending-exit source lifecycle must emit one exit intent"
                );
                let exported = strategy
                    .stage5d_export_runtime_private_extension()
                    .expect("pending exit extension exports");
                assert!(exported.pending_entry.is_none());
                assert!(exported.pending_exit.is_some());
                strategy
            }
            Stage5dR3OperationalStateCase::DeferredEntry => {
                let mut strategy =
                    stage5d_test_source_pending_entry_strategy(Stage5dR3aPendingSourceCase::MrLong);
                let req = match Strategy::state(&strategy) {
                    StrategyState::HybridIntradayRuntime {
                        pending_entry_request_id,
                        ..
                    } => pending_entry_request_id.expect("deferred entry source request id"),
                    StrategyState::Idle => panic!("deferred entry source must be hybrid"),
                };
                ...
                let blocked_ctx = stage5d_test_source_operational_ctx_with_position(0.0);
                let blocked = strategy.on_bar(
                    &blocked_ctx,
                    &stage5d_test_source_bar_ohlc(
                        stage5d_test_source_ts_local(2026, 1, 6, 8, 50, 0),
                        100.0,
                        100.1,
                        99.9,
                        100.0,
                ...
                assert!(
                    blocked.is_empty(),
                    "operational deferred-entry reissue must remain gated before model window opens"
                );
                let exported = strategy
                    .stage5d_export_runtime_private_extension()
                    .expect("deferred entry extension exports");
                assert!(exported.pending_entry.is_none());
                assert!(exported.pending_exit.is_none());
                strategy
            }
            Stage5dR3OperationalStateCase::DeferredExit => {
                ...
                let req = match Strategy::state(&strategy) {
                    StrategyState::HybridIntradayRuntime {
                        ...
                        ..
                    } => pending_exit_request_id.expect("deferred exit source request id"),
                    StrategyState::Idle => panic!("deferred exit source must be hybrid"),
                };
                let _ = strategy.on_ack(
                    &exit_ctx,
                    &crate::runtime_compat::CommandAck::rejected(
                ...
                let blocked_ctx = stage5d_test_source_operational_ctx_with_position(1.0);
                let blocked = strategy.on_bar(
                    &blocked_ctx,
                    &stage5d_test_source_bar_ohlc(
                        stage5d_test_source_ts_local(2026, 1, 6, 8, 50, 0),
                ...
                assert!(
                    blocked.is_empty(),
                    "operational deferred-exit reissue must remain gated before model window opens"
                );
                let exported = strategy
                    .stage5d_export_runtime_private_extension()
                    .expect("deferred exit extension exports");
                assert!(exported.pending_entry.is_none());
                assert!(exported.pending_exit.is_none());
                strategy
            }
            Stage5dR3OperationalStateCase::SafeModeCloseOnly => {
                let mut strategy = stage5d_test_source_operational_state_strategy(
                    Stage5dR3OperationalStateCase::PartialEntry,
                );
                let exported = strategy
                    .stage5d_export_runtime_private_extension()
                    .expect("partial entry extension exports before timer");
                let timer = exported
                    .partial_entry_timer
                    .expect("partial timer source-produced");
                let ctx = stage5d_test_source_operational_ctx_with_position(-1.0);
                let intents = strategy.on_timer(&ctx, timer.partial_started_at_ms + 3_001);
                assert!(
                    intents.iter().any(|intent| matches!(
                        intent.base_intent(),
                        crate::runtime_compat::Intent::Market {
                            side: crate::runtime_compat::OrderSide::Buy,
                            qty,
                            ..
                        } if (*qty - 1.0).abs() <= f64::EPSILON
                    )),
                    "operational safe-mode source path must keep exit repair enabled"
                );
                let exported = strategy
                    .stage5d_export_runtime_private_extension()
                    .expect("safe-mode extension exports");
                assert!(exported.pending_entry.is_none());
                assert!(exported.pending_exit.is_some());
                strategy
            }
    ...
    fn stage5d_test_apply_source_bootstrap_position(
        strategy: &mut crate::hybrid_intraday_runtime::HybridIntradayRuntimeStrategy,
        qty: f64,
    ) {
        let ctx = stage5d_test_source_operational_ctx_with_position(qty);
        let mut snapshot = crate::runtime_compat::BootstrapSnapshot {
            ...
            snapshot_ts_utc: Some(stage5d_test_source_ts_local(2026, 1, 6, 9, 11, 30)),
        };
        snapshot.positions_strategy.insert(
            ...
            crate::runtime_compat::PositionEvent {
                ...
                existing: true,
                avg_price: 100.0,
                ts_utc: stage5d_test_source_ts_local(2026, 1, 6, 9, 11, 30),
            },
        );
        let intents = strategy.on_bootstrap_snapshot(&ctx, &snapshot);
        assert!(
            intents.is_empty(),
            "source bootstrap truth alignment must not emit operational intents"
        );
    }

    fn stage5d_test_r3_operational_input_and_evidence_for_source(
        source_strategy: &crate::hybrid_intraday_runtime::HybridIntradayRuntimeStrategy,
        snapshot_id: &str,
    ) -> (
        Stage5dCanonicalEnvelopeExportInput,
        ...
        let source_state_json = serde_json::to_value(Strategy::state(source_strategy))
            .expect("operational source state serializes");
        let source_extension = source_strategy
            .stage5d_export_runtime_private_extension()
        ...
        evidence.current_shadow_pnl_points =
            crate::hybrid_intraday::format_riskgate_authority_decimal(source_pnl_points)
                .expect("operational source shadow pnl must be authority-canonical");
        evidence.ledger_tail_hash =
            stage5d_compute_riskgate_ledger_tail_hash(&evidence).expect("operational tail hash");
        stage5d_apply_riskgate_evidence_to_envelope(&mut envelope, &evidence);
        bind_fixture_to_strategy_config(&mut envelope, source_strategy);
        envelope.riskgate.ledger_tail_hash = evidence.ledger_tail_hash.clone();
        envelope.payload_checksum_sha256 = envelope
            .compute_payload_checksum_sha256()
            .expect("operational checksum");
        stage5d_test_normalize_persisted_checksum(&mut envelope);
        let input = Stage5dCanonicalEnvelopeExportInput {
            snapshot_id: envelope.snapshot_id.clone(),
            snapshot_revision: envelope.snapshot_revision,
    ...
    fn stage5d_test_assert_operational_case_shape(
        case: Stage5dR3OperationalStateCase,
        state: &Value,
        extension: &Stage5dRuntimePrivateExtension,
    ) {
        let runtime = state
            .get("HybridIntradayRuntime")
            .unwrap_or_else(|| panic!("{case:?}: state must be HybridIntradayRuntime"));
        assert_eq!(
            runtime.get("last_position_qty").and_then(Value::as_f64),
            Some(case.expected_position_qty()),
            "{case:?}: broker truth position quantity must be target-scoped and source-produced"
        );
        match case {
            Stage5dR3OperationalStateCase::PartialEntry => {
                assert!(extension.pending_entry.is_some());
                assert!(extension.partial_entry_timer.is_some());
                assert!(extension.pending_exit.is_none());
                assert_eq!(runtime.get("pending_exit_request_id"), Some(&Value::Null));
                assert!(
                    runtime
                        .get("pending_entry_request_id")
                        .and_then(Value::as_str)
                        .is_some(),
                    "partial entry must preserve request id"
                );
            }
            Stage5dR3OperationalStateCase::PendingExit => {
                assert!(extension.pending_entry.is_none());
                assert!(extension.pending_exit.is_some());
                assert!(
                    runtime
                        .get("pending_exit_request_id")
                        .and_then(Value::as_str)
                        .is_some(),
                    "pending exit must preserve request id"
                );
                assert_eq!(
                    runtime.get("current_owner").and_then(Value::as_str),
                    Some("intraday_breakout"),
                    "pending exit must preserve owner"
                );
            }
            Stage5dR3OperationalStateCase::DeferredEntry => {
                assert!(extension.pending_entry.is_none());
                ...
                assert!(
                    runtime
                        .get("deferred_entry_request_id")
                        .and_then(Value::as_str)
                        .is_some(),
                    "deferred entry must preserve original request id"
                );
                assert_eq!(
                    runtime
                        .get("deferred_entry_entry_style")
                        .and_then(Value::as_str),
                    Some("bracket"),
                    "deferred MR entry must preserve bracket style"
                );
                assert!(
                    runtime
                        ...
                        && runtime
                            ...
                            .and_then(Value::as_f64)
                            .is_some(),
                    "deferred MR entry must preserve stop/take"
                );
            }
            Stage5dR3OperationalStateCase::DeferredExit => {
                assert!(extension.pending_entry.is_none());
                ...
                assert!(
                    runtime
                        .get("deferred_exit_request_id")
                        .and_then(Value::as_str)
                        .is_some(),
                    "deferred exit must preserve original request id"
                );
                assert_eq!(
                    runtime.get("deferred_exit_owner").and_then(Value::as_str),
                    Some("intraday_breakout"),
                    "deferred exit must preserve owner"
                );
            }
            Stage5dR3OperationalStateCase::SafeModeCloseOnly => {
                assert!(extension.pending_entry.is_none());
                assert!(extension.pending_exit.is_some());
                assert_eq!(
                    runtime.get("safe_mode_close_only").and_then(Value::as_bool),
                    Some(true),
                    "safe-mode close-only flag must survive strict bytes"
                );
                assert_eq!(
                    runtime.get("safe_mode_reason").and_then(Value::as_str),
                    Some("partial_entry_fill_timeout"),
                    "safe-mode reason must remain redacted and exact"
                );
                assert_eq!(
                    runtime.get("entry_ready").and_then(Value::as_bool),
                    Some(true),
                    "safe-mode must block normal entry through close-only flag, not by disabling repair"
                );
            }
        }
    }
    ...
    fn stage5d_test_r3_operational_source_full_restart(
        ...
        let source_strategy = stage5d_test_source_operational_state_strategy(case);
        let source_state =
            serde_json::to_value(Strategy::state(&source_strategy)).expect("operational state");
        let source_extension = source_strategy
            .stage5d_export_runtime_private_extension()
            .expect("operational extension exports");
        stage5d_test_assert_operational_case_shape(case, &source_state, &source_extension);
        let source_semantic_fingerprint =
            crate::stage5c_paper_host::stage5c_semantic_value_fingerprint(&source_state)
                .expect("operational semantic fingerprint");
        ...
        .expect("operational source-owned package export must succeed");
        assert!(!report.redis_opened);
        assert!(!report.finam_opened);
        assert!(!report.dispatch_opened);
        assert!(!report.runtime_live_opened);
        let package_json = package
            .to_json_strict()
            .expect("operational package serializes strictly");
        drop(source_strategy);

        let decoded = Stage5dCanonicalRestartPackage::from_json_str_strict(&package_json)
            .expect("operational strict package decodes");
        assert_eq!(
            decoded.envelope.strategy_state.strategy_state_json, source_state,
            "{case:?}: strict package must preserve exact source semantic state"
        );
        assert_eq!(
            decoded.report.semantic_payload_fingerprint, source_semantic_fingerprint,
            "{case:?}: semantic fingerprint must survive durable package"
        );
        ...
        let interim_extension = fresh_strategy
            .stage5d_export_runtime_private_extension()
            .expect("operational interim extension exports");
        if source_extension.pending_entry.is_some() {
            assert_ne!(
                interim_extension.pending_entry, source_extension.pending_entry,
                "{case:?}: semantic set_state placeholder must not substitute final private DTO"
            );
        }
        let strict_envelope = decoded.envelope;
        let validated_evidence = decoded.validated_evidence;
        ...
        let admission =
            stage5d_test_admission_for_envelope(&strict_envelope, case.expected_position_qty());
        let loaded = crate::stage5c_paper_host::Stage5cRuntimeStateLoadedPaperStrategy::stage5d_test_loaded_from_parts(
            fresh_strategy,
            admission,
            restored_indexes,
        ...
        let validated = strict_envelope
            .clone()
            .validate_restore_contract_schema_only()
            .expect("operational strict envelope remains valid");
        let bound = expect_stage5d_ok(
            stage5d_bind_runtime_state_loaded(loaded, validated),
            "operational loaded state must bind",
        );
        let applied = expect_stage5d_ok(
            stage5d_apply_runtime_private_extension(bound),
            "operational private extension must apply before broker bootstrap",
        );
        let post_apply_extension = applied
            .loaded
            .stage5d_strategy()
            .stage5d_export_runtime_private_extension()
            .expect("operational post-apply extension exports");
        assert_eq!(
            post_apply_extension, source_extension,
            "{case:?}: private apply must restore exact source-produced private extension"
        );
        let post_apply_state =
            serde_json::to_value(Strategy::state(applied.loaded.stage5d_strategy()))
                .expect("operational post-apply state");
        stage5d_test_assert_operational_case_shape(case, &post_apply_state, &post_apply_extension);
        assert_eq!(
            strict_envelope.recovery_indexes.pending_requests.len(),
            case.expected_pending_requests(),
            "{case:?}: pending request index must preserve lifecycle request ids"
        );

        let bootstrapped = expect_stage5d_bootstrap_ok(
            stage5d_notify_broker_truth_bootstrap_at(applied, strict_envelope.persisted_at_ts_utc),
            "operational broker-truth bootstrap must be consistent with source state",
        );
        let injected = expect_stage5d_riskgate_ok(
            stage5d_inject_authoritative_riskgate(bootstrapped, validated_evidence),
            "operational riskgate injection must succeed",
        );
        assert!(injected.recovery_complete());
        let recovery_plan_fingerprint = injected.recovery_plan_fingerprint().to_string();
        let restored = stage5d_test_assert_injected_restores_indexes_once(
            ...
            &strict_envelope.recovery_indexes.known_order_ids,
            &strict_envelope.recovery_indexes.pending_requests,
            "operational restored callback must run exactly once",
        );
        let restored_state = serde_json::to_value(Strategy::state(restored.stage5d_strategy()))
            .expect("operational restored state");
        let restored_extension = restored
            .stage5d_strategy()
            .stage5d_export_runtime_private_extension()
            .expect("operational restored extension exports");
        stage5d_test_assert_operational_case_shape(case, &restored_state, &restored_extension);
        assert_eq!(
            restored_state, strict_envelope.strategy_state.strategy_state_json,
        ...
        .expect("operational Stage 5C continuation must succeed");
        assert!(warmed.receipt().warmup_started());
        assert!(!warmed.receipt().pending_recovery_started());
        assert!(!warmed.receipt().semantic_bar_enabled());
        let mut report = report;
        ...
        Stage5dFinalR2PackageOutcome {
            ...
            package_checksum_sha256: report
                .package_checksum_sha256
                .clone()
                .expect("operational package checksum"),
            envelope_checksum_sha256: report.payload_checksum_sha256.clone(),
            evidence_checksum_sha256: report
                .riskgate_evidence_checksum_sha256
                .clone()
                .expect("operational evidence checksum"),
            evidence_fingerprint_sha256: report
                .riskgate_evidence_fingerprint_sha256
                .clone()
                .expect("operational evidence fingerprint"),
            semantic_fingerprint_sha256: report.semantic_payload_fingerprint.clone(),
            recovery_index_fingerprint_sha256: report.recovery_index_fingerprint.clone(),
            recovery_plan_fingerprint_sha256: recovery_plan_fingerprint,
            restored_receipt_summary,
    ...
    fn stage5d_test_source_working_protective_strategy(
        ...
        let mut strategy = stage5d_test_source_known_order_strategy(tp_order_id);
        let ctx = stage5d_test_source_operational_ctx_with_position(1.0);
        let comment = stage5d_test_hybrid_comment("5didxord01", "BO", "SL").replace(
            "HYB|sid=hyb-test|",
            &format!("HYB|sid={}|", ctx.strategy_id),
        );
        let intents = strategy.on_stop_order(
            &ctx,
            &crate::runtime_compat::StopOrderEvent {
                stop_order_id: sl_stop_order_id.clone(),
                ...
                comment: Some(comment),
                end_time: None,
                ts_utc: stage5d_test_source_ts_local(2026, 1, 6, 12, 12, 0),
            },
        );
        assert!(
            intents.is_empty(),
            "recovery-index working protective stop source event must not emit intents"
        );
        let state = serde_json::to_value(Strategy::state(&strategy))
            .expect("working protective source state serializes");
        assert_eq!(
            stage5d_runtime_object(&state, "working_protective_source")
                .get("tp_order_id")
                .and_then(Value::as_str),
            Some(tp_order_id.as_str()),
            "protective source must retain TP semantic id"
        );
        assert_eq!(
            stage5d_runtime_object(&state, "working_protective_source")
                .get("sl_stop_order_id")
                .and_then(Value::as_str),
            Some(sl_stop_order_id.as_str()),
            "protective source must retain SL stop semantic id"
        );
        let extension = strategy
            .stage5d_export_runtime_private_extension()
            .expect("working protective private extension exports");
        assert_eq!(
            extension.expected_working_sets.expected_working_order_ids,
            vec![tp_order_id.clone()],
            "protective source export must derive TP working set"
        );
        assert_eq!(
            extension
                .expected_working_sets
                .expected_working_stop_order_ids,
            vec![sl_stop_order_id.clone()],
            "protective source export must derive SL stop working set"
        );
        strategy
    }
    ...
    fn stage5d_test_r3_recovery_index_source_full_restart(
        ...
        let source_stop_order_id = BrokerStopOrderId::new("STAGE5D-R3-IDX-SL-STOP");
        ...
        let source_pending_entry = source_extension.pending_entry.clone();
        let source_pending_cycle_id = if matches!(case, Stage5dR3RecoveryIndexCase::PendingRequest)
        {
            Some(stage5d_json_string_field(
                &source_state,
                "pending_entry_cycle_id",
                "recovery-index source pending",
            ))
        } else {
            None
        };
        let source_pending_created_ts_utc =
            if matches!(case, Stage5dR3RecoveryIndexCase::PendingRequest) {
                Some(stage5d_json_i64_field(
                    &source_state,
                    "pending_entry_created_ts_utc",
                    "recovery-index source pending",
                ))
        ...
        if let (
            Stage5dR3RecoveryIndexCase::PendingRequest,
            Some(source_entry),
            Some(source_cycle_id),
            Some(source_created_ts_utc),
        ) = (
            case,
            ...
            stage5d_test_assert_recovery_index_pending_entry_field_level(
                "source runtime pending entry",
                &source_state,
                &source_extension,
                source_entry,
                source_cycle_id,
                source_created_ts_utc,
            );
        }
        let (input, evidence) = stage5d_test_r3_operational_input_and_evidence_for_source(
            &source_strategy,
        ...
        let canonical_envelope: Stage5dPersistenceEnvelope =
            serde_json::from_str(&package_a.envelope_json)
                .expect("canonical envelope json decodes for field-level proof");
        let decoded = Stage5dCanonicalRestartPackage::from_json_str_strict(&package_json)
        ...
        match case {
            ...
            | Stage5dR3RecoveryIndexCase::WorkingProtective => {
                ...
                if matches!(case, Stage5dR3RecoveryIndexCase::WorkingProtective) {
                    assert_eq!(
                        strict_envelope
                            .runtime_private_extension
                            .expected_working_sets
                            .expected_working_stop_order_ids,
                        vec![source_stop_order_id.clone()],
                        "working protective hint must carry independent source-produced SL stop"
                    );
                }
            }
            Stage5dR3RecoveryIndexCase::PendingRequest => {
                let source_entry = source_pending_entry
                    .as_ref()
                    .expect("pending source entry must be captured");
                let source_cycle_id = source_pending_cycle_id
                    .as_deref()
                    .expect("pending source cycle id must be captured");
                let source_created_ts_utc = source_pending_created_ts_utc
                    .expect("pending source created ts must be captured");
                stage5d_test_assert_recovery_index_pending_entry_field_level(
                    "canonical envelope pending entry",
                    &canonical_envelope.strategy_state.strategy_state_json,
                    &canonical_envelope.runtime_private_extension,
                    source_entry,
                    source_cycle_id,
                    source_created_ts_utc,
                );
                stage5d_test_assert_recovery_index_pending_entry_field_level(
                    "strict decoded pending entry",
                    &strict_envelope.strategy_state.strategy_state_json,
                    &strict_envelope.runtime_private_extension,
                    source_entry,
                    source_cycle_id,
                    source_created_ts_utc,
                );
        ...
        if let (
            ...
            let post_apply_state =
                serde_json::to_value(Strategy::state(applied.loaded.stage5d_strategy()))
                    .expect("post-apply pending state");
            stage5d_test_assert_recovery_index_pending_entry_field_level(
                "post-private-apply pending entry",
                &post_apply_state,
                &post_apply_extension,
                source_entry,
                source_cycle_id,
                source_created_ts_utc,
            );
        ...
        let bootstrapped = match case {
            Stage5dR3RecoveryIndexCase::KnownOrder
            | Stage5dR3RecoveryIndexCase::WorkingProtective => {
                let stop_truth = match case {
                    Stage5dR3RecoveryIndexCase::WorkingProtective => {
                        stage5d_supplemental_stop_truth_for_envelope(
                            &strict_envelope,
                            vec![stage5d_supplemental_stop_order_truth(
                                &strict_envelope,
                                source_stop_order_id.clone(),
                            )],
                        )
                    }
                    _ => stage5d_supplemental_stop_truth_for_envelope(&strict_envelope, Vec::new()),
                };
        ...
        let restored = match case {
            ...
            | Stage5dR3RecoveryIndexCase::WorkingProtective => {
                ...
                let restored = expect_stage5d_restore_ok(
                    ...
                    "recovery-index restored callback must run through production broker-owned-ID normalization",
                );
                assert_eq!(
        ...
        if matches!(case, Stage5dR3RecoveryIndexCase::PendingRequest) {
            ...
            assert!(
                ...
                "{case:?}: production Stage 5D normalization must remove broker-owned TP state after receipt/index preservation"
            );
            assert!(
                stage5d_runtime_object(&restored_state, "recovery-index")
                    .get("sl_stop_order_id")
                    .is_none_or(Value::is_null),
                "{case:?}: production Stage 5D normalization must remove broker-owned SL stop state after receipt/index preservation"
            );
        }
    ...
    fn stage5d_test_assert_restored_recovery_index_behavior(
        ...
        match case {
            ...
            Stage5dR3RecoveryIndexCase::PendingRequest => {
                ...
                let extension_before = probe
                    .stage5d_export_runtime_private_extension()
                    .expect("pending before terminal extension exports");
                stage5d_test_assert_recovery_index_pending_entry_field_level(
    ...
    impl Stage5dR3PositiveCoreCase {
        fn snapshot_id(self) -> &'static str {
            match self {
                Self::CleanFlat => "stage5d-final-r3-positive-core-clean-flat",
                Self::BrokerOpenLong => "stage5d-final-r3-positive-core-broker-open-long",
                Self::BrokerOpenShort => "stage5d-final-r3-positive-core-broker-open-short",
            }
        }

        fn expected_qty(self) -> f64 {
            match self {
                Self::CleanFlat => 0.0,
                Self::BrokerOpenLong => 1.0,
                Self::BrokerOpenShort => -1.0,
            }
        }

        fn expected_side(self) -> Option<&'static str> {
            match self {
                Self::CleanFlat => None,
                Self::BrokerOpenLong => Some("long"),
                Self::BrokerOpenShort => Some("short"),
            }
        }
    }

    fn stage5d_test_source_clean_flat_strategy(
    ) -> crate::hybrid_intraday_runtime::HybridIntradayRuntimeStrategy {
        let mut strategy = stage5d_test_riskgate_runtime_strategy();
        stage5d_test_seed_authoritative_riskgate_materialized_state(&mut strategy);
        let ctx = stage5d_test_source_ctx();
        ...
        assert!(
            intents.is_empty(),
            "r1b clean flat source lifecycle must not emit entry/exit intents"
        );
        match Strategy::state(&strategy) {
            StrategyState::HybridIntradayRuntime {
                last_position_qty,
                ...
                assert!(current_side.is_none());
                assert!(pending_entry_request_id.is_none());
                assert!(pending_exit_request_id.is_none());
            }
            StrategyState::Idle => panic!("r1b clean flat must produce a real Hybrid state"),
        }
        strategy
    }

    fn stage5d_test_source_broker_open_strategy(
        side: Stage5dSide,
    ) -> crate::hybrid_intraday_runtime::HybridIntradayRuntimeStrategy {
        let pending_case = match side {
            Stage5dSide::Long => Stage5dR3aPendingSourceCase::BoLong,
            Stage5dSide::Short => Stage5dR3aPendingSourceCase::BoShort,
        };
        let mut strategy = stage5d_test_source_pending_entry_strategy(pending_case);
        let ctx = stage5d_test_source_paper_ctx();
        let qty = match side {
            Stage5dSide::Long => 1.0,
            Stage5dSide::Short => -1.0,
        };
        let intents = strategy.on_position(
            &ctx,
            &crate::runtime_compat::PositionEvent {
                ...
                existing: false,
                avg_price: 100.0,
                ts_utc: stage5d_test_source_ts_local(2026, 1, 6, 12, 10, 0),
            },
        );
        assert!(
            intents.is_empty(),
            "r1b BO market fill must not create bracket/cleanup intents"
        );
        let exported = strategy
            .stage5d_export_runtime_private_extension()
            .expect("r1b source broker-open extension exports");
        assert!(
            exported.pending_entry.is_none()
                && exported.pending_exit.is_none()
                && exported.partial_entry_timer.is_none(),
            "r1b source broker-open lifecycle must finish entry without pending tails"
        );
        match Strategy::state(&strategy) {
            StrategyState::HybridIntradayRuntime {
                last_position_qty,
                ...
                assert_eq!(
                    *current_owner,
                    Some(crate::hybrid_intraday::Owner::IntradayBreakout)
                );
                let expected_source_side = match side {
                    Stage5dSide::Long => crate::hybrid_intraday::Side::Long,
                    Stage5dSide::Short => crate::hybrid_intraday::Side::Short,
                };
                assert_eq!(
                    *current_side,
                    Some(expected_source_side),
                    "r1b source broker-open side must be source-produced"
                );
                assert!(active_cycle_id.is_some());
                assert!(pending_entry_request_id.is_none());
                assert!(pending_exit_request_id.is_none());
            }
            StrategyState::Idle => panic!("r1b broker-open must produce a real Hybrid state"),
        }
        strategy
    }

    fn stage5d_test_r3_positive_core_source_strategy(
        case: Stage5dR3PositiveCoreCase,
    ) -> crate::hybrid_intraday_runtime::HybridIntradayRuntimeStrategy {
        match case {
            Stage5dR3PositiveCoreCase::CleanFlat => stage5d_test_source_clean_flat_strategy(),
            Stage5dR3PositiveCoreCase::BrokerOpenLong => {
                stage5d_test_source_broker_open_strategy(Stage5dSide::Long)
            }
            Stage5dR3PositiveCoreCase::BrokerOpenShort => {
                stage5d_test_source_broker_open_strategy(Stage5dSide::Short)
            }
        }
    }

    fn stage5d_test_r3_positive_core_input_and_evidence_for_source(
        source_strategy: &crate::hybrid_intraday_runtime::HybridIntradayRuntimeStrategy,
        snapshot_id: &str,
    ) -> (
        Stage5dCanonicalEnvelopeExportInput,
        ...
        let source_state_json = serde_json::to_value(Strategy::state(source_strategy))
            .expect("r1b positive-core source state serializes");
        let source_extension = source_strategy
            .stage5d_export_runtime_private_extension()
            .expect("r1b positive-core source private extension exports");
        let mut envelope = flat_persisted_fixture();
        envelope.snapshot_id = snapshot_id.to_string();
        envelope.binding.strategy_id = "stage5d-final-r3-positive-core-r1b".to_string();
        envelope.strategy_state.strategy_state_json = source_state_json;
        envelope.runtime_private_extension = source_extension;
        let identity = stage5d_test_riskgate_identity_for(source_strategy, &envelope);
        let source_shadow_session = envelope.strategy_state.strategy_state_json
        ...
        evidence.current_shadow_pnl_points =
            crate::hybrid_intraday::format_riskgate_authority_decimal(source_pnl_points)
                .expect("r1b source shadow pnl must be authority-canonical");
        evidence.ledger_tail_hash =
            stage5d_compute_riskgate_ledger_tail_hash(&evidence).expect("r1b ledger tail hash");
        stage5d_apply_riskgate_evidence_to_envelope(&mut envelope, &evidence);
        bind_fixture_to_strategy_config(&mut envelope, source_strategy);
        envelope.riskgate.ledger_tail_hash = evidence.ledger_tail_hash.clone();
        envelope.payload_checksum_sha256 = envelope
            .compute_payload_checksum_sha256()
            .expect("r1b source core checksum");
        stage5d_test_normalize_persisted_checksum(&mut envelope);
        let input = Stage5dCanonicalEnvelopeExportInput {
            snapshot_id: envelope.snapshot_id.clone(),
            snapshot_revision: envelope.snapshot_revision,
    ...
    fn stage5d_test_r3_positive_core_source_full_restart(
        case: Stage5dR3PositiveCoreCase,
    ) -> Stage5dFinalR2PackageOutcome {
        let source_strategy = stage5d_test_r3_positive_core_source_strategy(case);
        let source_state =
            serde_json::to_value(Strategy::state(&source_strategy)).expect("r1b source state");
        let source_semantic_fingerprint =
            crate::stage5c_paper_host::stage5c_semantic_value_fingerprint(&source_state)
                .expect("r1b source semantic fingerprint");
        let source_extension = source_strategy
            .stage5d_export_runtime_private_extension()
            .expect("r1b source private extension exports");
        assert!(
            source_extension.pending_entry.is_none()
                && source_extension.pending_exit.is_none()
                && source_extension.partial_entry_timer.is_none(),
            "{case:?}: r1b accepted core source must not carry pending tails"
        );
        assert!(
            source_extension.runtime_pending_finalizations.is_empty(),
            "{case:?}: r1b accepted core source must not carry pending riskgate finalizations"
        );
        let source_runtime = source_state
            .get("HybridIntradayRuntime")
            .unwrap_or_else(|| panic!("{case:?}: r1b source state must be Hybrid"));
        assert_eq!(
            source_runtime
                .get("last_position_qty")
                .and_then(Value::as_f64),
            Some(case.expected_qty()),
            "{case:?}: source lifecycle must produce expected qty"
        );
        assert_eq!(
            source_runtime.get("current_side").and_then(Value::as_str),
            case.expected_side(),
            "{case:?}: source lifecycle must produce expected side"
        );
        let (input, evidence) = stage5d_test_r3_positive_core_input_and_evidence_for_source(
            &source_strategy,
            case.snapshot_id(),
        );
        let (package, report) = stage5d_export_canonical_restart_package_from_runtime(
        ...
        .expect("r1b source-owned positive-core package export must succeed");
        assert!(!report.redis_opened);
        assert!(!report.finam_opened);
        assert!(!report.dispatch_opened);
        assert!(!report.runtime_live_opened);
        let package_json = package
            .to_json_strict()
            .expect("r1b positive-core package serializes");
        drop(source_strategy);

        let decoded = Stage5dCanonicalRestartPackage::from_json_str_strict(&package_json)
            .expect("r1b strict package decode must succeed");
        assert_eq!(
            decoded.envelope.strategy_state.strategy_state_json, source_state,
            "{case:?}: package must contain exact source-produced semantic state"
        );
        assert_eq!(
            decoded.report.semantic_payload_fingerprint, source_semantic_fingerprint,
            "{case:?}: semantic fingerprint must survive durable bytes"
        );
        ...
        let injected = expect_stage5d_riskgate_ok(
            stage5d_inject_authoritative_riskgate(bootstrapped, validated_evidence),
            &format!("{case:?}: r1b positive-core riskgate injection must succeed"),
        );
        assert!(injected.recovery_complete());
        let recovery_plan_fingerprint = injected.recovery_plan_fingerprint().to_string();
        let restored = stage5d_test_assert_injected_restores_indexes_once(
            ...
            &strict_envelope.recovery_indexes.known_order_ids,
            &strict_envelope.recovery_indexes.pending_requests,
            "r1b positive-core restored callback must run exactly once",
        );
        let restored_state_json =
            serde_json::to_value(Strategy::state(restored.stage5d_strategy()))
                .expect("r1b restored state serializes");
        assert_eq!(
            restored_state_json, strict_envelope.strategy_state.strategy_state_json,
            "{case:?}: actual post-apply/restored state must match strict source envelope"
        );
        let restored_runtime = restored_state_json
            .get("HybridIntradayRuntime")
            .unwrap_or_else(|| panic!("{case:?}: restored state must be Hybrid"));
        assert_eq!(
            restored_runtime
                .get("last_position_qty")
                .and_then(Value::as_f64),
            Some(case.expected_qty()),
            "{case:?}: broker-truth quantity must survive restored callback"
        );
        assert_eq!(
            restored_runtime.get("current_side").and_then(Value::as_str),
            case.expected_side(),
            "{case:?}: broker-truth side must survive restored callback"
        );
        let restored_receipt_summary = format!(
            "runtime_state_restored={};warmup_started={};pending_recovery_started={};semantic_bar_enabled={};known_orders={};pending_requests={}",
            restored.receipt().runtime_state_restored(),
        ...
        .expect("r1b positive-core Stage 5C continuation must succeed");
        assert!(warmed.receipt().warmup_started());
        assert!(!warmed.receipt().pending_recovery_started());
        assert!(!warmed.receipt().semantic_bar_enabled());
        let mut report = report;
        ...
        Stage5dFinalR2PackageOutcome {
            ...
            package_checksum_sha256: report
                .package_checksum_sha256
                .clone()
                .expect("r1b package checksum"),
            envelope_checksum_sha256: report.payload_checksum_sha256.clone(),
            evidence_checksum_sha256: report
                .riskgate_evidence_checksum_sha256
                .clone()
                .expect("r1b evidence checksum"),
            evidence_fingerprint_sha256: report
                .riskgate_evidence_fingerprint_sha256
                .clone()
                .expect("r1b evidence fingerprint"),
            semantic_fingerprint_sha256: report.semantic_payload_fingerprint.clone(),
            recovery_index_fingerprint_sha256: report.recovery_index_fingerprint.clone(),
            recovery_plan_fingerprint_sha256: recovery_plan_fingerprint,
            restored_receipt_summary,
    ...
    fn stage5d_test_r3_current_shadow_discovery_without_preseed(
        ...
        let source_pnl_points = source_state_json["HybridIntradayRuntime"]
            ["risk_gate_shadow_pnl_points"]
            .as_f64()
            .expect("r1b shadow source pnl");
        let source_pnl_text =
            crate::hybrid_intraday::format_riskgate_authority_decimal(source_pnl_points)
                .expect("r1b shadow pnl canonical");
        assert_eq!(source_pnl_text, expected_pnl_text);
        let (input, evidence) = stage5d_test_r3_positive_core_input_and_evidence_for_source(
            &source_strategy,
            snapshot_id,
        );
    ...
    fn stage5d_test_r3_current_shadow_full_restart_with_stage5c_continuation(
        ...
        assert!(
            exported.runtime_pending_finalizations.is_empty(),
            "r3 current-shadow core path must not smuggle pending finalizations"
        );
        assert!(
            exported.pending_entry.is_none()
                && exported.partial_entry_timer.is_none()
                && exported.pending_exit.is_none(),
            "r3 current-shadow core path must be source callback produced without pending lifecycle post-edits"
        );
        let source_state_json =
        ...
        let source_pnl_text =
            crate::hybrid_intraday::format_riskgate_authority_decimal(source_pnl_points)
                .expect("r3 source shadow pnl must be authority-canonical");
        assert_eq!(
            source_pnl_text, expected_pnl_text,
            "r3 source current-shadow expectation must match source-produced canonical pnl"
        );
        let mut evidence = stage5d_test_riskgate_evidence_before_source_pending(
            &identity,
            &envelope,
        ...
        evidence.current_shadow_pnl_points = source_pnl_text;
        evidence.ledger_tail_hash =
            stage5d_compute_riskgate_ledger_tail_hash(&evidence).expect("r3 shadow tail hash");
        stage5d_apply_riskgate_evidence_to_envelope(&mut envelope, &evidence);
        bind_fixture_to_strategy_config(&mut envelope, &source_strategy);
        envelope.riskgate.ledger_tail_hash = evidence.ledger_tail_hash.clone();
        envelope.payload_checksum_sha256 = envelope
        ...
        .expect("r3 source current-shadow canonical package export must succeed");
        assert!(!report.redis_opened);
        assert!(!report.finam_opened);
        assert!(!report.dispatch_opened);
        assert!(!report.runtime_live_opened);
        let package_json = package
            .to_json_strict()
            .expect("r3 source current-shadow package serializes strictly");
        drop(source_strategy);

        let decoded = Stage5dCanonicalRestartPackage::from_json_str_strict(&package_json)
            .expect("r3 source current-shadow package decodes strictly");
        assert_eq!(
            decoded.envelope.strategy_state.strategy_state_json, source_state_json,
            "r3 current-shadow package must persist exact source callback state"
        );
        assert_eq!(
            decoded.report.semantic_payload_fingerprint, source_semantic_fingerprint,
        ...
        let injected = expect_stage5d_riskgate_ok(
            stage5d_inject_authoritative_riskgate(bootstrapped, validated_evidence),
            "r3 current-shadow package restart must inject authoritative riskgate",
        );
        let recovery_plan_fingerprint = injected.recovery_plan_fingerprint().to_string();
        assert!(injected.recovery_complete());
        let restored = stage5d_test_assert_injected_restores_indexes_once(
            ...
            &strict_envelope.recovery_indexes.known_order_ids,
            &strict_envelope.recovery_indexes.pending_requests,
            "r3 current-shadow package path must restore indexes exactly once",
        );
        let restored_state_json =
            serde_json::to_value(Strategy::state(restored.stage5d_strategy()))
                .expect("r3 current-shadow restored state serializes");
        assert_eq!(
            restored_state_json, strict_envelope.strategy_state.strategy_state_json,
            "r3 current-shadow restored runtime state must match decoded strict envelope"
        );
        let restored_receipt_summary = format!(
            "runtime_state_restored={};warmup_started={};pending_recovery_started={};semantic_bar_enabled={};known_orders={};pending_requests={}",
            restored.receipt().runtime_state_restored(),
        ...
        .expect("r3 source current-shadow Stage 5C history warmup continuation must succeed");
        assert!(warmed.receipt().warmup_started());
        assert!(!warmed.receipt().pending_recovery_started());
        assert!(!warmed.receipt().semantic_bar_enabled());
        let mut report = decoded.report;
        ...
        Stage5dFinalR2PackageOutcome {
            ...
            package_checksum_sha256: report
                .package_checksum_sha256
                .clone()
                .expect("r3 package checksum"),
            envelope_checksum_sha256: report.payload_checksum_sha256.clone(),
            evidence_checksum_sha256: report
                .riskgate_evidence_checksum_sha256
                .clone()
                .expect("r3 evidence checksum"),
            evidence_fingerprint_sha256: report
                .riskgate_evidence_fingerprint_sha256
                .clone()
                .expect("r3 evidence fingerprint"),
            semantic_fingerprint_sha256: report.semantic_payload_fingerprint.clone(),
            recovery_index_fingerprint_sha256: report.recovery_index_fingerprint.clone(),
            recovery_plan_fingerprint_sha256: recovery_plan_fingerprint,
            restored_receipt_summary,
    ...
    fn stage5d_supplemental_stop_order_truth(
        envelope: &Stage5dPersistenceEnvelope,
        stop_order_id: BrokerStopOrderId,
    ) -> Stage5dSupplementalWorkingStopOrderTruth {
        Stage5dSupplementalWorkingStopOrderTruth {
            account_id: envelope.binding.account_id.clone(),
            instrument: envelope.binding.instrument_id.to_instrument_id(),
            stop_order_id,
            ...
            side: "sell".to_string(),
            qty: "1".to_string(),
            stop_price: "2180".to_string(),
            limit_price: Some("2179.5".to_string()),
            comment: Some(
                stage5d_test_hybrid_comment("5didxord01", "BO", "SL").replace(
                    "HYB|sid=hyb-test|",
                    &format!("HYB|sid={}|", envelope.binding.strategy_id),
                ),
            ),
            received_ts: envelope.persisted_at_ts_utc,
        }
    ...
    fn stage5d_supplemental_stop_truth_for_envelope(
        envelope: &Stage5dPersistenceEnvelope,
        working_stops: Vec<Stage5dSupplementalWorkingStopOrderTruth>,
    ) -> Stage5dSupplementalWorkingStopTruth {
        let mut truth = Stage5dSupplementalWorkingStopTruth {
            schema_version: STAGE5D_SUPPLEMENTAL_WORKING_STOP_TRUTH_SCHEMA_VERSION,
            snapshot_id: envelope.snapshot_id.clone(),
        ...
        truth.fingerprint_sha256 =
            stage5d_compute_supplemental_working_stop_truth_fingerprint(&truth)
                .expect("supplemental stop truth fingerprint");
        truth
    }

    fn stage5d_test_applied_for_envelope_with_active_orders(
        envelope: Stage5dPersistenceEnvelope,
        active_orders: Vec<broker_core::BrokerOrderSnapshot>,
    ) -> Stage5dPrivateStateAppliedPaperStrategy {
        let mut fresh_strategy = stage5d_test_riskgate_runtime_strategy();
        ...
        let admission = stage5d_test_admission_for_envelope(
            &envelope,
            stage5d_persisted_position_qty(&envelope).expect("position qty"),
        )
        .stage5d_test_with_target_active_orders(active_orders);
        let loaded = crate::stage5c_paper_host::Stage5cRuntimeStateLoadedPaperStrategy::stage5d_test_loaded_from_parts(
            fresh_strategy,
            admission,
            restored_indexes,
        ...
        let validated = envelope
            .clone()
            .validate_restore_contract_schema_only()
            .expect("negative working-set envelope must validate schema");
        let bound = expect_stage5d_ok(
            stage5d_bind_runtime_state_loaded(loaded, validated),
            "negative working-set loaded state must bind before exact truth check",
        );
        expect_stage5d_ok(
            stage5d_apply_runtime_private_extension(bound),
            "negative working-set private extension must apply before exact truth check",
        )
    }
    ...
    impl Stage5dRiskRecBoundedFileStore {
        fn new(label: &str) -> Self {
            let root = std::env::temp_dir().join(format!(
                "stage5d-riskrec-{label}-{}",
                Utc::now().timestamp_nanos_opt().expect("timestamp nanos")
            ));
            std::fs::create_dir_all(&root).expect("riskrec temp store dir");
            Self {
                root,
                generation: 0,
            }
        ...
        fn write_state(
            ...
            match state {
                ...
                Stage5dRiskRecStoreState::PartialWrite => {
                    ...
                    std::fs::write(self.package_path(), partial).expect("partial write");
                }
    ...
    fn stage5d_riskrec_store_commit_and_reload(
        ...
        let committed = store.read_committed().expect("riskrec committed read");
        assert_eq!(committed.receipt, receipt);
        stage5d_riskrec_ready_from_package(&committed.package_json).expect("riskrec reload ready")
    }
    ...
    struct Stage5dRiskRecRunOutcome {
        ...
        final_evidence_fingerprint_sha256: String,
        final_recovery_plan_fingerprint_sha256: String,
        ordered_identity_hashes: Vec<String>,
        ordered_generations: Vec<u64>,
        stage5c_warmup_completed: bool,
    }

    fn stage5d_riskrec_run_production_recovery(
        source: &Stage5dRiskRecSourcePackage,
        label: &str,
    ) -> Stage5dRiskRecRunOutcome {
        let mut store = Stage5dRiskRecBoundedFileStore::new(label);
        assert!(!source.evidence.ledger_tail_hash.is_empty());
        store.write_state(
            Stage5dRiskRecStoreState::NotWritten,
            &source.package_json,
            None,
        ...
        assert!(
            store.read_committed().is_err(),
            "not-written store must reject"
        );
        store.write_state(
            Stage5dRiskRecStoreState::PartialWrite,
            &source.package_json,
        ...
        assert!(store.read_committed().is_err(), "partial write must reject");
        store.write_state(
            Stage5dRiskRecStoreState::FullWrittenUncommitted,
            &source.package_json,
            None,
        ...
        assert!(
            store.read_committed().is_err(),
            "full written uncommitted must reject"
        );
        ...
        let final_package_sha256 = sha256_text(&final_package_json);
        let receipt = stage5d_riskrec_make_commit_receipt(
            &ready,
            &final_package_json,
        ...
        let final_read = store
            .read_committed()
            .expect("riskrec final committed read");
        assert_eq!(
        ...
        let final_noop = stage5d_apply_next_riskgate_recovery_action(final_ready.clone())
            .expect("complete plan must be explicit no-op");
        assert_eq!(
            final_noop.action,
            Stage5dRiskGateRecoveryDecision::AlreadyAcknowledged
        );
        let (bootstrapped, _strict_envelope, validated_evidence) =
            stage5d_test_bootstrap_strict_envelope_with_strategy(
                final_ready.envelope.clone(),
                stage5d_test_riskgate_runtime_strategy(),
                final_ready.evidence.clone(),
            );
        let injected = expect_stage5d_riskgate_ok(
            stage5d_inject_authoritative_riskgate(bootstrapped, validated_evidence),
            "riskrec final restored injection",
        );
        let restored = stage5d_test_assert_injected_restores_once(
            injected,
            "riskrec final production path must restore exactly once",
        );
        let warmed = crate::stage5c_paper_host::stage5d_test_warmup_stage5c_history_at(
            restored,
            stage5d_test_history_batch_for_final_package(&final_ready.envelope),
            final_ready.envelope.persisted_at_ts_utc + chrono::Duration::seconds(1),
        )
        .expect("riskrec Stage 5C warmup continuation");
        assert!(warmed.receipt().warmup_started());
        Stage5dRiskRecRunOutcome {
            actions,
            action_fingerprints,
            ...
            final_recovery_plan_fingerprint_sha256: final_ready.recovery_plan_fingerprint_sha256,
            ordered_identity_hashes: final_ready
                .envelope
                .riskgate
                .durable_finalization_outbox
                .iter()
                .map(|row| row.identity_hash.clone())
                .collect(),
            ordered_generations: final_ready
                .envelope
                .riskgate
                .durable_finalization_outbox
                .iter()
    ...
    fn stage5d_final_r3_riskgate_recovery_r1_source_produced_matrix() {
        ...
        let single_outcome = stage5d_riskrec_run_production_recovery(&single, "single");
        let single_actions = single_outcome.actions.clone();
        assert_eq!(
            single_actions,
            vec![
            ...
            "single pending finalization must execute exact production checkpoint progression"
        );
        assert!(
            single_outcome.stage5c_warmup_completed,
            "riskrec Stage 5C continuation proof must be actual warmup"
        );
        ...
        let multi_outcome = stage5d_riskrec_run_production_recovery(&multi, "multi");
        let multi_actions = multi_outcome.actions.clone();
        assert!(
            multi_actions
                .iter()
        ...
        let complete_actions = vec!["already_acknowledged"];
        assert_eq!(
            complete_actions,
        ...
        let generated_single = serde_json::json!({
            ...
            "recovery_plan_fingerprint_sha256": single_outcome.final_recovery_plan_fingerprint_sha256,
            "ordered_finalization_identity_hashes": single_outcome.ordered_identity_hashes,
            "ordered_finalization_generations": single_outcome.ordered_generations,
            "ordered_recovery_action_fingerprints": single_outcome.action_fingerprints,
            "expected_checkpoint_progression": single_actions,
            "final_commit_receipt_fingerprint": single_outcome.final_receipt_fingerprint,
            "stage5c_warmup_completed": true,
            "stage5e_closed": true
        });
        let generated_multi = serde_json::json!({
            ...
            "recovery_plan_fingerprint_sha256": multi_outcome.final_recovery_plan_fingerprint_sha256,
            "ordered_finalization_identity_hashes": multi_outcome.ordered_identity_hashes,
            "ordered_finalization_generations": multi_outcome.ordered_generations,
            "ordered_recovery_action_fingerprints": multi_outcome.action_fingerprints,
            "expected_ordered_sessions": multi.pending_sessions,
            "expected_checkpoint_progression": multi_actions,
            "final_commit_receipt_fingerprint": multi_outcome.final_receipt_fingerprint,
            "stage5c_warmup_completed": true,
            "stage5e_closed": true
        });
        let generated_complete = serde_json::json!({
            ...
            "ordered_finalization_identity_hashes": single_outcome.ordered_identity_hashes,
            "ordered_finalization_generations": single_outcome.ordered_generations,
            "expected_checkpoint_progression": complete_actions,
            "final_commit_receipt_fingerprint": single_outcome.final_receipt_fingerprint,
            "stage5c_warmup_completed": true,
            "stage5e_closed": true
        });
        assert_eq!(golden_single, generated_single);
    ...
    #[test]
    fn stage5d_final_r3_operational_state_r1_source_produced_full_restart_matrix() {
        ...
        for case in [
            ...
            let outcome = stage5d_test_r3_operational_source_full_restart(case);
            assert!(
                outcome.warmed_processed_bars > 0,
                "{case:?}: Stage 5C continuation must process history"
            );
            assert_eq!(
                outcome.runtime_pending_finalizations_count, 0,
                "{case:?}: operational-state r1 must not promote riskgate-finalization cases"
            );
            assert_eq!(
                outcome.durable_finalization_outbox_count, 0,
                "{case:?}: operational-state r1 must not open finalization outbox cases"
            );
            executed += 1;
        }
    ...
    #[test]
    fn stage5d_final_r3_recovery_index_r1r1_working_set_negatives_fail_closed() {
        // recovery_index_r1r1_negative_matrix_executed
        // recovery_index_r1r1_missing_expected_tp_fails_closed
        // recovery_index_r1r1_extra_unknown_tp_fails_closed
        // recovery_index_r1r1_expected_stop_missing_fails_closed
        // recovery_index_r1r1_extra_unknown_stop_fails_closed
        // recovery_index_r1r1_wrong_account_fails_closed
        // recovery_index_r1r1_wrong_instrument_fails_closed
        // recovery_index_r1r1_tp_as_stop_wrong_kind_fails_closed
        // recovery_index_r1r1_sl_as_order_wrong_kind_fails_closed
        // recovery_index_r1r1_tp_sl_swap_fails_closed
        // recovery_index_r1r1_duplicate_id_across_sets_fails_closed
        ...
        let mut extra_order = stage5d_active_order(
            envelope.binding.account_id.clone(),
            envelope.binding.instrument_id.to_instrument_id(),
            BrokerOrderId::new("STAGE5D-R3-IDX-EXTRA-ORDER"),
            envelope.persisted_at_ts_utc,
        );
        cases.push((
            "extra unknown normal order",
            vec![active_order.clone(), extra_order.clone()],
            stop_truth.clone(),
            Stage5dBootstrapBlockReason::ExtraWorkingOrder,
        ));
        cases.push((
            "expected stop missing",
            vec![active_order.clone()],
            stage5d_supplemental_stop_truth_for_envelope(&envelope, Vec::new()),
            Stage5dBootstrapBlockReason::ExpectedWorkingStopMissing,
        ));
        let extra_stop = stage5d_supplemental_stop_order_truth(
            &envelope,
            BrokerStopOrderId::new("STAGE5D-R3-IDX-EXTRA-STOP"),
        );
        cases.push((
            "extra unknown stop",
            vec![active_order.clone()],
            ...
            Stage5dBootstrapBlockReason::ExtraWorkingStop,
        ));
        extra_order.account_id = BrokerAccountId::new("ACC_WRONG_0001");
        cases.push((
            "wrong account",
            vec![extra_order.clone()],
            stop_truth.clone(),
            Stage5dBootstrapBlockReason::SnapshotInstrumentMismatch,
        ));
        extra_order.account_id = envelope.binding.account_id.clone();
        extra_order.instrument = InstrumentId {
            symbol: "RI".to_string(),
            venue_symbol: Some("RI@RTSX".to_string()),
            exchange: Exchange::Moex,
            market: Market::Other("unknown".to_string()),
        };
        cases.push((
            "wrong instrument",
            vec![extra_order.clone()],
        ...
        cases.push((
            "TP represented as stop wrong kind",
            Vec::new(),
            stage5d_supplemental_stop_truth_for_envelope(
                &envelope,
                vec![
                    stage5d_supplemental_stop_order_truth(
                        &envelope,
        ...
        cases.push((
            ...
            vec![
                active_order.clone(),
                stage5d_active_order(
                    envelope.binding.account_id.clone(),
                    envelope.binding.instrument_id.to_instrument_id(),
                    BrokerOrderId::new(sl_id.as_str()),
                    envelope.persisted_at_ts_utc,
                ),
            ],
            stage5d_supplemental_stop_truth_for_envelope(&envelope, Vec::new()),
        ...
        duplicate_envelope
            .runtime_private_extension
            .expected_working_sets
            .expected_working_stop_order_ids = vec![BrokerStopOrderId::new(tp_id.as_str())];
        duplicate_envelope.recovery_indexes.known_stop_order_ids = duplicate_envelope
            .runtime_private_extension
            .expected_working_sets
            .expected_working_stop_order_ids
            .clone();
        if let Value::Object(fields) =
            &mut duplicate_envelope.strategy_state.strategy_state_json["HybridIntradayRuntime"]
        {
            fields.insert(
                "sl_stop_order_id".to_string(),
                Value::String(tp_id.as_str().to_string()),
            );
        }
        duplicate_envelope.payload_checksum_sha256 = duplicate_envelope
            .compute_payload_checksum_sha256()
            .expect("duplicate checksum");
        cases.push((
            "duplicate ID across order and stop sets",
            vec![active_order.clone()],
            stage5d_supplemental_stop_truth_for_envelope(
                &duplicate_envelope,
                vec![stage5d_supplemental_stop_order_truth(
            ...
            Stage5dBootstrapBlockReason::WorkingSetDuplicateId,
        ));

        for (label, active_orders, truth, expected_reason) in cases {
            let applied = if label == "duplicate ID across order and stop sets" {
                stage5d_test_applied_for_envelope_with_active_orders(
                    duplicate_envelope.clone(),
                    active_orders,
                )
            } else {
                stage5d_test_applied_for_envelope_with_active_orders(
            ...
            assert_eq!(blocked.reason(), expected_reason, "{label}");
            assert!(
                blocked.input_capability_preserved(),
                "{label}: controlled block must retain applied capability"
            );
        }
    }
    ...
    #[test]
    fn stage5d_final_r3_positive_core_source_produced_full_restart_matrix() {
        let mut actual_source_core_cases_executed = 0usize;
        for case in [
            Stage5dR3PositiveCoreCase::CleanFlat,
            Stage5dR3PositiveCoreCase::BrokerOpenLong,
            Stage5dR3PositiveCoreCase::BrokerOpenShort,
        ] {
            let outcome = stage5d_test_r3_positive_core_source_full_restart(case);
            assert_eq!(outcome.warmed_processed_bars, 1);
            assert!(outcome
                .restored_receipt_summary
                .contains("runtime_state_restored=true"));
            ...
            assert!(!outcome.recovery_plan_fingerprint_sha256.is_empty());
            assert_eq!(outcome.runtime_pending_finalizations_count, 0);
            assert_eq!(outcome.durable_finalization_outbox_count, 0);
            actual_source_core_cases_executed += 1;
        }
        assert_eq!(actual_source_core_cases_executed, 3);

        // Stage 5D-final-restart-r3 positive-core-r1b marker:
        // positive_core_accepted_count_3, positive_core_clean_flat_actual_source_lifecycle,
        ...
        // positive_core_accepted_cases_executed_3, actual_source_core_cases_executed_3,
        // positive_core_todo_count_14, stage5e_closed.
    }

    #[test]
    fn stage5d_final_r3_current_shadow_discovery_localizes_materialized_gap() {
        for (snapshot_id, bars, expected_pnl) in [
            (
                "stage5d-final-r3-current-shadow-discovery-long",
                vec![stage5d_test_source_bar_ohlc(
                    stage5d_test_source_ts_local(2026, 1, 6, 9, 0, 0),
                    99.7,
                    102.0,
            ...
            (
                "stage5d-final-r3-current-shadow-discovery-short",
                vec![stage5d_test_source_bar_ohlc(
                    stage5d_test_source_ts_local(2026, 1, 6, 9, 0, 0),
                    100.3,
                    100.3,
            ...
            (
                "stage5d-final-r3-current-shadow-discovery-realized-pnl",
                vec![
                    stage5d_test_source_bar_ohlc(
                        stage5d_test_source_ts_local(2026, 1, 6, 9, 0, 0),
                        99.7,
    ...
    fn stage5d_final_r3_resumption_inventory_and_r3a_r1_reuse() {
        ...
        assert_eq!(inventory["status"], "aggregate_closure_r2_candidate");
        for surface in [
            "stage5e",
            "redis",
            "finam",
        ...
        .into_iter()
        .collect();
        let accepted_core_ids: std::collections::HashSet<_> = [
            "positive_clean_flat",
            "positive_broker_consistent_open_long",
            "positive_broker_consistent_open_short",
        ]
        ...
        .into_iter()
        .collect();
        let accepted_operational_ids: std::collections::HashSet<_> = [
            "positive_partial_entry",
            "positive_pending_exit",
            "positive_deferred_entry",
            "positive_deferred_exit",
        ...
        for case_id in accepted_core_ids {
            let row = rows
                ...
                .expect("r3 positive core row present");
            assert_eq!(
                row["owning_test"],
            ...
            assert_eq!(row["canonical_package_path"], serde_json::json!(true));
            assert_eq!(row["source_object_destroyed"], serde_json::json!(true));
            assert_eq!(row["strict_decode_used"], serde_json::json!(true));
            assert_eq!(row["fresh_runtime_used"], serde_json::json!(true));
            assert_eq!(
        ...
        for case_id in accepted_operational_ids.iter() {
            let row = rows
                .iter()
                .find(|row| row["case_id"] == *case_id)
                .expect("r3 operational-state row present");
            assert_eq!(
                row["owning_test"],
