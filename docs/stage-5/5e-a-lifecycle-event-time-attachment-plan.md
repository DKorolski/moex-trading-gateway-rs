...
- market event time: `last_history_bar_close < first_fresh_live_bar_close` and
  `callback_bar_close == first_fresh_live_bar_close`; market event time is
  never compared with lifecycle wall-clock time;
- broker recovery event time: broker source-event watermarks are compared only
  against other broker source-event watermarks;
- recovery completion: `RecoveryAcceptedReceipt` and
  `LifecycleRecoveryCompletedReceipt` causally precede callback invocation,
  without a numeric timestamp comparison between broker source time and local
  wall clock;
