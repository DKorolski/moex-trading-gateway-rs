...
- `Place` -> `place / 0`
- `Cancel` -> `cancel:<BrokerOrderId> / 1`
- `Replace` -> `replace / 2`
- `Market Buy` -> `market / 3`
- `Market Sell` -> `market / 4`
- `CreateStopLimit` -> `create_stop_limit / 5`
- `DeleteStopLimit` -> `delete_stop_limit:<BrokerStopOrderId> / 6`
...
Cleanup request IDs include the broker object identity so cancel-all-protection
can carry multiple TP/SL cleanup intents from the same source timestamp without
colliding.
