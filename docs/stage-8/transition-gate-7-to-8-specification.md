...
6. inactive persistent kill switch;
...
## 5. Command and FINAM mapping contract
...
## 6. Broker outcome classification
...
- proven no-match only after the separately specified bounded observation
  policy and fresh snapshots;
- conflict/multiple candidates;
- still unknown.

Conflict and still-unknown states block new live commands. A proven no-match
does not silently redispatch the old request; it permits only a new request and
new operator arm after manual decision.

## 8. Max-one micro authority
...
The kill switch is persistent and checked:
...
It blocks new PLACE. Emergency CANCEL policy must be independently scoped to
an exact owned working order and cannot become a general execution bypass.
Read failure, stale state or conflicting switch generations means active.
...
| HTTP 4xx | typed rejection unless response is malformed/semantically contradictory |
...
## 12. Redis and durability inheritance
...
## 14. Evidence required before Stage 8B
...
## 15. Gate acceptance rule

This specification is accepted only when all 45 rows in
`TRANSITION_GATE_7_TO_8_ACCEPTANCE_MATRIX_2026-08-14.csv` pass, the negative
harness passes and an independent reviewer records acceptance against an exact
commit and immutable archive.
