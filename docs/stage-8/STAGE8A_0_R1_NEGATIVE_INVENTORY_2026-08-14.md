# Stage 8A-0 R1 negative inventory — exact 41 cases
...
37. `remove --all-targets from Stage8A-0 workspace regression command`
38. `remove --test-threads=1 from inherited workspace/doc regression commands`
39. `omit test/doctest/clippy/fmt/checker logs from handoff`
40. `stop hash-binding packaged regression artifacts in evidence JSON`
41. `omit the known inherited Redis timing-flake disclosure/isolated rerun witness`
