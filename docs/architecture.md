...
8. M3 dry order-path work may build durable/local contracts, approved-only mock
   execution simulation, dry cancel simulation, broker-truth recovery helpers,
   and synthetic ACKs, but FINAM order endpoints remain blocked until the
   live-order gate is explicitly opened.
