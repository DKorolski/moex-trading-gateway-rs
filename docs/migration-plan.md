...
- Adds `crates/finam-gateway/src/m3d2_real_transport_lifecycle.rs`.
- Binds the reviewed M3d-2c `reqwest` transport to the durable order-path
  store and SQLite transition audit, but only against loopback local mocks.
- PLACE lifecycle persists `BeginSubmit` before HTTP send and maps classified
  outcomes to `Submitted`, `SubmittedPendingBrokerOrderId`,
  `TimeoutUnknownPending`, broker reject, or manual/reconciliation-required
  states.
- CANCEL lifecycle persists `RequestCancel` before HTTP send and treats cancel
  acceptance as pending broker-truth reconciliation, not as terminal canceled.
- Crash-window tests prove conservative restart handling before send, after
  begin-before-send, after send-before-response-persistence, and after
  classified outcome before external ACK publication.
- ACK candidate export is diagnostic/redacted only: status/reason plus id
  presence/length, never raw JWT/path/body/account/client/broker ids.
- Evidence is generated by `scripts/m3d2d_lifecycle_evidence.py`; exact send
  surface remains unchanged in the reviewed M3d-2c transport module.

M3d-2d explicitly still not allowed:
...
- Adds full lifecycle matrix coverage for PLACE and CANCEL endpoint outcomes at
  the durable order-path layer.
- PLACE matrix covers accepted with id, accepted without id, validation reject,
  unauthorized, rate limit, maintenance/server errors, malformed/body-read
  ambiguity, timeout, and post-send send error.
- CANCEL matrix covers accepted 200/202/204, same-id accepted, mismatched-id
  accepted, 404/409/410 reconciliation, unauthorized, rate limit, maintenance,
  timeout, decode failure, and body-read failure.
- Adds explicit external endpoint firewall to
  `M3d2RealOrderEndpointTransportConfig`: default config cannot silently route
  to `https://api.finam.ru`; loopback is allowed only for local mock tests;
  future live-gate mode remains blocked.
- Scanner/evidence prove no real transport construction/default config use
  outside approved test modules and no gateway command-consumer route to the
  real transport.
- Evidence is generated by `scripts/m3d2e_closure_evidence.py`.

M3d-2 protected endpoint stage closure:

- M3d-2 is closed only when M3d-2e evidence reports
  `m3d2_protected_endpoint_stage_closed = true`,
  `lifecycle_matrix_ok = true`, and
  `external_endpoint_enablement_firewall_ok = true`.
- Closure still does not authorize external FINAM order calls, strategy
  runtime attachment, `LiveReady`, stop/SLTP/brackets, replace, or multi-leg
  surfaces.

M3d-2f external firewall hardening:

- Closes the final pre-M3e firewall gap by blocking all non-loopback order
  endpoints, not only `https://api.finam.ru`.
- `ExternalFinamDisabled` is strict in M3d: it allows only loopback local mock
  endpoints and blocks `OtherExternal` hosts.
- `FutureExternalFinamRequiresLiveGate` remains blocked even for loopback.
- Evidence is generated by `scripts/m3d2f_firewall_evidence.py` and must report
  `external_other_endpoint_blocked = true`,
  `no_non_loopback_order_endpoint_allowed = true`, and
  `m3d2_protected_endpoint_stage_closed = true`.
- After M3d-2f, M3d-2 protected endpoint stage may be treated as closed, while
  live trading and external FINAM order calls remain blocked.

M3e-1 command consumer skeleton:

- Starts the Redis command-consumer / ACK-DLQ lifecycle track without attaching
  strategies, runtime, live readiness, or external FINAM order endpoints.
- Adds a dry-run command consumer boundary that can decode a command-stream
  `Envelope<BrokerCommand>`, validate schema/message type/payload/TTL, and then
  publish either a redacted command ACK or a redacted command DLQ record.
- Valid, non-expired commands are rejected with `DryRunOnly`; expired commands
  produce an `ExpiredCommand` ACK; malformed or wrong-message entries go to the
  command DLQ.
- XACK is modeled only after ACK or DLQ publication. Raw command payloads,
  tokens, bodies, comments, and client identifiers are not exported.
- Evidence is generated by `scripts/m3e1_command_consumer_evidence.py`.
- Still not allowed: endpoint transport invocation, non-loopback/external FINAM
  order calls, command consumer attachment to strategies, runtime/live
  attachment, `LiveReady`, stop/SLTP/brackets, or first live micro.

M3e-2 idempotency and durable command lifecycle:

- Adds a command lifecycle store boundary with a file-backed JSON implementation
  for source-bound restart/replay tests and an in-memory implementation for
  targeted unit tests.
- A typed command is persisted as `CommandReceived` before any future endpoint
  path can be considered. M3e-2 still never invokes endpoint transport.
- `request_id` idempotency is enforced before ACK publication: duplicate or
  replayed commands publish a redacted `DuplicateCommand` ACK and do not create
  a second processing or endpoint-attempt path.
- Expired commands are persisted as terminal local `ExpiredCommand` outcomes.
- Invalid command payloads go to redacted command DLQ and do not mutate command
  lifecycle state.
- ACK/DLQ publication failure blocks modeled XACK. Restart after
  `CommandReceived` or after ACK publication remains conservative and does not
  duplicate endpoint attempts.
- Evidence is generated by `scripts/m3e2_command_lifecycle_evidence.py`.
- Still not allowed: real Redis `XREADGROUP/XACK/XAUTOCLAIM`, endpoint
  transport invocation, non-loopback/external FINAM order calls, runtime/live
  attachment, `LiveReady`, stop/SLTP/brackets, replace, multi-leg, or first live
  micro.

M3e-3 command consumer to local-mock endpoint boundary:

- Connects the durable command consumer path to the existing classified
  local-mock endpoint boundary for the first endpoint-bound vertical slice.
- The endpoint path is still local/mock only: there are no non-loopback calls,
  no `api.finam.ru`, no real FINAM POST/DELETE, and no runtime/live attachment.
- Place commands persist `CommandReceived`, pass local preflight, persist
  order-path intent, then persist `BeginSubmit` before the local mock endpoint
  classified transport is called.
- Duplicate `request_id` replay after local-mock endpoint execution returns a
  redacted `DuplicateCommand` ACK and does not perform a second endpoint
  attempt.
- Local preflight rejection publishes a redacted local-reject ACK before any
  endpoint attempt.
- ACK publication is hardened with `AckPublishPlanned`: the lifecycle store is
  updated before ACK publish, and the final `AckPublished` update must succeed
  before modeled XACK. If ACK publish succeeded but final lifecycle update
  failed, restart/replay recovers explicitly without a second endpoint attempt.
- Evidence is generated by `scripts/m3e3_local_mock_endpoint_evidence.py`.
- Still not allowed: real Redis `XREADGROUP/XACK/XAUTOCLAIM`,
  non-loopback/external FINAM order calls, runtime/live attachment,
  `LiveReady`, stop/SLTP/brackets, replace, multi-leg, or first live micro.

M3e-3a cancel command to local-mock endpoint boundary:

- Extends the M3e-3 local-mock endpoint boundary from place commands to cancel
  commands.
- Cancel commands persist `CommandReceived`, load the existing broker-order
  mapping from the order-path store, run cancel preflight, then persist
  `RequestCancel` before the local mock classified cancel endpoint is called.
- Cancel accepted publishes a redacted ACK but is not treated as terminal
  canceled; it remains a submitted cancel state requiring later broker-truth
  reconciliation.
- Cancel `404/409/410` responses map to reconciliation/manual policy and never
  blind-success.
- Duplicate cancel `request_id` replay publishes a redacted `DuplicateCommand`
  ACK and performs no second endpoint attempt.
- Preflight rejection occurs before any endpoint attempt.
- Evidence is generated by `scripts/m3e3a_cancel_local_mock_endpoint_evidence.py`.
- Still not allowed: real Redis `XREADGROUP/XACK/XAUTOCLAIM`,
  non-loopback/external FINAM order calls, runtime/live attachment,
  `LiveReady`, stop/SLTP/brackets, replace, multi-leg, or first live micro.
...
- Adds a real Redis consumer-group smoke for the M3e command stream:
  `XREADGROUP -> local-mock command processing -> ACK/DLQ publish -> XACK`.
- Real `XACK` is applied only after ACK or DLQ publication succeeds.
- Injected ACK-publish and DLQ-publish failures leave source command entries
  pending; no `XACK` is performed in those failure paths.
- Real `XAUTOCLAIM` recovers pending entries after simulated consumer crashes.
- Replayed duplicate `request_id`, `CommandReceived`, endpoint-before-ACK, and
  ACK-before-XACK scenarios publish safe duplicate/recovery ACKs without a
  second endpoint attempt.
- Both `PlaceOrder` and `CancelOrder` are covered, still through the
  local-mock classified endpoint boundary only.
- Evidence is generated by `scripts/m3e4_redis_command_consumer_evidence.py`.
- Still not allowed: non-loopback/external FINAM order calls, real FINAM
  POST/DELETE, runtime/live attachment, `LiveReady`, stop/SLTP/brackets,
  replace, multi-leg, or first live micro.

M3f-1 broker-truth reconciliation contract and scheduler skeleton:

- Introduces the broker-truth reconciliation request contract without opening
  live trading.
- Models read-only inputs: `GetOrders`, `GetOrder`, `Trades`, and `Positions`.
- Schedules reconciliation requests from
  `SubmittedPendingBrokerOrderId`, `TimeoutUnknownPending`,
  `CancelSubmitted`, and `CancelTimeoutUnknownPending`.
- Ignores stable/terminal states at this scheduler layer.
- Exports only redacted identity diagnostics: presence flags and SHA-256
  values, no raw account/client/broker ids.
- Keeps all trading boundaries closed: no FINAM POST/DELETE, no non-loopback
  order endpoint, no runtime/live attachment, no `LiveReady`, no
  stop/SLTP/brackets.
- Evidence is generated by `scripts/m3f1_reconciliation_contract_evidence.py`.

M3f-2 broker-truth outcome application / recovery policy:

- Adds deterministic policy for applying broker-truth observations to M3f
  reconciliation requests, still without live/runtime mutation.
- `GetOrder` is treated as feasible only when `broker_order_id` is present.
- Missing broker id recovers through client-order correlation from read-only
  `GetOrders` / `Trades` / `Positions`.
- Identity completeness is explicit for direct broker-id lookup vs
  client-order recovery vs insufficient identity.
- Outcome policy covers recovered broker id, active order, terminal order,
  unexplained trade, stale truth, and conflicting identity.
- Request deduplication is modeled by request id plus redacted account,
  instrument, client-order, and broker-order identity.
- Evidence is generated by
  `scripts/m3f2_reconciliation_outcome_policy_evidence.py`.

M3f-3 broker-truth snapshot application / order-trade-position consistency:

- Applies simulated/read-only order, trade, and position snapshots to M3f
  reconciliation requests.
- Same identity with a different `request_id` is explicit conflict and requires
  manual intervention.
- Terminal broker order discovered through client-order recovery is one-step
  `RecoverByClientOrderIdAndTerminal`.
- Detects orphan broker orders, orphan broker trades, position mismatch, and
  stale local pending records.
- Reports remain redacted and read-only; no real FINAM POST/DELETE, no runtime
  live attachment, no `LiveReady`, no stop/SLTP/brackets.
- Evidence is generated by `scripts/m3f3_snapshot_application_evidence.py`.

M3f-4 reconciliation runner / reporting / readiness blockers:

- Loads unresolved order-path records from `OrderPathStore`, schedules M3f
  reconciliation, applies modeled broker-truth snapshots, and persists safe
  order-path transitions.
- `RecoverByClientOrderIdAndTerminal` persists recovered broker id and terminal
  state atomically in one store update.
- Converts orphan order/trade, position mismatch, same-identity/different
  request, stale pending, and manual intervention cases into readiness
  blockers.
- Emits redacted runner report with counts and hashes only.
- Keeps all trading boundaries closed: no real FINAM POST/DELETE, no runtime
  live attachment, no `LiveReady`, no stop/SLTP/brackets.
- Evidence is generated by `scripts/m3f4_reconciliation_runner_evidence.py`.

M3f-4a reconciliation runner idempotency / closure evidence:

- Proves partial-apply restart idempotency: a terminal/recovered record already
  persisted before a crash is ignored on rerun and does not oscillate.
- Keeps manual-intervention records stable and readiness-blocking across reruns.
- Adds redacted JSON report sink and blocker summary for operator diagnostics.
- Treats broker snapshots matching existing terminal/recovered local records as
  known broker truth, not orphan broker orders.
- Evidence is generated by `scripts/m3f4a_reconciliation_closure_evidence.py`.
...
- Defines required readiness inputs after M3f closure: auth, account,
  instrument registry, schedule, clean reconciliation, orders/trades stream or
  polling fallback, fresh positions snapshot, first live bar, and non-stale
  stream transport.
- Historical/read-only bars cannot satisfy first-live-bar gate.
- Even with all inputs present, this stage returns `Reconciliation` with
  `OperatorLiveArmMissing`, not `LiveReady`.
- Evidence is generated by `scripts/m3g1_readiness_contract_evidence.py`.

M3g-2 market-data stream / first-live-bar gate:

- Adds first-live-bar gate simulator requiring `LiveStream`, finality,
  timestamp freshness, current session containment, monotonic open timestamps,
  and dedupe.
- Historical/read-only/backfill bars cannot satisfy the first-live-bar gate.
- Missing/rejected first live bar hard-blocks readiness; stale live bar marks
  the stream stale.
- Even with accepted first live bar, this stage does not emit `LiveReady`.
- Evidence is generated by `scripts/m3g2_first_live_bar_gate_evidence.py`.
