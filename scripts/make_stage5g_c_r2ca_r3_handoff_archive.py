#!/usr/bin/env python3
"""Build the push-bound self-attesting R3 receipt-clock review archive."""
...
import stage5g_c_r2ca_r3_handoff_safety_check as safety

ROOT = Path(__file__).resolve().parents[1]
HANDOFF_DIR = ROOT / "reports/handoff"
CONTRACT_PATH = "docs/stage-5/stage5g-c-r2ca-r3-exact-receipt-clock-bracket-authority.json"
...
def main() -> int:
    ...
    if common.run_text(["git", "status", "--porcelain", "--untracked-files=all"]):
        raise SystemExit("refusing R3 handoff: source tree is dirty")
    branch = common.run_text(["git", "branch", "--show-current"])
    if branch != safety.BRANCH:
        raise SystemExit(f"R3 handoff requires {safety.BRANCH}, got {branch}")
    if common.run_text(["git", "rev-parse", "--show-object-format"]) != "sha1":
        raise SystemExit("R3 handoff requires SHA-1 Git objects")
    ...
    if parent_ref != safety.BASE_REF:
        raise SystemExit(f"R3 must directly follow {safety.BASE_REF}")
    ...
    if changed_paths != safety.EXPECTED_CHANGED_PATHS:
        raise SystemExit(
            f"R3 changed-path scope drift: expected {safety.EXPECTED_CHANGED_PATHS!r}, "
    ...
    with tempfile.TemporaryDirectory(prefix="stage5g-r2ca-r3-handoff-") as raw_temp:
        temp = Path(raw_temp)
        results = {
        ...
        if preseal.returncode != 0:
            ...
            raise SystemExit("R3 preseal safety check failed")
    ...
    final = subprocess.run(
        ["python3", "scripts/stage5g_c_r2ca_r3_handoff_safety_check.py", str(archive)],
        cwd=ROOT,
        check=False,
    )
    if final.returncode != 0:
        raise SystemExit("R3 final safety check failed")
