...
def main() -> int:
    ...
    cases = [
        ...
        ("remove-restart-continuation-guard", lambda r: mutate_text(r, timer, "evidence.broker_truth.received_ts.timestamp_millis() < continuation_checkpoint", "false")),
        ...
        ("reduce-restart-guard-to-last-truth", lambda r: mutate_text(r, timer, "evidence.broker_truth.received_ts.timestamp_millis() < continuation_checkpoint", "replay.last_broker_truth_received_at.is_some_and(|last| evidence.broker_truth.received_ts < last)")),
        ("remove-restart-broker-truth-regression", lambda r: mutate_text(r, timer, ".is_some_and(|last| evidence.broker_truth.received_ts < last)", ".is_some_and(|_| false)")),
        ("block-exact-replay-by-continuation", lambda r: move_before(r, timer, restart_guard, "    let identity = evidence_identity(&evidence);\n")),
