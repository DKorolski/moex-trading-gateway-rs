#!/usr/bin/env python3
"""Create an immutable Stage 8A-4 implementation R4 review handoff."""
...
import stage8a4_implementation_handoff_safety_check as safety
...
def fail(message: str) -> None:
    raise SystemExit(f"stage8a4-implementation-handoff: FAIL {message}")
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="stage8a4-implementation-handoff-") as raw_dir:
        artifact_dir = Path(raw_dir) / "gate"
        environment = os.environ.copy()
        environment["STAGE8A4_IMPLEMENTATION_ARTIFACT_DIR"] = str(artifact_dir)
        gate = subprocess.run(
            ["bash", "scripts/stage8a4_implementation_gate.sh"], cwd=ROOT, env=environment,
            stdout=subprocess.PIPE, stderr=subprocess.STDOUT, check=False,
        )
        gate_output = redacted(gate.stdout)
        marker = (
            b"stage8a4-implementation-r4-gate: PASS rows=94 negatives=58 "
            b"tests=31 pure-reducer=true execution=false"
        )
        ...
        artifact_payloads = {
            path.name: redacted(path.read_bytes())
            for path in sorted(artifact_dir.iterdir()) if path.is_file() and path.read_bytes()
        }
        required = {"stage8a4-implementation-gate-summary.json", "proof-map.stdout.txt"}
        ...
        commit_marker = (
            ...
            f"rejected_implementation_r3_ref={checker.R3}\n"
            "candidate_stage=Stage 8A-4 implementation R4\n"
            "candidate_status=implementation_r4_independent_acceptance_pending\n"
        ...
        evidence = json.dumps({
            "schema_version": 1,
            "stage": "8A-4-implementation-R4",
            "candidate_status": "implementation_r4_independent_acceptance_pending",
            "source_ref": head,
            "source_branch": branch,
            "accepted_stage8a4_design_ref": checker.BASE,
            "accepted_stage8a4_design_review_sha256": checker.REVIEW_SHA256,
            ...
            "rejected_implementation_r3_ref": checker.R3,
            "implementation_r3_review_sha256": checker.R3_REVIEW_SHA256,
            "implementation_r4_correction_spec_sha256": checker.R4_SPEC_SHA256,
            "acceptance_rows": 94,
            "negative_cases": 58,
            "focused_tests": 31,
            ...
            "next_after_independent_acceptance": "Stage 8A-4 durable composition planning only",
        }, indent=2, sort_keys=True).encode() + b"\n"
        ...
        members.extend([
            ...
            ("handoff-evidence/stage8a4-implementation-full-gate.txt", gate_output, 0o644),
            ("handoff-evidence/stage8a4-implementation-evidence.json", evidence, 0o644),
        ])
        members.extend(
