#!/usr/bin/env python3
"""Validate immutable Stage 8A-4 I2 handoff structure and evidence binding."""
...
REQUIRED = {
    "handoff-commit.txt",
    "handoff-evidence/stage8a4-durable-composition-i2-evidence.json",
    "handoff-evidence/stage8a4-durable-composition-i2-full-gate.txt",
    "handoff-evidence/source-tree-manifest.json",
    "handoff-evidence/gate-artifacts/stage8a4-durable-composition-i2-gate-summary.json",
    "docs/stage-8/stage8a4-durable-composition-i2-authority.json",
}
...
def check(archive_path: str) -> dict[str, object]:
    with zipfile.ZipFile(archive_path) as archive:
        ...
        evidence = json.loads(
            archive.read("handoff-evidence/stage8a4-durable-composition-i2-evidence.json")
        )
        ...
        gate = archive.read("handoff-evidence/stage8a4-durable-composition-i2-full-gate.txt")
...
def main() -> None:
    if len(sys.argv) != 2:
        raise SystemExit("usage: stage8a4_durable_composition_i2_handoff_safety_check.py ARCHIVE")
    try:
        result = check(sys.argv[1])
    except (ValueError, KeyError, zipfile.BadZipFile, json.JSONDecodeError) as error:
        print(f"stage8a4-durable-composition-i2-handoff-safety: FAIL {error}", file=sys.stderr)
        raise SystemExit(1)
    print("stage8a4-durable-composition-i2-handoff-safety: PASS " + json.dumps(result, sort_keys=True))
