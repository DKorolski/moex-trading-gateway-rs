...
EXPECTED_CLOSED_SURFACES = {
    ...
    "strategy_state_mutation", "autonomous_event_loop",
}
EXPECTED_INVARIANTS = {
...
def main() -> int:
    ...
    if payload.get("status") != "design_frozen_no_runtime_code":
    ...
    plan = PLAN.read_text()
    for marker in (
        "Stage 5E-b3c", "opaque, linear receipt", "instrument-scoped fresh Open-session evidence",
        "calendar/market-sequence receipt", "revalidate b3b schedule expiry", "event-key identity",
        "on_broker_bar", "intent construction", "b3b core hash unchanged",
    ):
