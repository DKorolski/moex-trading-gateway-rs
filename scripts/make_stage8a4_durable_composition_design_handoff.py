#!/usr/bin/env python3
"""Create an immutable Stage 8A-4 durable-composition design handoff."""
...
import stage8a4_durable_composition_design_handoff_safety_check as safety
...
def fail(message: str) -> None:
    raise SystemExit(f"stage8a4-durable-composition-design-handoff: FAIL {message}")
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="stage8a4-durable-design-handoff-") as raw_dir:
        artifact_dir = Path(raw_dir) / "gate"
        environment = os.environ.copy()
        environment["STAGE8A4_DURABLE_DESIGN_ARTIFACT_DIR"] = str(artifact_dir)
        gate = subprocess.run(
            ["bash", "scripts/stage8a4_durable_composition_design_gate.sh"], cwd=ROOT,
        ...
        evidence = json.dumps({
            ...
            "next_after_independent_acceptance": "durable-composition implementation specification only",
        }, indent=2, sort_keys=True).encode() + b"\n"
        ...
        members.extend([
            ...
            ("handoff-evidence/stage8a4-durable-composition-design-full-gate.txt", gate_output, 0o644),
            ("handoff-evidence/stage8a4-durable-composition-design-evidence.json", evidence, 0o644),
        ])
        members.extend(
