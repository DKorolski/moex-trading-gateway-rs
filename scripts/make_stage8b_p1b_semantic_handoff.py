#!/usr/bin/env python3
"""Create the immutable Stage 8B-P1-b semantic implementation handoff."""
...
import stage8b_p1b_semantic_handoff_safety_check as safety
...
EVIDENCE_TEMPLATE = ROOT / "docs/stage-8/stage8b-p1b-semantic-commit-evidence.json"
...
def main() -> None:
    if run("git", "status", "--porcelain", "--untracked-files=all").decode().strip():
        raise SystemExit("stage8b-p1b-semantic-handoff: FAIL dirty worktree")
    branch = run("git", "branch", "--show-current").decode().strip()
    if branch != BRANCH:
        raise SystemExit(f"stage8b-p1b-semantic-handoff: FAIL branch={branch}")
    ...
    if run("git", "merge-base", source_ref, PREDECESSOR).decode().strip() != PREDECESSOR:
        raise SystemExit("stage8b-p1b-semantic-handoff: FAIL predecessor drift")

    gate = subprocess.run(
        ["bash", "scripts/stage8b_p1b_semantic_commit_gate.sh"],
        cwd=ROOT,
        stdout=subprocess.PIPE,
        stderr=subprocess.STDOUT,
        check=False,
    )
    if gate.returncode != 0 or b"PASS stage8b-p1b-semantic-commit-gate" not in gate.stdout:
    ...
    print(
        ...
        "stage8b-p1b-semantic-handoff: PASS"
    )
