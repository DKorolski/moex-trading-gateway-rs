#!/usr/bin/env python3
"""Generate M3h-4 runtime dry emission state restore evidence."""
...
DOC = Path("docs/m3h4-runtime-dry-emission-state-restore.md")
...
CHECKS = [
    ...
    ["python3", "-m", "py_compile", "scripts/m3h4_runtime_dry_emission_state_restore_evidence.py"],
]
...
def evidence_summary(root: Path) -> dict[str, Any]:
    ...
    required_patterns = [
        ...
        "PublishFailed",
        "record_not_emitted",
        "m3h4_runtime_dry_emission_restore_after_published_does_not_republish",
        "m3h4_runtime_dry_emission_restore_after_pending_is_conservative_no_republish",
        "m3h4_runtime_dry_emission_not_emitted_rollback_policy_is_durable",
    ...
    return {
        ...
        "durable_lifecycle_store_present": (
            "M3hRuntimeDryCommandLifecycleStore" in source
            and "M3hJsonRuntimeDryCommandLifecycleStore" in source
        ),
        "published_restart_no_republish": (
            "m3h4_runtime_dry_emission_restore_after_published_does_not_republish"
            in source
        ),
        "pending_restart_conservative": (
            "m3h4_runtime_dry_emission_restore_after_pending_is_conservative_no_republish"
            in source
        ),
        "not_emitted_rollback_durable": (
            "m3h4_runtime_dry_emission_not_emitted_rollback_policy_is_durable"
            in source
            and "record_not_emitted" in source
        ),
        ...
        "closure_documented": "M3h-4 hardens the M3h-3 dry command emitter"
        in doc,
        "real_finam_order_endpoint_used": False,
        "external_order_endpoint_allowed": False,
        "runtime_live_attachment_allowed": False,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate M3h-4 runtime dry emission state restore evidence."
    )
    parser.add_argument(
        "--source-archive",
        type=Path,
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("reports/m3h-runtime-shadow/m3h4-runtime-dry-emission-state-restore-evidence.json"),
    )
    ...
    policy_ok = (
        ...
        and summary["durable_lifecycle_store_present"]
        and summary["published_restart_no_republish"]
        and summary["pending_restart_conservative"]
        and summary["not_emitted_rollback_durable"]
    ...
    evidence = {
        ...
        "durable_lifecycle_store_present": summary["durable_lifecycle_store_present"],
        "published_restart_no_republish": summary["published_restart_no_republish"],
        "pending_restart_conservative": summary["pending_restart_conservative"],
        "not_emitted_rollback_durable": summary["not_emitted_rollback_durable"],
