#!/usr/bin/env python3
"""Generate M3j-10 operator GO + readonly refresh evidence."""
...
DOC = Path("docs/m3j10-operator-go-readonly-refresh-package.md")

CHECKS = [
    ["cargo", "fmt", "--all", "--check"],
    ["cargo", "test", "-p", "finam-gateway", "m3j10", "--", "--nocapture"],
    ["cargo", "test", "--all"],
    ["cargo", "clippy", "--workspace", "--all-targets", "--", "-D", "warnings"],
    ["bash", "scripts/forbidden_surface_scan.sh"],
    ["bash", "scripts/forbidden_surface_negative_harness.sh"],
    ["bash", "scripts/order_endpoint_scanner_transition_spec.sh"],
    ["python3", "-m", "py_compile", "scripts/m3j10_operator_go_readonly_refresh_evidence.py"],
]
...
def evidence_summary(root: Path) -> dict[str, Any]:
    ...
    required_patterns = [
        ...
        "m3j10_operator_go_readonly_refresh_ready_but_live_micro_still_blocked",
        "m3j10_stale_readonly_active_orders_expired_arm_or_live_input_blocks_refresh",
    ]
    forbidden_patterns = [
        ".post(",
        ".delete(",
    ...
    return {
        ...
        "operator_go_readonly_refresh_only": "operator_go_readonly_refresh_only: true" in source,
        "refresh_package_ready_model_present": "refresh_package_ready" in source,
        "live_micro_go": False,
        "no_live_boundary": (
        ...
        "unsafe_live_input_diagnostic_present": "unsafe_live_input_detected" in source,
        "m3j10_documented": "M3j-10 is a pre-live package" in doc,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate M3j-10 operator GO + readonly refresh evidence."
    )
    parser.add_argument("--source-archive", type=Path)
    parser.add_argument("--m3j9-evidence", type=Path)
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("reports/m3j-pre-live/m3j10-operator-go-readonly-refresh-evidence.json"),
    )
    ...
    m3j9_evidence = artifact_summary(
        (root / args.m3j9_evidence).resolve() if args.m3j9_evidence else None
    )
    ...
    artifact_manifest_ok = m3j9_evidence["provided"]
    policy_ok = (
        all(summary["required_patterns_present"].values())
        and all(summary["forbidden_patterns_absent"].values())
        and summary["operator_go_readonly_refresh_only"]
        and summary["refresh_package_ready_model_present"]
        and not summary["live_micro_go"]
        and summary["no_live_boundary"]
        and summary["unsafe_live_input_diagnostic_present"]
        and summary["m3j10_documented"]
    )
    ...
    evidence = {
        ...
        "summary": summary,
        "artifact_manifest": {"m3j9_evidence": m3j9_evidence},
        "operator_go_readonly_refresh_only": True,
        "live_micro_go": False,
