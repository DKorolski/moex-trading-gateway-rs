...
def main() -> int:
    ...
    def mutate_stage5e_b3f_source_and_full_rebind_checker(path_value, mutator):
        def apply(root, _manifest, _marker):
            ...
            stage5c_region = module.marked_region(
                stage5c_source,
                "STAGE5E-B3F-SETTLEMENT-IMPLEMENTATION",
                "stage5c-private-bridge-v1",
            )
            stage5e_region = module.marked_region(
                stage5e_source,
                "STAGE5E-B3F-SETTLEMENT-IMPLEMENTATION",
                "private-process-local-v1",
            )
            stage5e_callback_tokens = module.balanced_item_body(
                module.rust_tokens(stage5e_region),
                ["pub", "(", "crate", ")", "mod", "callback_settlement"],
            )
            actual_signature_contracts = (
                (
                    module.EXPECTED_STAGE5C_REGION_PRODUCTION_SIGNATURES,
                    module.production_signature_contract(
                        module.top_level_items(module.rust_tokens(stage5c_region)),
                        "Stage 5C B3F region",
                    ),
                ),
                (
                    module.EXPECTED_STAGE5E_CALLBACK_PRODUCTION_SIGNATURES,
                    module.production_signature_contract(
                        module.top_level_items(stage5e_callback_tokens),
                        "Stage 5E callback_settlement",
                    ),
                ),
            )
            for expected, actual in actual_signature_contracts:
                for key, old_value in expected.items():
                    ...
                    if checker.count(old_value) != 1:
                        raise AssertionError(
                            f"B3F item-signature pin not unique: {key}"
                        )
                    checker = checker.replace(old_value, new_value, 1)
    ...
    def insert_stage5e_b3f_outer_item(source, item):
        marker = (
            "    // STAGE5E-B3F-SETTLEMENT-IMPLEMENTATION-BEGIN: "
            "private-process-local-v1\n"
        )
        anchor = marker + "    impl Stage5ePaperCallbackResultEscrow {"
        if source.count(anchor) != 1:
            raise AssertionError("B3F outer-region insertion anchor not unique")
    ...
    def mutate_stage5e_b3f_ui_harness_and_rehash_checker(mutator):
        def apply(root, _manifest, _marker):
            inventory_path = (
                root
                / "docs/stage-5/stage5e-b3f-callback-settlement-escrow-design-inventory.json"
            ...
            payload = json.loads(inventory_path.read_text())
            ui_path = root / "scripts/stage5e_b3f_production_ui_harness.py"
            ui_path.write_text(mutator(ui_path.read_text()))
            payload["production_ui_harness_contract"]["sha256"] = hashlib.sha256(
                ui_path.read_bytes()
            ).hexdigest()
            inventory_path.write_text(
                json.dumps(payload, indent=2, sort_keys=True) + "\n"
    ...
    cases = [
        ...
        Case("stage5e-b3f-r2-material-raw-constructor-opened", "design inventory drift", mutate_stage5e_b3f_inventory_for_checker(lambda p: p["stage5c_material_construction_contract"].__setitem__("constructor_capability", "none")), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r2-material-fields-made-public", "design inventory drift", mutate_stage5e_b3f_inventory_for_checker(lambda p: p["stage5c_material_construction_contract"].__setitem__("material_visibility", "pub_crate_public_fields")), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r2-second-material-constructor", "design inventory drift", mutate_stage5e_b3f_inventory_for_checker(lambda p: p["stage5c_material_construction_contract"].__setitem__("constructor_definition_count", 2)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r2-second-material-seal-issuer", "design inventory drift", mutate_stage5e_b3f_inventory_for_checker(lambda p: p["stage5c_material_construction_contract"].__setitem__("seal_issuer_call_site_count", 2)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r2-success-raw-getter-opened", "design inventory drift", mutate_stage5e_b3f_inventory_for_checker(lambda p: p["stage5c_success_return_contract"].__setitem__("raw_getters_allowed", True)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r2-success-proof-field-omitted", "design inventory drift", mutate_stage5e_b3f_inventory_for_checker(lambda p: p["stage5c_success_return_contract"]["proof_fields"].remove("batch_state_fingerprint")), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r2-second-success-transfer", "design inventory drift", mutate_stage5e_b3f_inventory_for_checker(lambda p: p["stage5c_success_return_contract"].__setitem__("transfer_method_call_site_count", 2)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r2-terminal-into-parts-opened", "design inventory drift", mutate_stage5e_b3f_inventory_for_checker(lambda p: p["stage5c_terminal_return_contract"].__setitem__("generic_parts_allowed", True)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r2-second-terminal-transfer", "design inventory drift", mutate_stage5e_b3f_inventory_for_checker(lambda p: p["stage5c_terminal_return_contract"].__setitem__("transfer_method_call_site_count", 2)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r2-settled-strategy-not-moved", "design inventory drift", mutate_stage5e_b3f_inventory_for_checker(lambda p: p["stage5c_success_return_contract"].__setitem__("settled_move_count", 0)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r2-named-mapper-wildcard-opened", "design inventory drift", mutate_stage5e_b3f_inventory_for_checker(lambda p: p["named_authority_functions"]["stage5c_error_mapper"].__setitem__("wildcard_arm_allowed", True)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r2-second-audit-builder", "design inventory drift", mutate_stage5e_b3f_inventory_for_checker(lambda p: p["named_authority_functions"]["audit_commitment_builder"].__setitem__("call_site_count", 2)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r2-second-settlement-identity-builder", "design inventory drift", mutate_stage5e_b3f_inventory_for_checker(lambda p: p["named_authority_functions"]["settlement_identity_builder"].__setitem__("definition_count", 2)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r2-preflight-ok-outcome-dropped", "design inventory drift", mutate_stage5e_b3f_inventory_for_checker(lambda p: p["terminal_ownership_matrix"]["preflight_ok_terminal"]["fields"].remove("opaque_exact_ok_callback_outcome_with_intent_vector")), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r2-callback-error-dropped", "design inventory drift", mutate_stage5e_b3f_inventory_for_checker(lambda p: p["terminal_ownership_matrix"]["callback_validation_error_terminal"]["fields"].remove("opaque_exact_callback_validation_error")), "5E-b3f-callback-settlement-escrow-design", True),
        ...
        Case("stage5e-b3f-r4-expected-binding-owner-reversed", "design inventory drift", mutate_stage5e_b3f_inventory_for_checker(lambda p: p["stage5c_preflight_bridge_contract"].__setitem__("expected_binding_owner", "callback_authority::callback_settlement")), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r4-expected-binding-constructor-owner-reversed", "design inventory drift", mutate_stage5e_b3f_inventory_for_checker(lambda p: p["stage5c_preflight_bridge_contract"].__setitem__("expected_binding_constructor_owner", "callback_authority::callback_settlement")), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r4-expected-binding-fields-crate-public", "design inventory drift", mutate_stage5e_b3f_inventory_for_checker(lambda p: p["stage5c_preflight_bridge_contract"].__setitem__("expected_binding_visibility", "pub_crate_public_fields")), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r4-expected-binding-lifetime-removed", "design inventory drift", mutate_stage5e_b3f_inventory_for_checker(lambda p: p["stage5c_preflight_bridge_contract"].__setitem__("expected_binding_lifetime_bound", False)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r4-expected-binding-owned-scalar-copy", "design inventory drift", mutate_stage5e_b3f_inventory_for_checker(lambda p: p["stage5c_preflight_bridge_contract"].__setitem__("expected_binding_constructor_input_mode", "owned_scalar_copy")), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r4-second-expected-binding-constructor-definition", "design inventory drift", mutate_stage5e_b3f_inventory_for_checker(lambda p: p["stage5c_preflight_bridge_contract"].__setitem__("expected_binding_constructor_definition_count", 2)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r4-expected-binding-inspected-outside-stage5c", "design inventory drift", mutate_stage5e_b3f_inventory_for_checker(lambda p: p["stage5c_preflight_bridge_contract"].__setitem__("expected_binding_fields_inspected_only_by_stage5c_owner", False)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r4-expected-binding-borrow-escapes-consume", "design inventory drift", mutate_stage5e_b3f_inventory_for_checker(lambda p: p["stage5c_preflight_bridge_contract"].__setitem__("expected_binding_borrows_end_before_escrow_consume", False)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r4-event-key-authority-renamed", "design inventory drift", mutate_stage5e_b3f_inventory_for_checker(lambda p: p["b3b_event_key_validation_authority_contract"].__setitem__("function", "validate_event_key_ad_hoc")), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r4-event-key-authority-owner-moved-stage5c", "design inventory drift", mutate_stage5e_b3f_inventory_for_checker(lambda p: p["b3b_event_key_validation_authority_contract"].__setitem__("owner", "strategy_runtime_core::stage5c_paper_host")), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r4-second-event-key-authority-definition", "design inventory drift", mutate_stage5e_b3f_inventory_for_checker(lambda p: p["b3b_event_key_validation_authority_contract"].__setitem__("definition_count", 2)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r4-second-event-key-authority-call", "design inventory drift", mutate_stage5e_b3f_inventory_for_checker(lambda p: p["b3b_event_key_validation_authority_contract"].__setitem__("call_site_count", 2)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r4-canonical-event-key-delegate-replaced", "design inventory drift", mutate_stage5e_b3f_inventory_for_checker(lambda p: p["b3b_event_key_validation_authority_contract"].__setitem__("canonical_delegate", "stage5c_event_key_encoder")), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r4-second-event-key-encoder-opened", "design inventory drift", mutate_stage5e_b3f_inventory_for_checker(lambda p: p["b3b_event_key_validation_authority_contract"].__setitem__("second_encoder_allowed", True)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r4-second-event-key-domain-opened", "design inventory drift", mutate_stage5e_b3f_inventory_for_checker(lambda p: p["b3b_event_key_validation_authority_contract"].__setitem__("second_domain_string_allowed", True)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r4-event-key-proof-reusable", "design inventory drift", mutate_stage5e_b3f_inventory_for_checker(lambda p: p["b3b_event_key_validation_authority_contract"].__setitem__("proof_reusable_authority", True)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r4-preflight-mismatch-made-opaque", "design inventory drift", mutate_stage5e_b3f_inventory_for_checker(lambda p: p["stage5c_preflight_mismatch_contract"].__setitem__("representation", "opaque_struct")), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r4-preflight-mismatch-variant-removed", "design inventory drift", mutate_stage5e_b3f_inventory_for_checker(lambda p: p["stage5c_preflight_mismatch_contract"]["variants"].remove("AuditEventKey")), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r4-preflight-mismatch-wildcard-opened", "design inventory drift", mutate_stage5e_b3f_inventory_for_checker(lambda p: p["stage5c_preflight_mismatch_contract"].__setitem__("wildcard_arm_allowed", True)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r4-preflight-mismatch-generic-conversion", "design inventory drift", mutate_stage5e_b3f_inventory_for_checker(lambda p: p["stage5c_preflight_mismatch_contract"].__setitem__("generic_conversion_allowed", True)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r4-second-preflight-mismatch-mapper-call", "design inventory drift", mutate_stage5e_b3f_inventory_for_checker(lambda p: p["stage5c_preflight_mismatch_contract"].__setitem__("mapper_call_site_count", 2)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r4-preflight-mismatch-mapping-weakened", "design inventory drift", mutate_stage5e_b3f_inventory_for_checker(lambda p: p["stage5c_preflight_mismatch_contract"]["mapping"].__setitem__("AuditEventKey", "PaperModeMismatch")), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r4-named-preflight-mapper-wildcard", "design inventory drift", mutate_stage5e_b3f_inventory_for_checker(lambda p: p["named_authority_functions"]["stage5c_preflight_mismatch_mapper"].__setitem__("wildcard_arm_allowed", True)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r4-event-key-raw-audit-getters-opened", "design inventory drift", mutate_stage5e_b3f_inventory_for_checker(lambda p: p["b3b_event_key_validation_authority_contract"].__setitem__("raw_audit_lineage_getters_allowed", True)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r4-callback-settlement-module-unreachable", "design inventory drift", mutate_stage5e_b3f_inventory_for_checker(lambda p: p["module_visibility_contract"].__setitem__("callback_settlement_module", "private_unreexported_child")), "5E-b3f-callback-settlement-escrow-design", True),
        ...
        Case("stage5e-b3f-rehash-retained-close-issued-check-removed", "Stage 5C retained-close authority missing", mutate_stage5e_b3f_source_and_rehash_checker("crates/strategy-runtime-core/src/stage5c_paper_host.rs", lambda s: s.replace("accepted_bar_close > authority_issued_at", "false", 1)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-rehash-retained-close-callback-check-removed", "Stage 5C retained-close authority missing", mutate_stage5e_b3f_source_and_rehash_checker("crates/strategy-runtime-core/src/stage5c_paper_host.rs", lambda s: s.replace("accepted_bar_close > callback_invoked_at", "false", 1)), "5E-b3f-callback-settlement-escrow-design", True),
        ...
        Case("stage5e-b3f-rehash-compile-fail-fixture-removed", "required B3F compile-fail fixture missing", mutate_stage5e_b3f_source_and_rehash_checker("crates/strategy-runtime-core/src/lib.rs", lambda s: s.replace("b3f_compile_fail_borrow_survives_consume", "removed_b3f_compile_fail_fixture", 1)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r5-rehash-raw-retained-close-getter-opened", "retained metadata impl surface drift", mutate_stage5e_b3f_source_and_rehash_checker("crates/strategy-runtime-core/src/stage5c_paper_host.rs", lambda s: s.replace("#[cfg(test)]\nimpl Stage5ePreCallbackAttributionSnapshot", "impl Stage5eAcceptedBarSettlementMetadata {\n    pub(crate) fn accepted_bar_close_ts(&self) -> i64 { self.accepted_bar_close_ts }\n}\n\n#[cfg(test)]\nimpl Stage5ePreCallbackAttributionSnapshot", 1)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r5-rehash-scalar-chronology-extraction-opened", "settlement chronology regressed to raw scalar extraction", mutate_stage5e_b3f_source_and_rehash_checker("crates/strategy-runtime-core/src/stage5e_no_io_lifecycle.rs", lambda s: s.replace("let expected = construct_stage5c_expected_preflight_binding(escrow, seal);", "let _raw = escrow.retained_bar_metadata.accepted_bar_close_ts();\\n            let expected = construct_stage5c_expected_preflight_binding(escrow, seal);", 1)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r5-rehash-compile-fail-unconditional", "B3F compile-fail evidence may not use unconditional compile_error", mutate_stage5e_b3f_source_and_rehash_checker("crates/strategy-runtime-core/src/lib.rs", lambda s: s.replace("// b3f_compile_fail_consume_seal_clone_or_copy", "// b3f_compile_fail_consume_seal_clone_or_copy\\n//! compile_error!(\\\"fake witness\\\");", 1)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r6-rehash-alternative-retained-close-getter", "retained metadata impl surface drift", mutate_stage5e_b3f_source_and_rehash_checker("crates/strategy-runtime-core/src/stage5c_paper_host.rs", lambda s: s.replace("#[cfg(test)]\nimpl Stage5eAcceptedBarSettlementMetadata", "impl Stage5eAcceptedBarSettlementMetadata {\n    pub(crate) fn retained_close_scalar(&self) -> i64 { self.accepted_bar_close_ts }\n}\n\n#[cfg(test)]\nimpl Stage5eAcceptedBarSettlementMetadata", 1)), "5E-b3f-callback-settlement-escrow-design", True),
        ...
        Case("stage5e-b3f-r7-rehash-consume-seal-from-unit", "settlement seal impl surface opened", mutate_stage5e_b3f_source_and_rehash_checker("crates/strategy-runtime-core/src/stage5e_no_io_lifecycle.rs", lambda s: s.replace("pub(crate) struct Stage5ePaperSettlementConsumeSeal(());", "pub(crate) struct Stage5ePaperSettlementConsumeSeal(());\n        impl From<()> for Stage5ePaperSettlementConsumeSeal {\n            fn from(_: ()) -> Self { Self(()) }\n        }", 1)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r7-rehash-consume-seal-default", "settlement seal impl surface opened", mutate_stage5e_b3f_source_and_rehash_checker("crates/strategy-runtime-core/src/stage5e_no_io_lifecycle.rs", lambda s: s.replace("pub(crate) struct Stage5ePaperSettlementConsumeSeal(());", "pub(crate) struct Stage5ePaperSettlementConsumeSeal(());\n        impl Default for Stage5ePaperSettlementConsumeSeal {\n            fn default() -> Self { Self(()) }\n        }", 1)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r7-rehash-aliased-payload-capability-trait", "Stage 5E protected-type alias opened", mutate_stage5e_b3f_source_and_rehash_checker("crates/strategy-runtime-core/src/stage5e_no_io_lifecycle.rs", lambda s: s.replace("pub(super) struct Stage5ePaperSettlementPayload {", "type ConsumeCapabilityAlias = Stage5ePaperSettlementConsumeSeal;\n        pub(super) struct Stage5ePaperSettlementPayload {\n            _consume_capability: ConsumeCapabilityAlias,", 1).replace("enum Stage5ePaperSettlementPreflightDecision", "trait PayloadCapabilityAccess { fn capability(&self) -> &Stage5ePaperSettlementConsumeSeal; }\n        impl PayloadCapabilityAccess for Stage5ePaperSettlementPayload {\n            fn capability(&self) -> &Stage5ePaperSettlementConsumeSeal { &self._consume_capability }\n        }\n        enum Stage5ePaperSettlementPreflightDecision", 1)), "5E-b3f-callback-settlement-escrow-design", True),
        ...
        Case("stage5e-b3f-r7-rehash-macro-generated-metadata-bridge", "Stage 5C protected macro expansion surface opened", mutate_stage5e_b3f_source_and_rehash_checker("crates/strategy-runtime-core/src/stage5c_paper_host.rs", lambda s: s.replace("#[cfg(test)]\nimpl Stage5eAcceptedBarSettlementMetadata", "macro_rules! export_retained_close {\n    () => { fn leaked(metadata: &Stage5eAcceptedBarSettlementMetadata) -> i64 { metadata.accepted_bar_close_ts } };\n}\nexport_retained_close!();\n\n#[cfg(test)]\nimpl Stage5eAcceptedBarSettlementMetadata", 1)), "5E-b3f-callback-settlement-escrow-design", True),
        ...
        Case("stage5e-b3f-r9-full-rebind-unrestricted-preflight-issuer", "Stage 5E callback_settlement exact production item/signature vector drift", mutate_stage5e_b3f_source_and_full_rebind_checker("crates/strategy-runtime-core/src/stage5e_no_io_lifecycle.rs", lambda s: s.replace("        #[allow(clippy::result_large_err)]\n        pub(crate) fn validate_and_settle_stage5e_paper_callback_escrow(", "        pub(crate) fn issue_preflight_seal_unrestricted() -> Stage5ePaperSettlementPreflightSeal { Stage5ePaperSettlementPreflightSeal(()) }\n\n        #[allow(clippy::result_large_err)]\n        pub(crate) fn validate_and_settle_stage5e_paper_callback_escrow(", 1).replace("let preflight_seal = Stage5ePaperSettlementPreflightSeal(());", "let preflight_seal = issue_preflight_seal_unrestricted();", 1)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r9-full-rebind-unrestricted-success-issuer", "Stage 5E callback_settlement exact production item/signature vector drift", mutate_stage5e_b3f_source_and_full_rebind_checker("crates/strategy-runtime-core/src/stage5e_no_io_lifecycle.rs", lambda s: s.replace("        #[allow(clippy::result_large_err)]\n        pub(crate) fn validate_and_settle_stage5e_paper_callback_escrow(", "        pub(crate) fn issue_success_seal_unrestricted() -> Stage5ePaperSettlementSuccessSeal { Stage5ePaperSettlementSuccessSeal(()) }\n\n        #[allow(clippy::result_large_err)]\n        pub(crate) fn validate_and_settle_stage5e_paper_callback_escrow(", 1).replace("Stage5ePaperSettlementSuccessSeal(()),", "issue_success_seal_unrestricted(),", 1)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r9-full-rebind-unrestricted-terminal-issuer", "Stage 5E callback_settlement exact production item/signature vector drift", mutate_stage5e_b3f_source_and_full_rebind_checker("crates/strategy-runtime-core/src/stage5e_no_io_lifecycle.rs", lambda s: s.replace("        #[allow(clippy::result_large_err)]\n        pub(crate) fn validate_and_settle_stage5e_paper_callback_escrow(", "        pub(crate) fn issue_terminal_seal_unrestricted() -> Stage5ePaperSettlementTerminalSeal { Stage5ePaperSettlementTerminalSeal(()) }\n\n        #[allow(clippy::result_large_err)]\n        pub(crate) fn validate_and_settle_stage5e_paper_callback_escrow(", 1).replace("Stage5ePaperSettlementTerminalSeal(()),", "issue_terminal_seal_unrestricted(),", 1)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r9-full-rebind-const-seal-capability", "Stage 5E callback_settlement exact production item/signature vector drift", mutate_stage5e_b3f_source_and_full_rebind_checker("crates/strategy-runtime-core/src/stage5e_no_io_lifecycle.rs", lambda s: s.replace("        #[allow(clippy::result_large_err)]", "        pub(crate) const FORGED_PREFLIGHT: Stage5ePaperSettlementPreflightSeal = Stage5ePaperSettlementPreflightSeal(());\n\n        #[allow(clippy::result_large_err)]", 1)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r9-full-rebind-long-seal-alias-from", "Stage 5E protected-type alias opened", mutate_stage5e_b3f_source_and_full_rebind_checker("crates/strategy-runtime-core/src/stage5e_no_io_lifecycle.rs", lambda s: s.replace("pub(crate) struct Stage5ePaperSettlementConsumeSeal(());", "pub(crate) struct Stage5ePaperSettlementConsumeSeal(());\n        #[allow(unused_parens)]\n        type ConsumeAlias = ((((((((((((((((((((((((((((((Stage5ePaperSettlementConsumeSeal))))))))))))))))))))))))))))));\n        impl From<u8> for ConsumeAlias { fn from(_: u8) -> Self { Self(()) } }", 1)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r9-full-rebind-long-metadata-alias-scalar", "Stage 5C protected-type alias opened", mutate_stage5e_b3f_source_and_full_rebind_checker("crates/strategy-runtime-core/src/stage5c_paper_host.rs", lambda s: s.replace("// STAGE5E-B3F-SETTLEMENT-IMPLEMENTATION-BEGIN: stage5c-private-bridge-v1", "#[allow(unused_parens)]\ntype BarMeta = ((((((((((((((((((((((((((((((Stage5eAcceptedBarSettlementMetadata))))))))))))))))))))))))))))));\nfn leak_close(metadata: &BarMeta) -> i64 { metadata.accepted_bar_close_ts }\n\n// STAGE5E-B3F-SETTLEMENT-IMPLEMENTATION-BEGIN: stage5c-private-bridge-v1", 1)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r9-full-rebind-parameterless-seal-macro", "Stage 5E protected macro expansion surface opened", mutate_stage5e_b3f_source_and_full_rebind_checker("crates/strategy-runtime-core/src/stage5e_no_io_lifecycle.rs", lambda s: s.replace("        #[allow(clippy::result_large_err)]", "        macro_rules! forge_success { () => { pub(crate) fn macro_issue_success() -> Stage5ePaperSettlementSuccessSeal { Stage5ePaperSettlementSuccessSeal(()) } }; }\n        forge_success!();\n\n        #[allow(clippy::result_large_err)]", 1)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r9-full-rebind-parameterless-metadata-macro", "Stage 5C protected macro expansion surface opened", mutate_stage5e_b3f_source_and_full_rebind_checker("crates/strategy-runtime-core/src/stage5c_paper_host.rs", lambda s: s.replace("// STAGE5E-B3F-SETTLEMENT-IMPLEMENTATION-BEGIN: stage5c-private-bridge-v1", "macro_rules! leak_metadata { () => { fn macro_leak(metadata: &Stage5eAcceptedBarSettlementMetadata) -> i64 { metadata.accepted_bar_close_ts } }; }\nleak_metadata!();\n\n// STAGE5E-B3F-SETTLEMENT-IMPLEMENTATION-BEGIN: stage5c-private-bridge-v1", 1)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r9-full-rebind-alias-chain-impl-self", "Stage 5E protected-type alias opened", mutate_stage5e_b3f_source_and_full_rebind_checker("crates/strategy-runtime-core/src/stage5e_no_io_lifecycle.rs", lambda s: s.replace("pub(crate) struct Stage5ePaperSettlementConsumeSeal(());", "pub(crate) struct Stage5ePaperSettlementConsumeSeal(());\n        type ConsumeAliasOne = Stage5ePaperSettlementConsumeSeal;\n        type ConsumeAliasTwo = ConsumeAliasOne;\n        impl From<u8> for ConsumeAliasTwo { fn from(_: u8) -> Self { Self(()) } }", 1)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r9-full-rebind-alias-chain-return-type", "Stage 5E protected-type alias opened", mutate_stage5e_b3f_source_and_full_rebind_checker("crates/strategy-runtime-core/src/stage5e_no_io_lifecycle.rs", lambda s: s.replace("pub(crate) struct Stage5ePaperSettlementSuccessSeal(());", "pub(crate) struct Stage5ePaperSettlementSuccessSeal(());\n        type SuccessAliasOne = Stage5ePaperSettlementSuccessSeal;\n        type SuccessAliasTwo = SuccessAliasOne;\n        pub(crate) fn alias_issue_success() -> SuccessAliasTwo { Stage5ePaperSettlementSuccessSeal(()) }", 1)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r9-full-rebind-payload-alias-bridge-input", "Stage 5E protected-type alias opened", mutate_stage5e_b3f_source_and_full_rebind_checker("crates/strategy-runtime-core/src/stage5e_no_io_lifecycle.rs", lambda s: s.replace("pub(super) struct Stage5ePaperSettlementPayload {", "type PayloadAlias = Stage5ePaperSettlementPayload;\n        fn inspect_payload_alias(_payload: &PayloadAlias) {}\n\n        pub(super) struct Stage5ePaperSettlementPayload {", 1)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r10-full-rebind-cfg-not-test-unrestricted-issuer", "Stage 5E callback_settlement ambiguous or production cfg attribute opened", mutate_stage5e_b3f_source_and_full_rebind_checker("crates/strategy-runtime-core/src/stage5e_no_io_lifecycle.rs", lambda s: s.replace("        #[allow(clippy::result_large_err)]", "        #[cfg(not(test))]\n        pub(crate) fn production_only_unrestricted_preflight() -> Stage5ePaperSettlementPreflightSeal { Stage5ePaperSettlementPreflightSeal(()) }\n\n        #[allow(clippy::result_large_err)]", 1)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r10-full-rebind-consume-capability-removed", "Stage 5E outer impl semantic signature vector drift", mutate_stage5e_b3f_source_and_full_rebind_checker("crates/strategy-runtime-core/src/stage5e_no_io_lifecycle.rs", lambda s: s.replace("        fn consume_for_settlement(\n            self,\n            seal: &callback_settlement::Stage5ePaperSettlementConsumeSeal,\n        ) -> callback_settlement::Stage5ePaperSettlementPayload {\n            callback_settlement::Stage5ePaperSettlementPayload::from_escrow(self, seal)\n        }", "        fn consume_for_settlement(\n            self,\n        ) -> callback_settlement::Stage5ePaperSettlementPayload {\n            callback_settlement::Stage5ePaperSettlementPayload::from_escrow(self)\n        }", 1).replace("            pub(super) fn from_escrow(\n                escrow: Stage5ePaperCallbackResultEscrow,\n                _seal: &Stage5ePaperSettlementConsumeSeal,\n            ) -> Self {", "            pub(super) fn from_escrow(\n                escrow: Stage5ePaperCallbackResultEscrow,\n            ) -> Self {", 1)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r10-full-rebind-stage5c-issuer-accepts-unit", "Stage 5C B3F region critical function capability signature drift: issue_stage5c_b3f_settlement_material_seal", mutate_stage5e_b3f_source_and_full_rebind_checker("crates/strategy-runtime-core/src/stage5c_paper_host.rs", lambda s: s.replace("_consume_capability: &crate::stage5e_no_io_lifecycle::callback_authority::callback_settlement::Stage5ePaperSettlementConsumeSeal,", "_unrestricted: &(),", 2)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r10-full-rebind-constructor-function-item", "Stage 5E settlement constructor control-flow surface opened: fn", mutate_stage5e_b3f_source_and_full_rebind_checker("crates/strategy-runtime-core/src/stage5e_no_io_lifecycle.rs", lambda s: s.replace("            let preflight_seal = Stage5ePaperSettlementPreflightSeal(());", "            let constructor: fn(()) -> Stage5ePaperSettlementPreflightSeal = Stage5ePaperSettlementPreflightSeal;\n            let preflight_seal = constructor(());", 1)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r10-full-rebind-constructor-closure", "Stage 5E settlement constructor control-flow surface opened: ||", mutate_stage5e_b3f_source_and_full_rebind_checker("crates/strategy-runtime-core/src/stage5e_no_io_lifecycle.rs", lambda s: s.replace("            let preflight_seal = Stage5ePaperSettlementPreflightSeal(());", "            let issue = || Stage5ePaperSettlementPreflightSeal(());\n            let preflight_seal = issue();", 1)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r10-full-rebind-constructor-local-function", "Stage 5E settlement constructor control-flow surface opened: fn", mutate_stage5e_b3f_source_and_full_rebind_checker("crates/strategy-runtime-core/src/stage5e_no_io_lifecycle.rs", lambda s: s.replace("            let preflight_seal = Stage5ePaperSettlementPreflightSeal(());", "            fn issue_local() -> Stage5ePaperSettlementPreflightSeal { Stage5ePaperSettlementPreflightSeal(()) }\n            let preflight_seal = issue_local();", 1)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r10-full-rebind-preflight-raw-associated-bridge", "Stage 5E callback_settlement impl semantic signature vector drift", mutate_stage5e_b3f_source_and_full_rebind_checker("crates/strategy-runtime-core/src/stage5e_no_io_lifecycle.rs", lambda s: s.replace("            pub(super) fn from_escrow(\n                escrow: &'a Stage5ePaperCallbackResultEscrow,", "            pub(crate) fn raw_callback_authority_id(&self) -> [u8; 32] { self.escrow.callback_authority_id }\n\n            pub(super) fn from_escrow(\n                escrow: &'a Stage5ePaperCallbackResultEscrow,", 1)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r10-full-rebind-unsafe-seal-construction", "Stage 5E settlement constructor control-flow surface opened: transmute", mutate_stage5e_b3f_source_and_full_rebind_checker("crates/strategy-runtime-core/src/stage5e_no_io_lifecycle.rs", lambda s: s.replace("            let preflight_seal = Stage5ePaperSettlementPreflightSeal(());", "            let preflight_seal: Stage5ePaperSettlementPreflightSeal = unsafe { core::mem::transmute(()) };", 1)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r10-full-rebind-runtime-core-unsafe", "strategy-runtime-core unsafe token opened: crates/strategy-runtime-core/src/lib.rs", mutate_stage5e_b3f_source_and_full_rebind_checker("crates/strategy-runtime-core/src/lib.rs", lambda s: s + "\n#[allow(dead_code)]\nfn b3f_forbidden_raw_memory() { unsafe { core::hint::unreachable_unchecked() } }\n"), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r11-full-rebind-outer-unrestricted-escrow-consumer", "Stage 5E B3F outer exact item/signature vector drift", mutate_stage5e_b3f_source_and_full_rebind_checker("crates/strategy-runtime-core/src/stage5e_no_io_lifecycle.rs", lambda s: insert_stage5e_b3f_outer_item(s, "    pub(crate) fn b3f_unrestricted_take_callback_outcome(\n        escrow: Stage5ePaperCallbackResultEscrow,\n    ) -> Stage5ePaperCallbackOutcome {\n        escrow.callback_outcome\n    }")), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r11-full-rebind-outer-nested-raw-bridge", "Stage 5E B3F outer exact item/signature vector drift", mutate_stage5e_b3f_source_and_full_rebind_checker("crates/strategy-runtime-core/src/stage5e_no_io_lifecycle.rs", lambda s: insert_stage5e_b3f_outer_item(s, "    pub(crate) mod b3f_unrestricted_outer_bridge {\n        pub(super) fn take_callback_outcome(\n            escrow: super::Stage5ePaperCallbackResultEscrow,\n        ) -> super::Stage5ePaperCallbackOutcome {\n            escrow.callback_outcome\n        }\n    }")), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r11-full-rebind-outer-scalar-extractor", "Stage 5E B3F outer exact item/signature vector drift", mutate_stage5e_b3f_source_and_full_rebind_checker("crates/strategy-runtime-core/src/stage5e_no_io_lifecycle.rs", lambda s: insert_stage5e_b3f_outer_item(s, "    pub(crate) fn b3f_unrestricted_callback_authority_id(\n        escrow: &Stage5ePaperCallbackResultEscrow,\n    ) -> [u8; 32] {\n        escrow.callback_authority_id\n    }")), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r11-full-rebind-outer-receipt-bridge", "Stage 5E B3F outer exact item/signature vector drift", mutate_stage5e_b3f_source_and_full_rebind_checker("crates/strategy-runtime-core/src/stage5e_no_io_lifecycle.rs", lambda s: insert_stage5e_b3f_outer_item(s, "    pub(crate) fn b3f_unrestricted_terminal_receipt_bridge(\n        receipt: callback_settlement::Stage5ePaperSettlementTerminalReceipt,\n    ) -> callback_settlement::Stage5ePaperSettlementTerminalReceipt {\n        receipt\n    }")), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r11-full-rebind-transition-second-control-input", "Stage 5E callback_settlement critical function capability signature drift: validate_and_settle_stage5e_paper_callback_escrow", mutate_stage5e_b3f_source_and_full_rebind_checker("crates/strategy-runtime-core/src/stage5e_no_io_lifecycle.rs", lambda s: s.replace("        pub(crate) fn validate_and_settle_stage5e_paper_callback_escrow(\n            escrow: Stage5ePaperCallbackResultEscrow,\n        )", "        pub(crate) fn validate_and_settle_stage5e_paper_callback_escrow(\n            escrow: Stage5ePaperCallbackResultEscrow,\n            _bypass_preflight: bool,\n        )", 1)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r11-full-rebind-success-receipt-seal-removed", "Stage 5E callback_settlement critical function capability signature drift: construct_stage5e_validated_paper_settlement_receipt", mutate_stage5e_b3f_source_and_full_rebind_checker("crates/strategy-runtime-core/src/stage5e_no_io_lifecycle.rs", lambda s: s.replace("            settlement_identity: [u8; 32],\n            _seal: Stage5ePaperSettlementSuccessSeal,\n        ) -> Stage5eValidatedPaperSettlementReceipt {", "            settlement_identity: [u8; 32],\n        ) -> Stage5eValidatedPaperSettlementReceipt {", 1)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r11-full-rebind-terminal-receipt-seal-removed", "Stage 5E callback_settlement critical function capability signature drift: construct_stage5e_paper_settlement_terminal_receipt", mutate_stage5e_b3f_source_and_full_rebind_checker("crates/strategy-runtime-core/src/stage5e_no_io_lifecycle.rs", lambda s: s.replace("            audit_commitment: [u8; 32],\n            _seal: Stage5ePaperSettlementTerminalSeal,\n        ) -> Stage5ePaperSettlementTerminalReceipt {", "            audit_commitment: [u8; 32],\n        ) -> Stage5ePaperSettlementTerminalReceipt {", 1)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r11-full-rebind-terminal-mapper-seal-removed", "Stage 5E callback_settlement critical function capability signature drift: map_stage5c_settlement_error_exact", mutate_stage5e_b3f_source_and_full_rebind_checker("crates/strategy-runtime-core/src/stage5e_no_io_lifecycle.rs", lambda s: s.replace("        pub(crate) fn map_stage5c_settlement_error_exact(\n            error: crate::stage5c_paper_host::Stage5cIntentSettlementError,\n            _seal: &Stage5ePaperSettlementTerminalSeal,\n        )", "        pub(crate) fn map_stage5c_settlement_error_exact(\n            error: crate::stage5c_paper_host::Stage5cIntentSettlementError,\n        )", 1)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r11-full-rebind-settlement-identity-seal-removed", "Stage 5E callback_settlement critical function capability signature drift: construct_stage5e_b3f_settlement_identity", mutate_stage5e_b3f_source_and_full_rebind_checker("crates/strategy-runtime-core/src/stage5e_no_io_lifecycle.rs", lambda s: s.replace("            audit_commitment: [u8; 32],\n            _seal: &Stage5ePaperSettlementSuccessSeal,\n        ) -> [u8; 32] {", "            audit_commitment: [u8; 32],\n        ) -> [u8; 32] {", 1)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r12-full-rebind-preflight-magic-proceed", "Stage 5E B3F production-region semantic token digest drift", mutate_stage5e_b3f_source_and_full_rebind_checker("crates/strategy-runtime-core/src/stage5e_no_io_lifecycle.rs", lambda s: s.replace("            let escrow = preflight.escrow;\n            let expected = construct_stage5c_expected_preflight_binding(escrow, seal);", "            let escrow = preflight.escrow;\n            if escrow.callback_authority_id == [0x42; 32] {\n                return Stage5ePaperSettlementPreflightDecision::ProceedOk;\n            }\n            let expected = construct_stage5c_expected_preflight_binding(escrow, seal);", 1)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r12-full-rebind-stage5c-magic-validated-proof", "Stage 5C B3F production-region semantic token digest drift", mutate_stage5e_b3f_source_and_full_rebind_checker("crates/strategy-runtime-core/src/stage5c_paper_host.rs", lambda s: s.replace("        .bootstrap_receipt()\n        .admission;\n    if admission.strategy_id() != attribution_snapshot.strategy_id {", "        .bootstrap_receipt()\n        .admission;\n    if admission.strategy_id() == \"__b3f_review_bypass__\" {\n        return Ok(Stage5eStage5cPreflightValidatedProof(()));\n    }\n    if admission.strategy_id() != attribution_snapshot.strategy_id {", 1)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r12-full-rebind-authority-identity-magic-accept", "Stage 5E B3F production-region semantic token digest drift", mutate_stage5e_b3f_source_and_full_rebind_checker("crates/strategy-runtime-core/src/stage5e_no_io_lifecycle.rs", lambda s: s.replace("            let audit = &escrow.audit_lineage;\n            let nonzero = [", "            let audit = &escrow.audit_lineage;\n            if escrow.callback_authority_id == [0x43; 32] {\n                return true;\n            }\n            let nonzero = [", 1)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r12-full-rebind-chronology-magic-accept", "Stage 5E B3F production-region semantic token digest drift", mutate_stage5e_b3f_source_and_full_rebind_checker("crates/strategy-runtime-core/src/stage5e_no_io_lifecycle.rs", lambda s: s.replace("        ) -> bool {\n            audit._sequence_observed_at <= audit._sequence_expires_at", "        ) -> bool {\n            if audit._callback_authority_id == [0x44; 32] {\n                return true;\n            }\n            audit._sequence_observed_at <= audit._sequence_expires_at", 1)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r12-full-rebind-callback-validation-weakened", "Stage 5E B3F production-region semantic token digest drift", mutate_stage5e_b3f_source_and_full_rebind_checker("crates/strategy-runtime-core/src/stage5e_no_io_lifecycle.rs", lambda s: s.replace("                PrivateStage5ePaperCallbackOutcome::ValidationError(_) => {\n                    Stage5ePaperSettlementPreflightDecision::Terminal(\n                        Stage5ePaperSettlementTerminalReason::CallbackValidationError,\n                    )\n                }", "                PrivateStage5ePaperCallbackOutcome::ValidationError(_) => {\n                    Stage5ePaperSettlementPreflightDecision::ProceedOk\n                }", 1)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r12-full-rebind-settlement-terminal-branch-modified", "Stage 5E B3F production-region semantic token digest drift", mutate_stage5e_b3f_source_and_full_rebind_checker("crates/strategy-runtime-core/src/stage5e_no_io_lifecycle.rs", lambda s: s.replace("                Err(terminal) => Err(terminal.construct_stage5e_terminal_receipt(", "                Err(terminal) => {\n                    let _frozen_terminal_branch = &terminal;\n                    Err(terminal.construct_stage5e_terminal_receipt(", 1).replace("                    Stage5ePaperSettlementTerminalSeal(()),\n                )),\n            }\n        }\n\n        fn validate_preflight", "                    Stage5ePaperSettlementTerminalSeal(()),\n                ))\n                }\n            }\n        }\n\n        fn validate_preflight", 1)), "5E-b3f-callback-settlement-escrow-design", True),
        Case("stage5e-b3f-r12-full-rebind-audit-domain-literal-modified", "Stage 5E B3F production-region semantic token digest drift", mutate_stage5e_b3f_source_and_full_rebind_checker("crates/strategy-runtime-core/src/stage5e_no_io_lifecycle.rs", lambda s: s.replace('b"stage5e-b3f-audit-commitment-v1\\0"', 'b"stage5e-b3f-audit-commitment-v2\\0"', 1)), "5E-b3f-callback-settlement-escrow-design", True),
    ]
    ...
    constructor_inputs = json.loads(
        (
            ROOT
            / "docs/stage-5/stage5e-b3f-callback-settlement-escrow-design-inventory.json"
        ).read_text()
    )["stage5c_preflight_bridge_contract"]["expected_binding_constructor_inputs"]
    for input_index, _ in enumerate(constructor_inputs):
        cases.append(
            Case(
                f"stage5e-b3f-r4-expected-binding-constructor-input-{input_index}-removed",
                "design inventory drift",
                mutate_stage5e_b3f_inventory_for_checker(
                    lambda p, ii=input_index: p["stage5c_preflight_bridge_contract"][
                        "expected_binding_constructor_inputs"
                    ].pop(ii)
                ),
                "5E-b3f-callback-settlement-escrow-design",
                True,
