...
def main() -> None:
    ...
    require(authority.get("stage") == "8B-I", "stage drift")
    require(authority.get("status") == "no_send_implementation_candidate", "status drift")
    require(authority.get("branch") == "stage8b-i", "branch drift")
    ...
    require(authority.get("external_compile_fail_negative_cases") == 12, "compile negative count drift")
    ...
    require(authority.get("acceptance_rows") == 60 and authority.get("negative_cases") == 40, "authority count drift")

    with paths["matrix"].open(newline="", encoding="utf-8") as handle:
        rows = list(csv.DictReader(handle))
    require([row.get("id") for row in rows] == [f"I-{index:03d}" for index in range(1, 61)], "matrix ID/count drift")
    ...
    require(numbers == list(range(1, 41)), "negative inventory must be exact 1..40")
    ...
    for marker in (
        "capability.compose_stage8a2_no_send(&mut sink)", "context.classify(observation)",
    ...
    authority_types = (
        ...
        "Stage8bOperatorArm", "Stage8bFreshPreflightApproved", "Stage8bSealedAttemptCommitted",
    ...
    require("stage8b-i-external-compile-fail: PASS positive=1 negative=12" in compile_script, "compile-fail count marker drift")
    for marker in (
    ...
    for marker in (
        ...
        "stage8b-i-gate: PASS rows=60 negatives=40 compile_fail=12",
    ):
    ...
    for marker in (
        "Stage 8B-I", "no-send production types", "invoke_stage8b_operator_once",
        "compose_stage8b_effect_authority", "O_CREAT|O_EXCL|O_NOFOLLOW",
        "six prefixes", "Stage 8B-I does not authorize 8B-IT",
    ):
        require(marker in contract, f"contract marker missing: {marker}")

    print("stage8b-i-check: PASS rows=60 negatives=40 facade=1 root=1 compile_fail=12 hmac=true path_pinned=true arm_race=true crash_windows=6 no_send=true finam=false redis=false dispatch=false live=false real_orders=false stage8b_it=false stage8b_p=false stage8b_xe=false stage12=false")
