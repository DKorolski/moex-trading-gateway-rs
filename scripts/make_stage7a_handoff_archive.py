#!/usr/bin/env python3
"""Create immutable Stage 7A-R2a source plus evidence handoff."""
...
def main() -> None:
    ...
    if run(["git", "rev-parse", "HEAD^"]) != checker.R2A_PREDECESSOR:
    ...
    with tempfile.TemporaryDirectory(prefix="stage7a-handoff-") as temp:
        ...
        acceptance = json.loads((artifact_dir / "stage7a-r2a-acceptance.json").read_text())
        fault_matrix = json.loads((artifact_dir / "stage7a-r2a-fault-matrix.json").read_text())
    ...
    manifest = json.dumps(
        {
            "schema_version": 1,
            "source_ref": head,
            "predecessor": checker.R2A_PREDECESSOR,
    ...
    marker = (
        ...
        f"predecessor={checker.R2A_PREDECESSOR}\n"
    ...
    evidence = json.dumps(
        {
            ...
            "semantic_proof_map": "docs/stage-7/stage7a-r2a-acceptance-proof-map.json",
