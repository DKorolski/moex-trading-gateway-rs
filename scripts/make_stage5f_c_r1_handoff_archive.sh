...
if [[ -n "$(git -C "$repo_root" status --porcelain --untracked-files=all)" ]]; then
  echo "Refusing to build Stage 5F-c R1 handoff: source tree is dirty." >&2
...
tmp_dir="$(mktemp -d "${TMPDIR:-/tmp}/stage5f-c-r1-handoff.XXXXXX")"
...
run_gate() {
  local label="$1"
  shift
  local stdout_member="stage5f-r1-${label}-stdout.txt"
  local stderr_member="stage5f-r1-${label}-stderr.txt"
  local result_member="stage5f-r1-${label}-result.json"
  ...
  if [[ "$exit_code" -ne 0 ]]; then
    cat "$stdout_path"
    cat "$stderr_path" >&2
    echo "Stage 5F-c R1 gate failed: $label" >&2
    exit "$exit_code"
  fi
...
python3 - "$repo_root" "$tmp_dir/stage5f-r1-source-tree-manifest.json" \
  "$source_ref" "$source_commit" "$head_tree" <<'PY'
import hashlib
import json
import subprocess
import sys
...
git -C "$repo_root" cat-file commit "$source_ref" \
  >"$tmp_dir/stage5f-r1-commit-object.txt"

python3 - "$tmp_dir/handoff-commit.txt" "$archive_name" "$source_commit" "$source_ref" <<'PY'
import sys
from pathlib import Path

output, archive_name, source_commit, source_ref = sys.argv[1:]
...
python3 - "$tmp_dir" "$archive_name" "$source_commit" "$source_ref" \
  "$head_tree" "$rustc_version" "$cargo_version" <<'PY'
import hashlib
import json
import sys
from pathlib import Path
...
root = Path(sys.argv[1])
archive_name, source_commit, source_ref, head_tree, rustc_version, cargo_version = sys.argv[2:]
labels = [
    "fmt",
    "workspace-tests",
    "doctests",
...
gates = []
for label in labels:
    member = f"stage5f-r1-{label}-result.json"
    gates.append({
        "label": label,
...
payload = {
    ...
    "commit_object_sha256": sha256(root / "stage5f-r1-commit-object.txt"),
    "gates": gates,
    "head_tree": head_tree,
    ...
    "source_commit": source_commit,
    "source_ref": source_ref,
    "source_tree_manifest_sha256": sha256(root / "stage5f-r1-source-tree-manifest.json"),
    "stage": "5F-c-R1-review-closure",
    "status": "review_required_before_5f_d",
}
(root / "stage5f-r1-evidence-manifest.json").write_text(
    json.dumps(payload, indent=2, sort_keys=True) + "\n",
    encoding="utf-8",
)
PY
...
members = [
    "handoff-commit.txt",
    "stage5f-r1-commit-object.txt",
    "stage5f-r1-source-tree-manifest.json",
    "stage5f-r1-evidence-manifest.json",
]
for label in labels:
    members.extend([
        f"stage5f-r1-{label}-result.json",
        f"stage5f-r1-{label}-stdout.txt",
        f"stage5f-r1-{label}-stderr.txt",
    ])
...
preseal_stdout_member="stage5f-r1-archive-safety-stdout.txt"
preseal_stderr_member="stage5f-r1-archive-safety-stderr.txt"
...
if [[ "$preseal_exit_code" -ne 0 ]]; then
  ...
  echo "Stage 5F-c R1 preseal archive safety failed." >&2
...
payload = {
    "checked_evidence_manifest_sha256": sha256(root / "stage5f-r1-evidence-manifest.json"),
    "checked_source_tree_manifest_sha256": sha256(root / "stage5f-r1-source-tree-manifest.json"),
    "command": [
        "python3",
        "scripts/stage5f_c_r1_handoff_safety_check.py",
        "--archive",
        archive_name,
...
(root / "stage5f-r1-archive-safety-result.json").write_text(
    json.dumps(payload, indent=2, sort_keys=True) + "\n",
    encoding="utf-8",
)
PY
...
root = Path(sys.argv[2])
members = [
    "stage5f-r1-archive-safety-result.json",
    "stage5f-r1-archive-safety-stdout.txt",
    "stage5f-r1-archive-safety-stderr.txt",
]
with zipfile.ZipFile(archive, "a", compression=zipfile.ZIP_DEFLATED) as handle:
    for member in members:
        handle.write(root / member, member)
...
python3 "$repo_root/scripts/stage5f_c_r1_handoff_safety_check.py" \
  --archive "$tmp_archive"
...
echo "Stage 5F-c R1 handoff archive: $archive_path"
echo "Stage 5F-c R1 handoff SHA-256: $sha_path"
cat "$sha_path"
