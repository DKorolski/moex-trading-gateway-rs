#!/usr/bin/env python3
"""Git-backed delta and archive preseal for Stage 5G-e-d-b R2."""
...
def main() -> None:
    ...
    if delta != checker.EXPECTED_DELTA:
        fail(f"exact e-d-b R2 changed-path allowlist drifted: {delta}")
    ...
    print(
        f"stage5g-edb-r2-preseal: PASS delta={len(delta)}/{len(checker.EXPECTED_DELTA)} "
