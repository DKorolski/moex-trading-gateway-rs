#!/usr/bin/env python3
"""Generate M3j-11 final operator-run authorization gate evidence."""
...
DOC = Path("docs/m3j11-final-operator-run-authorization-gate.md")

CHECKS = [
    ["cargo", "fmt", "--all", "--check"],
    ["cargo", "test", "-p", "finam-gateway", "m3j11", "--", "--nocapture"],
    ["cargo", "test", "--all"],
    ["cargo", "clippy", "--workspace", "--all-targets", "--", "-D", "warnings"],
    ["bash", "scripts/forbidden_surface_scan.sh"],
    ["bash", "scripts/forbidden_surface_negative_harness.sh"],
    ["bash", "scripts/order_endpoint_scanner_transition_spec.sh"],
    ["python3", "-m", "py_compile", "scripts/m3j11_final_operator_run_authorization_evidence.py"],
]
...
def evidence_summary(root: Path) -> dict[str, Any]:
    ...
    required_patterns = [
        ...
        "m3j11_final_authorization_ready_but_not_attached_or_invoked",
        "m3j11_missing_actual_artifact_or_boundary_request_blocks_authorization",
    ]
    forbidden_patterns = [
        ".post(",
        ".delete(",
    ...
    return {
        ...
        "final_authorization_gate_only": "final_authorization_gate_only: true" in source,
        "live_micro_go": False,
        ...
        "m3j11_documented": "M3j-11 is the final authorization gate model" in doc,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate M3j-11 final operator-run authorization evidence."
    )
    parser.add_argument("--source-archive", type=Path)
    parser.add_argument("--m3j10-evidence", type=Path)
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("reports/m3j-pre-live/m3j11-final-operator-run-authorization-evidence.json"),
    )
    ...
    m3j10_evidence = artifact_summary(
        (root / args.m3j10_evidence).resolve() if args.m3j10_evidence else None
    )
    ...
    evidence = {
        ...
        "summary": summary,
        "artifact_manifest": {"m3j10_evidence": m3j10_evidence},
        "final_authorization_gate_only": True,
        "live_micro_go": False,
        "runtime_live_attachment_allowed": False,
        "boundary_invocation_performed": False,
