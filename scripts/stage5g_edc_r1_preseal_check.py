#!/usr/bin/env python3
"""One-commit source and archive seal for Stage 5G-e-d-c R1."""
...
EXPECTED = sorted([
    ...
    "docs/stage-5/stage5g-e-d-c-r1-application-authority-contract.json",
    "docs/stage-5/stage5g-e-d-c-r1-application-authority-contract.md",
    "scripts/make_stage5g_edc_r1_handoff_archive.py",
    "scripts/stage5g_edc_r1_check.py",
    "scripts/stage5g_edc_r1_gate.sh",
    "scripts/stage5g_edc_r1_negative_harness.py",
    "scripts/stage5g_edc_r1_preseal_check.py",
])
...
def fail(message: str) -> None:
    raise SystemExit(f"stage5g-edc-r1-preseal: FAIL: {message}")
...
def main() -> None:
    if output("git", "rev-parse", "HEAD^") != checker.BASE:
        fail("HEAD is not the direct successor to 18240b2")
    ...
    print(f"stage5g-edc-r1-preseal: PASS delta={len(delta)} archive={len(archived)}")
