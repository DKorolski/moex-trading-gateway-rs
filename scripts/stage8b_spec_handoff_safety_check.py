...
def check(path: str) -> dict[str, object]:
    with zipfile.ZipFile(path) as archive:
        ...
        if evidence.get("acceptance_rows") != 80 or evidence.get("negative_cases") != 60:
            raise ValueError("count mismatch")
        if evidence.get("specification_only") is not True:
            raise ValueError("specification scope mismatch")
        for key in ("production_implementation", "stage8b_i", "stage8b_p", "stage8b_x", "operator_arming", "finam_post_delete", "network_send", "redis_live", "ack_readiness_publication", "broker_dispatch", "runtime_live", "real_orders", "stage12"):
