#!/usr/bin/env python3
"""Verify a complete, commit-bound Stage 5G-b R1 handoff archive."""
...
SOURCE_MANIFEST = "stage5g-b-r1-source-tree-manifest.json"
EVIDENCE_MANIFEST = "stage5g-b-r1-evidence-manifest.json"
COMMIT_OBJECT = "stage5g-b-r1-commit-object.txt"
COMMIT_MARKER = "handoff-commit.txt"
SAFETY_RESULT = "stage5g-b-r1-archive-safety-result.json"
SAFETY_STDOUT = "stage5g-b-r1-archive-safety.stdout.txt"
SAFETY_STDERR = "stage5g-b-r1-archive-safety.stderr.txt"
EVIDENCE_PREFIX = "stage5g-b-r1-evidence/"
HEX40 = re.compile(r"^[0-9a-f]{40}$")
HEX64 = re.compile(r"^[0-9a-f]{64}$")

EXPECTED_COMMANDS: dict[str, list[str]] = {
    "stage5g-a-snapshot": ["bash", "scripts/stage5g_a_snapshot_gate.sh"],
    "base-checker": ["python3", "scripts/stage5g_b_mock_ack_check.py"],
    "base-negative": ["python3", "scripts/stage5g_b_mock_ack_negative_harness.py"],
    "r1-checker": ["python3", "scripts/stage5g_b_r1_check.py"],
    "r1-negative": ["python3", "scripts/stage5g_b_r1_negative_harness.py"],
    "fmt": ["cargo", "fmt", "--all", "--", "--check"],
    "focused-debug": [
        "cargo", "test", "-p", "strategy-runtime-core", "stage5g_mock_ack", "--quiet",
    ],
    "focused-release": [
        "cargo", "test", "-p", "strategy-runtime-core", "--release",
        "stage5g_mock_ack", "--quiet",
    ],
...
EXPECTED_CHANGED_PATHS = sorted(
    {
        ...
        "docs/stage-5/5g-b-r1-lifecycle-evidence-hardening.md",
        "docs/stage-5/stage5g-a-acceptance-descriptor.json",
        "docs/stage-5/stage5g-b-r1-contract.json",
        "scripts/stage5g_a_snapshot_gate.sh",
        "scripts/stage5g_b_r1_check.py",
        "scripts/stage5g_b_r1_negative_harness.py",
        "scripts/make_stage5g_b_r1_handoff_archive.py",
        "scripts/stage5g_b_r1_handoff_safety_check.py",
    }
)
...
def parse_marker(raw: bytes) -> dict[str, str]:
    ...
    common.exact_keys(
        marker,
        {
            ...
            "immutable_base", "stage5g_a_accepted_ref",
        },
        "handoff marker",
    )
    return marker
...
def validate_source(
    ...
    common.exact_keys(
        manifest,
        {
            ...
            "parent_ref", "immutable_base", "stage5g_a_accepted_ref", "head_tree", "members",
        },
        "source manifest",
    )
    ...
    common.require(manifest["source_commit"], marker["source_commit"], "source short ref")
    common.require(manifest["source_branch"], BRANCH, "source branch")
    common.require(manifest["parent_ref"], BASE_REF, "R1 direct parent")
    common.require(manifest["immutable_base"], BASE_REF, "immutable base")
    common.require(manifest["stage5g_a_accepted_ref"], STAGE5G_A_REF, "Stage 5G-a ref")
    ...
    for index, raw_member in enumerate(members):
        member = common.exact_keys(raw_member, {"git_mode", "path", "sha256"}, f"source[{index}]")
    ...
    if f"parent {BASE_REF}" not in commit_lines:
        common.fail("R1 immutable-base parent binding missing")
    return source_paths


def validate_gate_marker(label: str, stdout: str) -> None:
    markers = {
        "stage5g-a-snapshot": "stage5g-a-snapshot-gate: PASS",
        "base-checker": "stage5g-b-mock-ack-check: PASS",
        "base-negative": "stage5g-b-negative-harness: PASS 15/15",
        "r1-checker": "stage5g-b-r1-check: PASS",
        "r1-negative": "stage5g-b-r1-negative-harness: PASS 18/18",
    ...
    if label in {"focused-debug", "focused-release"} and "20 passed" not in stdout:
        common.fail(f"focused 20-test marker missing: {label}")
...
def validate_evidence(files: dict[str, bytes], marker: dict[str, str]) -> None:
    ...
    common.exact_keys(
        manifest,
        {
            "schema_version", "stage", "source_ref", "source_branch", "immutable_base",
            "stage5g_a_accepted_ref", "gate_count", "gates", "repository_state",
            "closed_surfaces",
        },
        "evidence manifest",
    )
    ...
    common.require(manifest["immutable_base"], BASE_REF, "evidence immutable base")
    common.require(manifest["stage5g_a_accepted_ref"], STAGE5G_A_REF, "evidence Stage 5G-a")
    closed = manifest["closed_surfaces"]
    if not isinstance(closed, dict) or set(closed) != {
        "redis_live_consumer", "redis_consumer_groups", "finam_transport", "http_post_delete",
        "broker_dispatch_execution", "order_trade_position_events", "runtime_live",
        "real_orders", "stage5g_c", "stage6",
    } or any(value is not False for value in closed.values()):
        common.fail("R1 closed-surface inventory is not exact and closed")
    ...
    for stem in ("git_status", "changed_paths"):
        ...
        common.require(
            common.sha256(files[member_name]), repository[f"{stem}_sha256"],
            f"repository evidence hash {stem}",
        )
    common.require(files[repository["git_status_member"]], b"", "embedded git status")
    changed = files[repository["changed_paths_member"]].decode("utf-8").splitlines()
    common.require(changed, EXPECTED_CHANGED_PATHS, "embedded changed paths")
...
def validate_archive(
    archive_path: Path, allow_missing_final_safety: bool
    ...
    common.require(marker["immutable_base"], BASE_REF, "marker immutable base")
    common.require(marker["stage5g_a_accepted_ref"], STAGE5G_A_REF, "marker Stage 5G-a")
    ...
    if not allow_missing_final_safety:
        ...
        common.require(result.get("source_ref"), marker["source_ref"], "safety result source")
...
def main() -> int:
    ...
    try:
        ...
        print(f"stage5g-b-r1-handoff-safety: FAIL: {error}", file=sys.stderr)
        return 1
    result: dict[str, Any] = {
        "schema_version": 1,
        "stage": STAGE,
    ...
    print(
        "stage5g-b-r1-handoff-safety: PASS "
