#!/usr/bin/env python3
"""Verify the self-attesting Stage 5G-c replay identity archive."""
...
STAGE = "5G-c-replay-package-identity-gate"
BRANCH = "stage5g-lifecycle"
BASE_REF = "470d898104b06fbf725f532554ba7a0fbde7e5c3"
SOURCE_MANIFEST = "stage5g-c-replay-identity-source-tree-manifest.json"
EVIDENCE_MANIFEST = "stage5g-c-replay-identity-evidence-manifest.json"
COMMIT_OBJECT = "stage5g-c-replay-identity-commit-object.txt"
COMMIT_MARKER = "handoff-commit.txt"
SAFETY_RESULT = "stage5g-c-replay-identity-archive-safety-result.json"
SAFETY_STDOUT = "stage5g-c-replay-identity-archive-safety.stdout.txt"
SAFETY_STDERR = "stage5g-c-replay-identity-archive-safety.stderr.txt"
EVIDENCE_PREFIX = "stage5g-c-replay-identity-evidence/"
EXPECTED_COMMANDS: dict[str, list[str]] = {
    "authority-check": ["python3", "scripts/stage5g_c_replay_identity_authority_check.py"],
    "semantic-negative": ["python3", "scripts/stage5g_c_replay_identity_negative_harness.py"],
...
EXPECTED_CHANGED_PATHS = sorted({
    ...
    "docs/adr/adr-stage5g-c-replay-package-identity.md",
...
CLOSED_SURFACES = {
    "stage5g_d", "redis_live_consumer_groups", "finam_transport",
...
def validate_gate_marker(label: str, stdout: str) -> None:
    markers = {
        "authority-check": "stage5g-c-replay-identity-authority-check: PASS",
        "semantic-negative": "stage5g-c-replay-identity-negative-harness: PASS 12/12",
...
def main() -> int:
    ...
    try:
        ...
        print(f"stage5g-c-replay-identity-handoff-safety: FAIL: {error}", file=sys.stderr)
        return 1
    result: dict[str, Any] = {
        "schema_version": 1, "stage": STAGE, "source_ref": marker["source_ref"],
        "archive_name": args.archive.name, "preseal_exit_code": 0,
        "member_count_before_final_safety": member_count, "verdict": "PASS",
    }
    if args.result_out is not None:
        args.result_out.write_text(json.dumps(result, indent=2, sort_keys=True) + "\n")
    print(f"stage5g-c-replay-identity-handoff-safety: PASS source_ref={marker['source_ref']} members={member_count}")
    return 0
