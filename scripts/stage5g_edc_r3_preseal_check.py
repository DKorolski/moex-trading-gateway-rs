#!/usr/bin/env python3
"""One-commit source and archive seal for Stage 5G-e-d-c R3."""
...
EXPECTED = sorted([
    ...
    "docs/stage-5/stage5g-e-d-c-r3-source-proof-field-map.md",
    "scripts/make_stage5g_edc_r3_handoff_archive.py",
    "scripts/stage5g_edc_r3_check.py",
    "scripts/stage5g_edc_r3_gate.sh",
    "scripts/stage5g_edc_r3_negative_harness.py",
    "scripts/stage5g_edc_r3_preseal_check.py",
])
...
def fail(message: str) -> None:
    raise SystemExit(f"stage5g-edc-r3-preseal: FAIL: {message}")
...
def main() -> None:
    if output("git", "rev-parse", "HEAD^") != checker.BASE:
        fail("HEAD is not the direct successor to 95901eb")
    ...
    print(f"stage5g-edc-r3-preseal: PASS delta={len(delta)} archive={len(archived)}")
