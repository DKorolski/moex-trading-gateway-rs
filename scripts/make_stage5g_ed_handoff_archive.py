#!/usr/bin/env python3
"""Build an immutable, origin-bound Stage 5G-e-d-b R5 review handoff."""
...
STAGE = "5G-e-d-b-r5"
REQUIRED_PARENT = "66c5fbd2518ec2e7398c88bb59cc7e4dae3ce1bd"
EXPECTED_DELTA = [
...
def exact_delta() -> list[tuple[str, str]]:
    output = run_text(["git", "diff", "--name-status", f"{REQUIRED_PARENT}..HEAD"])
...
def main() -> None:
    ...
    if parent_ref != REQUIRED_PARENT:
        fail(f"e-d-b R5 must be one clean successor to {REQUIRED_PARENT}; got parent {parent_ref}")
    ...
    if delta != EXPECTED_DELTA:
        fail(f"exact e-d-b R5 changed-path allowlist drifted: {delta}")
    ...
    if gate.returncode != 0:
        ...
        fail("stage5g_edb_r5_gate.sh failed")
    ...
    gate_result = json.dumps(
        {
            ...
            "stage": STAGE,
            "source_ref": source_ref,
            "command": ["bash", "scripts/stage5g_edb_r5_gate.sh"],
            "exit_code": 0,
    ...
    generated = {
        ...
        "stage5g-e-d-b-r5-gate-result.json": (gate_result, 0o644),
        "stage5g-e-d-b-r5-gate-output.txt": (gate.stdout, 0o644),
    }
