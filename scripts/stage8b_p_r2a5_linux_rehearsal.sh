...
if [[ "$(id -u)" != "0" ]]; then
  echo "stage8b-r2a5-rehearsal: must run as root" >&2
  exit 1
fi
...
LAUNCHER="$BIN_DIR/stage8b-r2a5-launcher"
...
for operation in PLACE CANCEL; do
  ...
  # The controlled layout publishes owner/API-tagged upstream fixtures first;
  # producers never consume a manually prepared final R2A intermediate record.
  test -f /var/lib/moex-trading/operational-authorities/stage7b-current-recovery-seal.json
  test ! -e /var/lib/moex-trading/stage8b/r2a5/authoritative-stores
  pids=()
  ...
  "$LAUNCHER" --controlled-fixed-layout >"/tmp/stage8b-r2a5-${operation,,}-evidence.json"
  wait "$server_pid"
  grep -Fq "\"operation\":\"$operation\"" "/tmp/stage8b-r2a5-${operation,,}-evidence.json"
  grep -Fq '"authorization_status":"ISSUED"' "/tmp/stage8b-r2a5-${operation,,}-evidence.json"
  echo "stage8b-r2a5-fixed-layout-$operation: PASS"
done

echo "stage8b-r2a5-linux-rehearsal: PASS"
