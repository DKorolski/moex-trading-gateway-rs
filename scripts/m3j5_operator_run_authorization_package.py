#!/usr/bin/env python3
"""Generate M3j-5 operator-run first-live-micro authorization package evidence."""
...
DOC = Path("docs/m3j5-operator-run-first-live-micro-authorization.md")
...
CHECKS = [
    ...
    ["python3", "-m", "py_compile", "scripts/m3j5_operator_run_authorization_package.py"],
]
...
def evidence_summary(root: Path) -> dict[str, Any]:
    ...
    required_patterns = [
        ...
        "live_micro_go: false",
        "m3j5_current_authorization_is_not_authorized_without_operator_go_artifact",
        "m3j5_operator_run_candidate_still_does_not_enable_live_boundary",
        "m3j5_missing_live_path_review_or_open_boundary_blocks_authorization",
    ]
    forbidden_patterns = [
        "M3j5OperatorRunAuthorizationReport::reqwest",
        "M3j5OperatorRunAuthorizationReport::POST",
        "M3j5OperatorRunAuthorizationReport::DELETE",
        "api.finam.ru/order",
    ]
    return {
        ...
        "operator_run_authorization_package_present": "m3j5_operator_run_authorization_report"
        in source,
        "current_authorization_not_authorized": "M3j5FirstLiveMicroAuthorization::NotAuthorized"
        in source,
        "operator_run_candidate_does_not_enable_live_micro": "live_micro_go: false" in source
        and "m3j5_operator_run_candidate_still_does_not_enable_live_boundary" in source,
        ...
        in source,
        "authorization_documented": "M3j-5 is a separate operator-run authorization package"
        in doc,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate M3j-5 operator-run first-live-micro authorization evidence."
    )
    parser.add_argument("--source-archive", type=Path)
    parser.add_argument("--m3j4-evidence", type=Path)
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("reports/m3j-pre-live/m3j5-operator-run-authorization-package.json"),
    )
    ...
    m3j4_evidence = artifact_summary(
        (root / args.m3j4_evidence).resolve() if args.m3j4_evidence else None
    )
    ...
    artifact_manifest_ok = m3j4_evidence["provided"]
    policy_ok = (
        all(summary["required_patterns_present"].values())
        and all(summary["forbidden_patterns_absent"].values())
        and summary["operator_run_authorization_package_present"]
        and summary["current_authorization_not_authorized"]
        and summary["operator_run_candidate_does_not_enable_live_micro"]
        and summary["live_micro_go_false"]
        and summary["no_live_boundary"]
        and summary["no_stop_sltp_bracket"]
        and summary["authorization_documented"]
    )
    ...
    evidence = {
        ...
        "summary": summary,
        "artifact_manifest": {"m3j4_evidence": m3j4_evidence},
        "current_authorization": "NOT_AUTHORIZED",
        "explicit_operator_go_artifact_present": False,
        "operator_run_candidate_available_in_source_model": True,
        "operator_run_candidate_does_not_enable_live_micro": True,
        "live_order_path_implemented": False,
        "live_order_path_diff_reviewed": False,
        "checks": checks,
        "all_checks_ok": all_checks_ok,
        "artifact_manifest_ok": artifact_manifest_ok,
        "operator_run_authorization_package_present": summary[
            "operator_run_authorization_package_present"
        ],
        "live_micro_go": False,
