#!/usr/bin/env python3
"""Evaluate all frozen Stage 7A rows from the reviewable R2c proof map."""
...
def main() -> None:
    ...
    if ids != expected_ids or not all(row["Blocking"] == "YES" for row in rows):
        raise SystemExit("stage7a-r2c-acceptance: FAIL: frozen matrix identity drift")
    ...
    if proof_ids != expected_ids or len(set(proof_ids)) != 52:
        raise SystemExit("stage7a-r2c-acceptance: FAIL: proof map incomplete or unordered")
    ...
    if pass_count != 52:
        ...
        raise SystemExit(f"stage7a-r2c-acceptance: FAIL rows={failed}")
    print("stage7a-r2c-acceptance: PASS evaluated=52 passed=52 semantic_map=complete")
