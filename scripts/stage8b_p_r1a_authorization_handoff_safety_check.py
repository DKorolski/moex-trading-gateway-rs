#!/usr/bin/env python3
"""Validate an immutable Stage 8B-P R1A corrective handoff."""
...
EVIDENCE = "handoff-evidence/stage8b-p-r1a-authorization-evidence.json"
GATE = "handoff-evidence/stage8b-p-r1a-authorization-gate.txt"
MANIFEST = "handoff-evidence/source-tree-manifest.json"
GENERATED = {"handoff-commit.txt", EVIDENCE, GATE, MANIFEST}
REQUIRED = GENERATED | {
    "docs/stage-8/STAGE8B_P_R1A_ACCEPTANCE_MATRIX_2026-08-25.csv",
    "docs/stage-8/STAGE8B_P_R1A_AUTHORIZATION_CONTRACT_2026-08-25.md",
    "docs/stage-8/STAGE8B_P_R1A_NEGATIVE_INVENTORY_2026-08-25.md",
    "docs/stage-8/stage8b-p-r1a-authorization-authority.json",
    "docs/stage-8/stage8b-p-r1a-freshness-budget-authority.json",
    "docs/stage-8/stage8b-p-r1a-network-policy-authority.json",
    "scripts/stage8b_p_r1a_authorization_check.py",
    "scripts/stage8b_p_r1a_authorization_gate.sh",
    "scripts/stage8b_p_r1a_authorization_negative_harness.py",
    "scripts/make_stage8b_p_r1a_authorization_handoff.py",
    "scripts/stage8b_p_r1a_authorization_handoff_safety_check.py",
}
...
def check(path: str) -> dict[str, object]:
    with zipfile.ZipFile(path) as archive:
        ...
        if evidence.get("stage") != "8B-P" or evidence.get("revision") != "R1A":
            raise ValueError("stage/revision mismatch")
        if evidence.get("status") != "design_only_corrective_authorization_candidate":
        ...
        if evidence.get("r1_candidate_ref") != "12a7aeec20824d3b90e18caa5961ba28a3eb7fd6":
            raise ValueError("R1 lineage mismatch")
        if evidence.get("acceptance_rows") != 64:
            raise ValueError("acceptance count mismatch")
        if evidence.get("r1a_negative_mutations") != 50 or evidence.get("inherited_r1_negative_mutations") != 48:
            raise ValueError("negative count mismatch")
        if evidence.get("total_negative_mutations") != 98:
            raise ValueError("total negative count mismatch")
        for key in (
            "account_credential_used", "broker_readonly_get", "operator_arm_issued",
            "dispatch_attempt_recorded", "transport_entered", "finam_post_delete",
            "broker_effect", "stage8b_p", "stage8b_xe", "redis_execution",
            "broker_dispatch", "runtime_live", "real_orders",
        ):
            if evidence.get(key) is not False:
                raise ValueError(f"closed surface opened: {key}")

        gate = archive.read(GATE)
        if b"stage8b-p-r1a-authorization-gate: PASS rows=64 new_negatives=50 inherited=48 total=98" not in gate:
            raise ValueError("R1A gate marker missing")
        if b"stage8b-p-r1-authorization-negative: PASS 48/48" not in gate:
            raise ValueError("inherited R1 negative marker missing")
...
def main() -> None:
    if len(sys.argv) != 2:
        raise SystemExit("usage: stage8b_p_r1a_authorization_handoff_safety_check.py ARCHIVE")
    try:
        result = check(sys.argv[1])
    except (OSError, ValueError, KeyError, zipfile.BadZipFile, json.JSONDecodeError) as error:
        print(f"stage8b-p-r1a-authorization-handoff-safety: FAIL {error}", file=sys.stderr)
        raise SystemExit(1)
    print("stage8b-p-r1a-authorization-handoff-safety: PASS " + json.dumps(result, sort_keys=True))
