#!/usr/bin/env python3
"""Generate M3j-12 runtime-live attach dry gate evidence."""
...
DOC = Path("docs/m3j12-runtime-live-attach-dry-gate.md")

CHECKS = [
    ["cargo", "fmt", "--all", "--check"],
    ["cargo", "test", "-p", "finam-gateway", "m3j12", "--", "--nocapture"],
    ["cargo", "test", "--all"],
    ["cargo", "clippy", "--workspace", "--all-targets", "--", "-D", "warnings"],
    ["bash", "scripts/forbidden_surface_scan.sh"],
    ["bash", "scripts/forbidden_surface_negative_harness.sh"],
    ["bash", "scripts/order_endpoint_scanner_transition_spec.sh"],
    ["python3", "-m", "py_compile", "scripts/m3j12_runtime_live_attach_dry_gate_evidence.py"],
]
...
def evidence_summary(root: Path) -> dict[str, Any]:
    ...
    required_patterns = [
        ...
        "m3j12_dry_attach_rehearsal_ready_without_live_or_boundary_invocation",
        "m3j12_expired_authorization_missing_kill_switch_or_real_boundary_blocks_rehearsal",
    ]
    forbidden_patterns = [
        ".post(",
        ".delete(",
    ...
    return {
        ...
        "runtime_live_attach_dry_gate_only": "runtime_live_attach_dry_gate_only: true" in source,
        "live_micro_go": False,
        ...
        "m3j12_documented": "M3j-12 is a no-send attach rehearsal package" in doc,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate M3j-12 runtime-live attach dry gate evidence."
    )
    parser.add_argument("--source-archive", type=Path)
    parser.add_argument("--m3j11-evidence", type=Path)
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("reports/m3j-pre-live/m3j12-runtime-live-attach-dry-gate-evidence.json"),
    )
    ...
    m3j11_evidence = artifact_summary(
        (root / args.m3j11_evidence).resolve() if args.m3j11_evidence else None
    )
    ...
    evidence = {
        ...
        "summary": summary,
        "artifact_manifest": {"m3j11_evidence": m3j11_evidence},
        "runtime_live_attach_dry_gate_only": True,
        "live_micro_go": False,
        "runtime_live_attachment_allowed": False,
        "boundary_invocation_performed": False,
