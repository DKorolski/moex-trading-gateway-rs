#!/usr/bin/env python3
"""Create the immutable Stage 5G-e-d-c R3 review handoff."""
...
import stage5g_edc_r3_check as checker
import stage5g_edc_r3_negative_harness as negative

ROOT = Path(__file__).resolve().parents[1]
OUT = ROOT / "reports" / "handoff"
PREDECESSOR_AGGREGATE_NEGATIVE_CASES = 522
...
def fail(message: str) -> None:
    raise SystemExit(f"stage5g-edc-r3-handoff: FAIL: {message}")
...
def main() -> None:
    ...
    gate = subprocess.run(
        ["bash", "scripts/stage5g_edc_r3_gate.sh"],
        cwd=ROOT,
        text=True,
    ...
    r3_new_cases = len(negative.direct_source_map_cases())
    current_head_cases = len(negative.cases())
    evidence = json.dumps({
        "schema_version": 1,
        "stage": "Stage 5G-e-d-c R3",
        "source_ref": head,
        "predecessor": checker.BASE,
        "accepted_predecessor": "95901eb9bf19e103e9acb82fb9726708f356b4cd",
        "gate_exit_code": 0,
        "negative_cases": {
            "predecessor_aggregate": PREDECESSOR_AGGREGATE_NEGATIVE_CASES,
            "current_r3_new": r3_new_cases,
            "current_head": current_head_cases,
            "lineage_aggregate": PREDECESSOR_AGGREGATE_NEGATIVE_CASES + r3_new_cases,
        },
        "source_map_descriptor": "docs/stage-5/stage5g-e-d-c-r3-source-proof-field-map.json",
        "application_evidence_schema_version": 3,
        "grst_witnesses": 12,
        "failure_boundaries": 14,
        "source_oracle_witnesses": 9,
    ...
    members += [
        ...
        ("handoff-evidence/stage5g-edc-r3-full-gate.txt", redacted, 0o644),
        ("handoff-evidence/stage5g-edc-r3-source-manifest.json", source_manifest, 0o644),
        ("handoff-evidence/stage5g-edc-r3-evidence-manifest.json", evidence, 0o644),
        ("handoff-evidence/stage5g-edc-r3-toolchain.txt",
         f"{run(['rustc', '--version'])}\n{run(['cargo', '--version'])}\n".encode(), 0o644),
    ]
