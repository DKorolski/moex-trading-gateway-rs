#!/usr/bin/env python3
"""Generate M3i-4 paper/shadow replay ACK-application evidence."""
...
DOC = Path("docs/m3i4-paper-shadow-replay-ack-application.md")
...
CHECKS = [
    ...
    ["python3", "-m", "py_compile", "scripts/m3i4_paper_shadow_replay_ack_application_evidence.py"],
]
...
def evidence_summary(root: Path) -> dict[str, Any]:
    ...
    required_patterns = [
        ...
        "ack_publish_before_xack",
        "TimeInForce::Day only",
        "m3i4_e2e_live_final_to_strategy_output_m3h_m3e_dry_ack_updates_state",
        "m3i4_ack_outcome_matrix_updates_paper_state_without_live_boundary",
    ]
    forbidden_patterns = [
    ...
    return {
        ...
        "e2e_livefinal_to_m3e_dry_ack_to_state": (
            "m3i4_e2e_live_final_to_strategy_output_m3h_m3e_dry_ack_updates_state"
            in source
        ),
        "ack_outcome_matrix_present": (
            "DryRunAcknowledged" in source
            and "DuplicateAcknowledged" in source
            and "ExpiredDropped" in source
            and "RejectedDropped" in source
            and "MissingOrStaleAck" in source
            and "ManualPolicyRequired" in source
        ),
        "pending_ack_visibility_present": "PendingAckMissingOrStale" in source,
        "tif_policy_scope_documented": (
            "TimeInForce::Day only" in source
            and "IOC/FOK/GTC require" in source
            and "TimeInForce::Day" in doc
        ),
        "only_m3h_output_path": (
            "m3i2_to_m3h_dry_command_candidate" in source
            and "M3hRuntimeDryCommandEmitter" in source
            and "direct_strategy_publish_allowed: false" in source
        ),
        ...
        "closure_documented": "M3i-4 connects the accepted M3i" in doc,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate M3i-4 paper/shadow replay ACK-application evidence."
    )
    parser.add_argument(
        "--source-archive",
        type=Path,
    ...
    policy_ok = (
        ...
        and summary["e2e_livefinal_to_m3e_dry_ack_to_state"]
        and summary["ack_outcome_matrix_present"]
        and summary["pending_ack_visibility_present"]
        and summary["tif_policy_scope_documented"]
    ...
    evidence = {
        ...
        "e2e_livefinal_to_m3e_dry_ack_to_state": summary[
            "e2e_livefinal_to_m3e_dry_ack_to_state"
        ],
        "ack_outcome_matrix_present": summary["ack_outcome_matrix_present"],
        "pending_ack_visibility_present": summary["pending_ack_visibility_present"],
        "tif_policy_scope_documented": summary["tif_policy_scope_documented"],
