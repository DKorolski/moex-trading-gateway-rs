#!/usr/bin/env python3
"""Direct-successor source/archive seal for Stage 6E-R1."""
...
import stage6e_r1_closed_surface_check as closed
...
def fail(message: str) -> None:
    raise SystemExit(f"stage6e-r1-preseal: FAIL: {message}")
...
def main() -> None:
    ...
    if output("git", "rev-parse", "HEAD^", root=root) != checker.BASE:
        fail("HEAD is not a direct successor of Stage 6E")
    ...
    checker.check(root)
    print(f"stage6e-r1-preseal: PASS delta={len(changed)} archive={len(archived)}")
