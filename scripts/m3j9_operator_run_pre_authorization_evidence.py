#!/usr/bin/env python3
"""Generate M3j-9 operator-run pre-authorization evidence."""
...
DOC = Path("docs/m3j9-operator-run-pre-authorization-evidence.md")

CHECKS = [
    ["cargo", "fmt", "--all", "--check"],
    ["cargo", "test", "-p", "finam-gateway", "m3j9", "--", "--nocapture"],
    ["cargo", "test", "--all"],
    ["cargo", "clippy", "--workspace", "--all-targets", "--", "-D", "warnings"],
    ["bash", "scripts/forbidden_surface_scan.sh"],
    ["bash", "scripts/forbidden_surface_negative_harness.sh"],
    ["bash", "scripts/order_endpoint_scanner_transition_spec.sh"],
    ["python3", "-m", "py_compile", "scripts/m3j9_operator_run_pre_authorization_evidence.py"],
]
...
def evidence_summary(root: Path) -> dict[str, Any]:
    ...
    required_patterns = [
        ...
        "m3j9_pre_authorization_bundle_ready_but_live_micro_still_blocked",
        "m3j9_stale_readonly_broker_truth_or_live_input_blocks_bundle",
    ]
    forbidden_patterns = [
        ".post(",
        ".delete(",
    ...
    return {
        ...
        "pre_authorization_evidence_only": "pre_authorization_evidence_only: true" in source,
        "evidence_bundle_ready_model_present": "evidence_bundle_ready" in source,
        "live_micro_go": False,
        "no_live_boundary": (
        ...
        "m3j9_documented": "M3j-9 closes the next pre-live review gap" in doc,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate M3j-9 operator-run pre-authorization evidence."
    )
    parser.add_argument("--source-archive", type=Path)
    parser.add_argument("--m3j8-evidence", type=Path)
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("reports/m3j-pre-live/m3j9-operator-run-pre-authorization-evidence.json"),
    )
    ...
    m3j8_evidence = artifact_summary(
        (root / args.m3j8_evidence).resolve() if args.m3j8_evidence else None
    )
    ...
    evidence = {
        ...
        "summary": summary,
        "artifact_manifest": {"m3j8_evidence": m3j8_evidence},
        "pre_authorization_evidence_only": True,
        "live_micro_go": False,
