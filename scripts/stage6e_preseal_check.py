#!/usr/bin/env python3
"""One-commit source/archive seal for Stage 6E."""
...
import stage6e_closed_surface_check as closed
...
def fail(message: str) -> None:
    raise SystemExit(f"stage6e-preseal: FAIL: {message}")
...
def main() -> None:
    ...
    if output("git", "rev-parse", "HEAD^", root=root) != checker.BASE:
        fail("HEAD is not a direct successor of accepted Stage 6D")
    ...
    checker.check(root)
    print(f"stage6e-preseal: PASS delta={len(changed)} archive={len(archived)}")
