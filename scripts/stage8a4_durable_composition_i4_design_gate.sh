...
git diff --quiet 593ff255ef7826a22e66c9aff6f7ea47acf47644 -- Cargo.toml Cargo.lock crates tests
cargo fmt --all -- --check
git diff --check

echo "stage8a4-durable-composition-i4-design-gate: PASS rows=40 negatives=24 implementation=false ack_publish=false xack=false redis=false finam=false dispatch=false live=false"
