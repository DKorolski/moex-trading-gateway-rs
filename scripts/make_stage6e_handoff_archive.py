#!/usr/bin/env python3
"""Create immutable Stage 6E source plus evidence handoff."""
...
def fail(message: str) -> None:
    raise SystemExit(f"stage6e-handoff: FAIL: {message}")
...
def main() -> None:
    ...
    negative = subprocess.check_output(["python3", "scripts/stage6e_negative_harness.py"], cwd=ROOT)
    ...
    evidence = json.dumps(
        {
            ...
            "semantic_cross_binding_before_restart_capability": True,
            "opaque_accepted_fresh_truth": True,
            "raw_fresh_truth_application": False,
            "integration_fingerprint_schema_version": 2,
            "durable_before_effect_preserved": True,
            "stage6a_b_c_compatibility_unchanged": True,
            "detached_stage6d_gate_passed": True,
            "detached_stage6c_r1_gate_passed": True,
            "execution_surfaces_open": False,
            "stage6_closes_only_after_independent_acceptance": True,
    ...
    members += [
        ...
        ("handoff-evidence/stage6e-full-gate.txt", redacted, 0o644),
        ("handoff-evidence/stage6e-evidence.json", evidence, 0o644),
        ("handoff-evidence/stage6e-negative.txt", negative, 0o644),
        (
            "handoff-evidence/stage6e-toolchain.txt",
            f"{run(['rustc', '--version'])}\n{run(['cargo', '--version'])}\n".encode(),
            0o644,
        ),
    ]
