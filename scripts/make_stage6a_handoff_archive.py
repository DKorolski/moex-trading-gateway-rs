...
def main():
    ...
    env=dict(os.environ); env["STAGE6A_R2_SKIP_PRESEAL"]="1"
    gate=subprocess.run(["bash","scripts/stage6a_r2_gate.sh"],cwd=ROOT,text=True,stdout=subprocess.PIPE,stderr=subprocess.STDOUT,env=env)
    ...
    evidence=json.dumps({"schema_version":1,"stage":"6A-R2","source_ref":head,"predecessor":checker.BASE,"gate_exit_code":0,"positive_test_count":73,"negative_case_minimum":145,"execution_surfaces_open":False,"golden_bytes_changed":False,"source_manifest_sha256":hashlib.sha256(manifest).hexdigest(),"gate_sha256":hashlib.sha256(redacted).hexdigest()},indent=2,sort_keys=True).encode()+b"\n"
