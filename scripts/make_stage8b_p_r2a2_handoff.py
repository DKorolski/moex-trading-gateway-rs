#!/usr/bin/env python3
"""Create the immutable Stage 8B-P R2A2 source/build handoff."""
...
import stage8b_p_r2a2_handoff_safety_check as safety
...
IMPLEMENTATION_REF = "55c598066e67d338f515c9c8f70015ae37f4fc63"
...
def main() -> None:
    if run("git", "status", "--porcelain", "--untracked-files=all").decode().strip():
        raise SystemExit("stage8b-p-r2a2-handoff: FAIL dirty worktree")
    branch = run("git", "branch", "--show-current").decode().strip()
    if branch != BRANCH:
        raise SystemExit(f"stage8b-p-r2a2-handoff: FAIL branch={branch}")
    ...
    if source_ref != run("git", "rev-parse", "@{upstream}").decode().strip():
        raise SystemExit("stage8b-p-r2a2-handoff: FAIL commit not pushed")
    if run("git", "merge-base", source_ref, PREDECESSOR).decode().strip() != PREDECESSOR:
        raise SystemExit("stage8b-p-r2a2-handoff: FAIL predecessor drift")

    gate = subprocess.run(
        ["bash", "scripts/stage8b_p_r2a2_semantic_provenance_gate.sh"],
        cwd=ROOT,
        stdout=subprocess.PIPE,
        stderr=subprocess.STDOUT,
        check=False,
    )
    if gate.returncode != 0 or b"stage8b-p-r2a2-gate: PASS" not in gate.stdout:
    ...
    build = json.loads((ROOT / "docs/stage-8/stage8b-p-r2a2-build-evidence.json").read_text())
    if helper_sha != build["helper"]["executable_sha256"]:
        raise SystemExit("stage8b-p-r2a2-handoff: FAIL helper build digest drift")
    ...
    evidence = (
        json.dumps({
            ...
            "stage": "8B-P",
            "revision": "R2A2",
            "status": "semantic_provenance_qualification_candidate",
