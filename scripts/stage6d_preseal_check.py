#!/usr/bin/env python3
"""One-commit source/archive seal for Stage 6D."""
...
import stage6d_closed_surface_check as closed
...
def fail(message: str) -> None:
    raise SystemExit(f"stage6d-preseal: FAIL: {message}")
...
def main() -> None:
    ...
    if output("git", "rev-parse", "HEAD^", root=root) != checker.BASE:
        fail("HEAD is not a direct successor of accepted Stage 6C-R1")
    ...
    if changed != closed.EXACT:
        fail(f"changed-path allowlist drift missing={sorted(closed.EXACT-changed)} extra={sorted(changed-closed.EXACT)}")
    ...
    checker.check(root)
    print(f"stage6d-preseal: PASS delta={len(changed)} archive={len(archived)}")
