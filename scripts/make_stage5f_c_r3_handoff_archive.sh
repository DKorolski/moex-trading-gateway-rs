...
if [[ -n "$(git -C "$repo_root" status --porcelain --untracked-files=all)" ]]; then
  echo "Refusing to build Stage 5F-c R3 handoff: source tree is dirty." >&2
...
tmp_dir="$(mktemp -d "${TMPDIR:-/tmp}/stage5f-c-r3-handoff.XXXXXX")"
...
run_gate() {
  local label="$1"
  shift
  local stdout_member="stage5f-r3-${label}-stdout.txt"
  local stderr_member="stage5f-r3-${label}-stderr.txt"
  local result_member="stage5f-r3-${label}-result.json"
  ...
  if [[ "$exit_code" -ne 0 ]]; then
    cat "$stdout_path"
    cat "$stderr_path" >&2
    echo "Stage 5F-c R3 gate failed: $label" >&2
    exit "$exit_code"
  fi
...
run_gate controlled-checker python3 scripts/stage5f_controlled_characterization_check.py
run_gate controlled-negative python3 scripts/stage5f_controlled_characterization_negative_harness.py
run_gate reachability-checker python3 scripts/stage5f_source_reachability_check.py
run_gate reachability-r2-negative python3 scripts/stage5f_source_reachability_negative_harness.py --r2-compat
run_gate reachability-negative python3 scripts/stage5f_source_reachability_negative_harness.py
run_gate seed-matrix cargo test -p strategy-runtime-core \
  stage5f_v2_all_state_seeds_roundtrip_exact -- --nocapture
run_gate f19-paired cargo test -p strategy-runtime-core \
  stage5f_f19_mr_owner_suppresses_paired_source_valid_bo_candidate -- --nocapture
run_gate f26-chain cargo test -p strategy-runtime-core \
  stage5f_f26_working_order_reaches_runtime_and_retains_stale_pending -- --nocapture
run_gate determinism cargo test -p strategy-runtime-core \
  stage5f_v2_candidate_repeat_is_byte_identical -- --nocapture
...
python3 - "$repo_root" "$tmp_dir/stage5f-r3-source-tree-manifest.json" \
...
git -C "$repo_root" cat-file commit "$source_ref" \
  >"$tmp_dir/stage5f-r3-commit-object.txt"
...
gates = []
for label in labels:
    member = f"stage5f-r3-{label}-result.json"
    gates.append({
        "label": label,
...
payload = {
    ...
    "commit_object_sha256": sha256(root / "stage5f-r3-commit-object.txt"),
    "gates": gates,
    "head_tree": head_tree,
    "parent_ref": parent_ref,
    ...
    "source_commit": source_commit,
    "source_ref": source_ref,
    "source_tree_manifest_sha256": sha256(root / "stage5f-r3-source-tree-manifest.json"),
    "stage": "5F-c-R3-runtime-consumed-reachability-closure",
    "status": "review_required_before_5f_d",
}
(root / "stage5f-r3-evidence-manifest.json").write_text(
    json.dumps(payload, indent=2, sort_keys=True) + "\n",
    encoding="utf-8",
)
PY
...
members = [
    "handoff-commit.txt",
    "stage5f-r3-commit-object.txt",
    "stage5f-r3-source-tree-manifest.json",
    "stage5f-r3-evidence-manifest.json",
]
for label in labels:
    members.extend([
        f"stage5f-r3-{label}-result.json",
        f"stage5f-r3-{label}-stdout.txt",
        f"stage5f-r3-{label}-stderr.txt",
    ])
...
preseal_stdout_member="stage5f-r3-archive-safety-stdout.txt"
preseal_stderr_member="stage5f-r3-archive-safety-stderr.txt"
preseal_started_at_utc="$(date -u '+%Y-%m-%dT%H:%M:%SZ')"
set +e
(
  cd "$repo_root"
  python3 scripts/stage5f_c_r3_handoff_safety_check.py \
...
if [[ "$preseal_exit_code" -ne 0 ]]; then
  ...
  echo "Stage 5F-c R3 preseal archive safety failed." >&2
...
payload = {
    "checked_evidence_manifest_sha256": sha256(root / "stage5f-r3-evidence-manifest.json"),
    "checked_source_tree_manifest_sha256": sha256(root / "stage5f-r3-source-tree-manifest.json"),
    "command": [
        "python3",
...
(root / "stage5f-r3-archive-safety-result.json").write_text(
    json.dumps(payload, indent=2, sort_keys=True) + "\n",
    encoding="utf-8",
)
PY
...
root = Path(sys.argv[2])
members = [
    "stage5f-r3-archive-safety-result.json",
    "stage5f-r3-archive-safety-stdout.txt",
    "stage5f-r3-archive-safety-stderr.txt",
]
with zipfile.ZipFile(archive, "a", compression=zipfile.ZIP_DEFLATED) as handle:
    for member in members:
        handle.write(root / member, member)
...
python3 "$repo_root/scripts/stage5f_c_r3_handoff_safety_check.py" \
  --archive "$tmp_archive"
...
echo "Stage 5F-c R3 handoff archive: $archive_path"
echo "Stage 5F-c R3 handoff SHA-256: $sha_path"
cat "$sha_path"
