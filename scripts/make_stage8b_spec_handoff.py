...
def main() -> None:
    ...
    marker = b"stage8b-spec-gate: PASS rows=80 negatives=60 specification=true implementation=false execution=false finam=false redis=false dispatch=false live=false stage8b_i=false stage8b_p=false stage8b_x=false stage12=false"
    if gate.returncode != 0 or marker not in gate.stdout:
        raise SystemExit(gate.stdout.decode(errors="replace"))

    manifest, entries = common.source_manifest(full_ref)
    evidence = (json.dumps({
        "schema_version": 1, "stage": "8B-S", "source_ref": full_ref,
        "source_short_ref": short_ref, "archive_name": archive_name,
        "branch": branch, "accepted_stage8b_d_merge_ref": BASE,
        "accepted_stage8b_d_candidate_ref": "f296d0be782b8aa550a20e27600ba16826214349",
        "acceptance_rows": 80, "negative_cases": 60,
        "gate_sha256": sha256(gate.stdout), "manifest_sha256": sha256(manifest),
        "specification_only": True, "production_implementation": False,
        "stage8b_i": False, "stage8b_p": False, "stage8b_x": False,
        "operator_arming": False, "finam_post_delete": False,
