#!/usr/bin/env python3
"""Create immutable Stage 6D source plus evidence handoff."""
...
def fail(message: str) -> None:
    raise SystemExit(f"stage6d-handoff: FAIL: {message}")
...
def main() -> None:
    ...
    negative = subprocess.check_output(["python3", "scripts/stage6d_negative_harness.py"], cwd=ROOT)
    ...
    evidence = json.dumps(
        {
            ...
            "predecessor": checker.BASE,
            "gate_exit_code": 0,
            "focused_test_count": 32,
            "restart_scenario_count": 10,
            "negative_case_count": 99,
            "first_boot_restart_explicit": True,
            "authenticated_frontier": True,
            "durable_before_effect": True,
            "stage5g_application_boundary_used": True,
            "exact_already_applied_noop": True,
            "paper_soak_ndjson": True,
    ...
    members += [
        ...
        ("handoff-evidence/stage6d-full-gate.txt", redacted, 0o644),
        ("handoff-evidence/stage6d-evidence.json", evidence, 0o644),
        ("handoff-evidence/stage6d-negative.txt", negative, 0o644),
        (
            "handoff-evidence/stage6d-toolchain.txt",
            f"{run(['rustc', '--version'])}\n{run(['cargo', '--version'])}\n".encode(),
            0o644,
        ),
    ]
