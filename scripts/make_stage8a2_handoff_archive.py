#!/usr/bin/env python3
"""Create the immutable Stage 8A-2 R1 implementation handoff."""
...
import stage8a2_handoff_safety_check as safety
...
def fail(message: str) -> None:
    raise SystemExit(f"stage8a2-handoff: FAIL {message}")
...
def main() -> None:
    ...
    checker.check(ROOT, git_scope=True, pin_hashes=True, exact_parent_delta=True)

    with tempfile.TemporaryDirectory(prefix="stage8a2-handoff-") as raw_dir:
        artifact_dir = Path(raw_dir) / "gate"
        environment = os.environ.copy()
        environment["STAGE8A2_ARTIFACT_DIR"] = str(artifact_dir)
        gate = subprocess.run(
            ["bash", "scripts/stage8a2_gate.sh"],
            cwd=ROOT,
            env=environment,
        ...
        marker = (
            b"stage8a2-r1-gate: PASS rows=50 negatives=37 builder-only=true "
            b"comment-none=true no-send=true next=8A-3-pending"
        )
        ...
        required_artifacts = {"stage8a2-gate-summary.json", "stage8a2-proof-map.json"}
        ...
        source_manifest = json.dumps(
            {
                ...
                "transition_baseline": checker.BASE,
                "accepted_stage8a1_ref": checker.ACCEPTED_STAGE8A1,
                "members": [
        ...
        commit_marker = (
            ...
            f"accepted_stage8a1_ref={checker.ACCEPTED_STAGE8A1}\n"
            "candidate_stage=Stage 8A-2 R1\n"
            "candidate_status=independent_acceptance_pending\n"
            "next_after_acceptance=Stage 8A-3 only\n"
        ...
        evidence = json.dumps(
            {
                ...
                "acceptance_rows": 50,
                "negative_cases": 37,
                "single_consuming_privacy_seam": True,
                "existing_place_builder_only": True,
                "existing_cancel_builder_only": True,
                "place_comment_none": True,
                "opaque_request_shape_witness": True,
                "in_memory_no_send_sink": True,
                "transport_available": False,
                "finam_post_delete_authorized": False,
                "http_classifier_available": False,
                ...
                "next_after_independent_acceptance": "Stage 8A-3 only",
            },
            indent=2,
            sort_keys=True,
        ).encode() + b"\n"
        ...
        members.extend(
            [
                ...
                ("handoff-evidence/stage8a2-full-gate.txt", gate_output, 0o644),
                ("handoff-evidence/stage8a2-evidence.json", evidence, 0o644),
            ]
        )
        members.extend(
