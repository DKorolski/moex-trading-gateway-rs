#!/usr/bin/env python3
"""Create the immutable Stage 8B-P R2A4 source/Linux-build handoff."""
...
import stage8b_p_r2a4_handoff_safety_check as safety
...
LINUX_BIN_DIR = ROOT / "tmp/stage8b-r2a4-build-a/release"
...
def main() -> None:
    if run("git", "status", "--porcelain", "--untracked-files=all").decode().strip():
        raise SystemExit("stage8b-p-r2a4-handoff: FAIL dirty worktree")
    branch = run("git", "branch", "--show-current").decode().strip()
    if branch != BRANCH:
        raise SystemExit(f"stage8b-p-r2a4-handoff: FAIL branch={branch}")
    ...
    if run("git", "merge-base", source_ref, PREDECESSOR).decode().strip() != PREDECESSOR:
        raise SystemExit("stage8b-p-r2a4-handoff: FAIL predecessor drift")

    gate = subprocess.run(
        ["bash", "scripts/stage8b_p_r2a4_gate.sh"],
        cwd=ROOT,
        stdout=subprocess.PIPE,
        stderr=subprocess.STDOUT,
        check=False,
    )
    if gate.returncode != 0 or b"stage8b-p-r2a4-gate: PASS" not in gate.stdout:
        raise SystemExit(gate.stdout.decode(errors="replace"))
    build = json.loads((ROOT / "docs/stage-8/stage8b-p-r2a4-build-evidence.json").read_text())
    binaries: dict[str, bytes] = {}
    for name, expected in build["linux_release_sha256"].items():
        data = (LINUX_BIN_DIR / name).read_bytes()
        if sha(data) != expected:
            raise SystemExit(f"stage8b-p-r2a4-handoff: FAIL Linux digest drift: {name}")
        binaries[name] = data
        ...
        json.dumps(
            {
                ...
                "stage": "8B-P",
                "revision": "R2A4",
                "status": "exact_package_deployable_provenance_candidate",
                ...
                "controlled_tests": 36,
                "r2a4_negative_mutations": 32,
                "inherited_negative_mutations": 134,
                "producer_identities": 11,
                "issuer_identities": 11,
                "linux_release_binaries": 10,
