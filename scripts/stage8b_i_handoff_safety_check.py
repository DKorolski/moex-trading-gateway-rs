#!/usr/bin/env python3
"""Validate provenance, tracked bytes and closed surfaces for Stage 8B-I."""
...
EVIDENCE = "handoff-evidence/stage8b-i-evidence.json"
GATE = "handoff-evidence/stage8b-i-gate.txt"
...
def check(path: str) -> dict[str, object]:
    with zipfile.ZipFile(path) as archive:
        ...
        if evidence.get("acceptance_rows") != 104 or evidence.get("negative_cases") != 82:
...
def main() -> None:
    if len(sys.argv) != 2:
        raise SystemExit("usage: stage8b_i_handoff_safety_check.py ARCHIVE")
    try:
        result = check(sys.argv[1])
    except (OSError, ValueError, KeyError, zipfile.BadZipFile, json.JSONDecodeError) as error:
        print(f"stage8b-i-handoff-safety: FAIL {error}", file=sys.stderr)
        raise SystemExit(1)
    print("stage8b-i-handoff-safety: PASS " + json.dumps(result, sort_keys=True))
