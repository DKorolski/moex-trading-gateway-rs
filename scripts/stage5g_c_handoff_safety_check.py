...
inherited.STAGE = "5G-c-order-trade-position-convergence"
...
inherited.EXPECTED_COMMANDS = {
    "stage5g-b-r3-snapshot": ["bash", "scripts/stage5g_b_r3_snapshot_gate.sh"],
    ...
    "production-integration": [
        ...
        "stage5gc_public_terminal_ack_converges_without_broker_callback", "--quiet",
    ],
...
inherited.EXPECTED_CHANGED_PATHS = sorted({
    ...
    "scripts/stage5g_b_r3_snapshot_gate.sh",
...
def validate_gate_marker(label: str, stdout: str) -> None:
    markers = {
        "stage5g-b-r3-snapshot": "stage5g-b-r3-snapshot-gate: ok",
        "c-checker": "stage5g-c-check: PASS",
        "c-negative": "stage5g-c-negative-harness: PASS 10/10",
    ...
    if label in {"focused-debug", "focused-release"} and "16 passed" not in stdout:
        inherited.common.fail(f"focused 16-test marker missing: {label}")
    if label == "production-integration" and "1 passed" not in stdout:
        inherited.common.fail("production integration 1-test marker missing")
