#!/usr/bin/env python3
"""Validate an immutable Stage 8B-P1-b semantic implementation handoff."""
...
EVIDENCE = "handoff-evidence/stage8b-p1b-semantic-commit-evidence.json"
GATE = "handoff-evidence/stage8b-p1b-semantic-commit-gate.txt"
...
REQUIRED = GENERATED | {
    ...
    "docs/stage-8/stage8b-p1b-semantic-commit-implementation.md",
    "docs/stage-8/stage8b-p1b-semantic-commit-acceptance-matrix.csv",
    "docs/stage-8/stage8b-p1b-semantic-commit-evidence.json",
    "scripts/stage8b_p1b_semantic_commit_check.py",
    "scripts/stage8b_p1b_semantic_commit_gate.sh",
    "scripts/stage8b_p1b_semantic_negative_harness.sh",
    "scripts/make_stage8b_p1b_semantic_handoff.py",
    "scripts/stage8b_p1b_semantic_handoff_safety_check.py",
}
PREDECESSOR = "073ae9f38acd06b7a5febdbfc1e75a7b460adf03"
...
def check(path: str) -> dict[str, object]:
    with zipfile.ZipFile(path) as archive:
        ...
        if evidence.get("stage") != "Stage 8B-P1-b":
            raise ValueError("stage mismatch")
        if evidence.get("status") != "IMPLEMENTATION_REVIEW_CANDIDATE":
        ...
        for key in (
            ...
            "targeted_clippy_all_targets_all_features_deny_warnings",
            "source_scope_checker",
            "negative_harness",
            "aggregate_gate",
        ):
        ...
        for marker_text in (
            b"PASS stage8b-p1b-semantic-commit-source-scope",
            b"PASS stage8b-p1b-semantic-negative-harness",
            b"PASS stage8b-p1b-semantic-commit-gate",
        ):
...
def main() -> None:
    if len(sys.argv) != 2:
        raise SystemExit("usage: stage8b_p1b_semantic_handoff_safety_check.py ARCHIVE")
    try:
        result = check(sys.argv[1])
    except (OSError, ValueError, KeyError, zipfile.BadZipFile, json.JSONDecodeError) as error:
        print(f"stage8b-p1b-semantic-handoff-safety: FAIL {error}", file=sys.stderr)
        raise SystemExit(1)
    print(
        "stage8b-p1b-semantic-handoff-safety: PASS "
