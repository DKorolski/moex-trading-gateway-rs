#!/usr/bin/env python3
"""Generate M3f-4a reconciliation closure evidence."""
...
DOC = Path("docs/m3f4a-runner-idempotency-closure.md")

CHECKS = [
    ["cargo", "fmt", "--all", "--check"],
    ["cargo", "test", "-p", "finam-gateway", "m3f4a", "--", "--nocapture"],
    ["cargo", "test", "--all"],
    ["cargo", "clippy", "--workspace", "--all-targets", "--", "-D", "warnings"],
    ["bash", "scripts/forbidden_surface_scan.sh"],
    ["bash", "scripts/forbidden_surface_negative_harness.sh"],
    ["bash", "scripts/order_endpoint_scanner_transition_spec.sh"],
    ["python3", "-m", "py_compile", "scripts/m3f4a_reconciliation_closure_evidence.py"],
]
...
def evidence_summary(root: Path) -> dict[str, Any]:
    ...
    required_patterns = [
        "m3f4a_partial_apply_restart_is_idempotent_and_report_sink_is_redacted",
        "m3f_write_reconciliation_runner_report_json",
        "M3fReconciliationBlockerSummary",
        "blocker_count_by_kind",
        "affected_request_count",
        "oldest_pending_age_secs",
        "manual_intervention_count",
        "m3f_snapshot_issue_matches_record_identity",
        "stable_rerun_report.scheduled_count, 0",
        "stable_rerun_report.persisted_transition_count, 0",
    ...
    forbidden_patterns = [
        ...
        "m3f_write_reconciliation_runner_report_json::POST",
        "m3f_write_reconciliation_runner_report_json::DELETE",
        "api.finam.ru/order",
    ]
    return {
        ...
        "m3f_reconciliation_stage_closed": all(
            pattern in source for pattern in required_patterns
        ),
        "runner_restart_idempotency_ok": (
            "partial_apply_restart_is_idempotent" in source
            and "stable_rerun_report.scheduled_count, 0" in source
            and "stable_rerun_report.persisted_transition_count, 0" in source
        ),
        "partial_apply_recovery_ok": (
            "m3f_persist_reconciliation_decision(" in source
            and "rerun after partial apply" in source
        ),
        "redacted_report_sink_ok": (
            "m3f_write_reconciliation_runner_report_json" in source
            and "assert!(!report_json.contains" in source
        ),
        "blocker_summary_ok": (
            "M3fReconciliationBlockerSummary" in source
            and "blocker_count_by_kind" in source
            and "oldest_pending_age_secs" in source
        ),
        "closure_documented": "Closure guarantees" in doc,
        "read_only_finam_surfaces_only": True,
        "real_finam_order_endpoint_used": False,
        "external_order_endpoint_allowed": False,
        "runtime_live_attachment_allowed": False,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Generate M3f-4a closure evidence.")
    parser.add_argument(
        "--source-archive",
        type=Path,
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("reports/m3f-broker-truth/m3f4a-reconciliation-closure-evidence.json"),
    )
    ...
    policy_ok = (
        ...
        and summary["m3f_reconciliation_stage_closed"]
        and summary["runner_restart_idempotency_ok"]
        and summary["partial_apply_recovery_ok"]
        and summary["redacted_report_sink_ok"]
        and summary["blocker_summary_ok"]
    ...
    evidence = {
        ...
        "m3f_reconciliation_stage_closed": summary["m3f_reconciliation_stage_closed"],
        "runner_restart_idempotency_ok": summary["runner_restart_idempotency_ok"],
        "partial_apply_recovery_ok": summary["partial_apply_recovery_ok"],
        "redacted_report_sink_ok": summary["redacted_report_sink_ok"],
        "blocker_summary_ok": summary["blocker_summary_ok"],
        "read_only_finam_surfaces_only": True,
        "real_finam_order_endpoint_used": False,
        "external_order_endpoint_allowed": False,
        "runtime_live_attachment_allowed": False,
