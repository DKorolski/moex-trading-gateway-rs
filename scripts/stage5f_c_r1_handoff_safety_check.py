#!/usr/bin/env python3
"""Self-contained safety verifier for a Stage 5F-c R1 review archive."""
...
SOURCE_MANIFEST = "stage5f-r1-source-tree-manifest.json"
EVIDENCE_MANIFEST = "stage5f-r1-evidence-manifest.json"
COMMIT_MARKER = "handoff-commit.txt"
COMMIT_OBJECT = "stage5f-r1-commit-object.txt"
SAFETY_RESULT = "stage5f-r1-archive-safety-result.json"
...
def validate_archive(archive: str, allow_missing_final_safety: bool) -> tuple[str, int]:
    ...
    exact_keys(marker, {"archive_name", "source_commit", "source_ref"}, "commit marker")
    ...
    exact_keys(
        source_manifest,
        {"head_tree", "members", "schema_version", "source_commit", "source_ref", "stage"},
    ...
    negative_stdout = files["stage5f-r1-controlled-negative-stdout.txt"].decode(errors="replace")
    require(sum(line.startswith("PASS ") for line in negative_stdout.splitlines()), 51, "R1 negative PASS count")
    b1_stdout = files["stage5f-r1-inherited-b1-stdout.txt"].decode(errors="replace")
    if "stage5f-inherited-b1-snapshot-gate: ok" not in b1_stdout:
        fail("inherited B1 closure marker missing")
    b3f_stdout = files["stage5f-r1-inherited-b3f-stdout.txt"].decode(errors="replace")
    ...
    if SAFETY_RESULT in files:
        ...
        if f"stage5f-c-r1-handoff-safety: ok source_ref={source_ref} gates=10" not in safety_stdout:
...
def main() -> int:
    ...
    try:
        ...
        print(f"stage5f-c-r1-handoff-safety: FAIL: {exc}", file=sys.stderr)
        return 1
    print(f"stage5f-c-r1-handoff-safety: ok source_ref={source_ref} gates={gate_count}")
    return 0
