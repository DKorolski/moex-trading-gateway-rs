#!/usr/bin/env python3
"""Validate the complete Stage 7A-R2 F1-F15 semantic fault evidence."""
...
MATRIX = Path("docs/stage-7/stage7a-r2-fault-matrix.json")
REQUIRED_FIELDS = {
...
def main() -> None:
    ...
    if [fault.get("id") for fault in faults] != expected_ids:
        raise SystemExit("stage7a-r2-fault-matrix: FAIL: F1-F15 identity drift")
    evaluated = []
    for fault in faults:
    ...
    if not report["all_faults_passed"]:
        ...
        raise SystemExit(f"stage7a-r2-fault-matrix: FAIL faults={failed}")
    print("stage7a-r2-fault-matrix: PASS faults=15 F1-F15=complete")
