#!/usr/bin/env python3
"""Generate M3j-8 guarded tiny live-order-path implementation-diff evidence."""
...
DOC = Path("docs/m3j8-guarded-tiny-live-order-path-implementation-diff.md")

CHECKS = [
    ["cargo", "fmt", "--all", "--check"],
    ["cargo", "test", "-p", "finam-gateway", "m3j8", "--", "--nocapture"],
    ["cargo", "test", "--all"],
    ["cargo", "clippy", "--workspace", "--all-targets", "--", "-D", "warnings"],
    ["bash", "scripts/forbidden_surface_scan.sh"],
    ["bash", "scripts/forbidden_surface_negative_harness.sh"],
    ["bash", "scripts/order_endpoint_scanner_transition_spec.sh"],
    ["python3", "-m", "py_compile", "scripts/m3j8_guarded_tiny_live_order_path_evidence.py"],
]
...
def evidence_summary(root: Path) -> dict[str, Any]:
    ...
    required_patterns = [
        ...
        "m3j8_guarded_tiny_live_order_path_candidate_prepared_without_invocation",
        "m3j8_missing_gate_audit_or_open_boundary_blocks_guarded_candidate",
    ]
    forbidden_patterns = [
        ".post(",
        ".delete(",
    ...
    return {
        ...
        "guarded_implementation_diff_present": "guarded_implementation_diff: true" in source,
        "boundary_invocation_performed": False,
        "default_reachable": False,
        "live_micro_go": False,
        "no_live_boundary": (
        ...
        "unsafe_boundary_input_diagnostic_present": "unsafe_boundary_input_detected" in source,
        "m3j8_documented": "M3j-8 is a pre-live implementation-diff package" in doc,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate M3j-8 guarded tiny live-order-path implementation-diff evidence."
    )
    parser.add_argument("--source-archive", type=Path)
    parser.add_argument("--m3j7-evidence", type=Path)
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("reports/m3j-pre-live/m3j8-guarded-tiny-live-order-path-evidence.json"),
    )
    ...
    m3j7_evidence = artifact_summary(
        (root / args.m3j7_evidence).resolve() if args.m3j7_evidence else None
    )
    ...
    artifact_manifest_ok = m3j7_evidence["provided"]
    policy_ok = (
        all(summary["required_patterns_present"].values())
        and all(summary["forbidden_patterns_absent"].values())
        and summary["guarded_implementation_diff_present"]
        and not summary["boundary_invocation_performed"]
        and not summary["default_reachable"]
        and not summary["live_micro_go"]
        and summary["no_live_boundary"]
        and summary["unsafe_boundary_input_diagnostic_present"]
        and summary["m3j8_documented"]
    )
    ...
    evidence = {
        ...
        "summary": summary,
        "artifact_manifest": {"m3j7_evidence": m3j7_evidence},
        "guarded_implementation_diff": True,
        "boundary_invocation_performed": False,
        "default_reachable": False,
        "live_micro_go": False,
