#!/usr/bin/env python3
"""Self-contained safety verifier for a Stage 5F-d review archive."""
...
SOURCE_MANIFEST = "stage5f-d-source-tree-manifest.json"
EVIDENCE_MANIFEST = "stage5f-d-evidence-manifest.json"
COMMIT_MARKER = "handoff-commit.txt"
COMMIT_OBJECT = "stage5f-d-commit-object.txt"
SAFETY_RESULT = "stage5f-d-archive-safety-result.json"
...
EXPECTED_COMMANDS = {
    ...
    "matrix-determinism": [
        ...
        "-p",
        "strategy-runtime-core",
        "stage5f_d_full_matrix_repeat_is_byte_identical",
        "--",
        "--nocapture",
        "--test-threads=1",
    ],
...
def validate_archive(archive: str, allow_missing_final_safety: bool) -> tuple[str, int]:
    ...
    require(evidence["status"], "review_required_before_5f_e", "evidence status")
    ...
    matrix_negative_stdout = files["stage5f-d-matrix-negative-stdout.txt"].decode(errors="replace")
    require(
        sum(line.startswith("PASS ") for line in matrix_negative_stdout.splitlines()),
        40,
        "Stage 5F-d negative PASS count",
    )
    matrix_checker_stdout = files["stage5f-d-matrix-checker-stdout.txt"].decode(errors="replace")
    if "stage5f-d-atomic-matrix-check: ok rows=34 groups=16 golden=true" not in matrix_checker_stdout:
        fail("Stage 5F-d matrix checker marker missing")
    for label in ("matrix-debug", "matrix-release"):
        output = files[f"stage5f-d-{label}-stdout.txt"].decode(errors="replace")
        if "stage5f_d_full_matrix_matches_frozen_golden ... ok" not in output:
            fail(f"{label} frozen-golden test marker missing")
        if "stage5f_f34_stage5c_pending_request_mismatch_terminal ... ok" not in output:
            fail(f"{label} full 34-row ownership marker missing")
    determinism_stdout = files["stage5f-d-matrix-determinism-stdout.txt"].decode(errors="replace")
    if "stage5f_d_full_matrix_repeat_is_byte_identical ... ok" not in determinism_stdout:
        fail("Stage 5F-d determinism marker missing")
    r3_stdout = files["stage5f-d-r3-snapshot-stdout.txt"].decode(errors="replace")
    if "stage5f-r3-snapshot-gate: ok" not in r3_stdout:
        fail("accepted R3 snapshot marker missing")
    b1_stdout = files["stage5f-d-inherited-b1-stdout.txt"].decode(errors="replace")
    if "stage5f-inherited-b1-snapshot-gate: ok" not in b1_stdout:
        fail("inherited B1 closure marker missing")
    b3f_stdout = files["stage5f-d-inherited-b3f-stdout.txt"].decode(errors="replace")
    if "stage5f-b3f-snapshot-provenance-gate: ok" not in b3f_stdout:
        fail("inherited B3F closure marker missing")
    functional_stdout = files["stage5f-d-functional-stdout.txt"].decode(errors="replace")
    if "stage5f-functional-development-gate: ok" not in functional_stdout:
        fail("functional development gate marker missing")
    ...
    if SAFETY_RESULT in files:
        ...
        if f"stage5f-d-handoff-safety: ok source_ref={source_ref} gates=13" not in safety_stdout:
...
def main() -> int:
    ...
    try:
        ...
        print(f"stage5f-d-handoff-safety: FAIL: {exc}", file=sys.stderr)
        return 1
    print(f"stage5f-d-handoff-safety: ok source_ref={source_ref} gates={gate_count}")
    return 0
