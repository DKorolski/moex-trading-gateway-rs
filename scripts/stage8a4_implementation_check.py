#!/usr/bin/env python3
"""Fail-closed Stage 8A-4 implementation R2 semantic checker."""
...
MATRIX = Path("docs/stage-8/STAGE8A_4_IMPLEMENTATION_R2_ACCEPTANCE_MATRIX_2026-08-15.csv")
NEGATIVE = Path("docs/stage-8/STAGE8A_4_IMPLEMENTATION_R2_NEGATIVE_INVENTORY_2026-08-15.md")
...
def check(root: Path = ROOT, *, git_scope: bool = True) -> None:
    ...
    require(authority["stage"] == "8A-4-implementation-R2", "stage drift")
    require(
        authority["status"] == "implementation_r2_independent_acceptance_pending",
    ...
    for field in (
        ...
        require(authority[field] is True, f"R2 invariant disabled: {field}")
    require(
    ...
    require(authority["focused_test_count"] == 28, "focused test count drift")
    require(authority["compile_fail_doctest_count"] == 3, "compile-fail count drift")
    require(authority["negative_case_count"] == 50, "negative count drift")
    expected_closed = {
    ...
    for field in (
        ...
        require(descriptor[field] is True, f"descriptor R2 invariant disabled: {field}")
    ...
    require(tests.count("#[test]") == 28, "focused test inventory drift")
    ...
    require("validate_timing(&exact_source.timing, context, policy)?" in source, "exact timing not validated")
    ...
    for test_name in (
        ...
        require(test_name in tests, f"R2 focused test missing: {test_name}")
    forbidden_source = (
    ...
    require(len(rows) == 86, "acceptance matrix must contain exactly 86 rows")
    require([row["id"] for row in rows] == [f"I{i:03d}" for i in range(1, 87)], "matrix IDs drift")
    negative = (root / NEGATIVE).read_text()
    require(len(re.findall(r"^\d+\.", negative, re.MULTILINE)) == 50, "negative inventory must contain 50 cases")
    ...
    for status_path in (CURRENT_STATUS, ROADMAP):
        ...
        require("implementation R2" in status, f"active implementation missing: {status_path}")
...
def main() -> None:
    try:
        ...
        raise SystemExit(1)
    print("stage8a4-implementation-check: PASS stage=R2 rows=86 focused-tests=28 negatives=50 compile-fail=3")
