#!/usr/bin/env python3
"""Generate M3e-1 command consumer skeleton evidence."""
...
CHECKS = [
    ["cargo", "fmt", "--all", "--check"],
    ["cargo", "test", "-p", "finam-gateway", "m3e1", "--", "--nocapture"],
    ["cargo", "test", "--all"],
    ...
    ["python3", "-m", "py_compile", "scripts/m3e1_command_consumer_evidence.py"],
]
...
def evidence_summary(root: Path) -> dict[str, Any]:
    ...
    required_patterns = [
        ...
        "pub struct M3eCommandConsumerDryRun",
        "pub async fn process_entry",
        "decode_m3e_command_entry",
        "MessageType::Command",
        "CommandAckReasonCode::DryRunOnly",
        "CommandAckReasonCode::ExpiredCommand",
        "M3eCommandDlqReason::InvalidJson",
        ...
        "m3e1_valid_command_publishes_redacted_dry_ack_then_xack_without_endpoint",
        "m3e1_expired_command_publishes_expired_ack_then_xack",
        "m3e1_invalid_command_payload_publishes_redacted_dlq_then_xack",
        "m3e1_wrong_message_type_goes_to_dlq_without_ack_or_endpoint",
    ]
    forbidden_patterns = [
        "M3eCommandConsumerDryRun::place_order_execution",
        "M3eCommandConsumerDryRun::cancel_order_execution",
        "M3eCommandConsumerDryRun::M3d2RealOrderEndpointTransport",
    ]
    return {
        ...
        "command_consumer_skeleton_present": "M3eCommandConsumerDryRun" in source,
        "ack_publication_present": "M3eCommandConsumerAction::AckPublished" in source,
        "dlq_publication_present": "M3eCommandConsumerAction::DlqPublished" in source,
        "xack_after_publish_model_present": "xack_after_ack_or_dlq_publish: true" in source,
        "endpoint_transport_absent": (
            "endpoint_transport_invoked: false" in source
            and "external_order_endpoint_allowed: false" in source
        ),
        "raw_payload_not_exported": "raw_payload_exported: false" in source,
        "raw_command_comment_not_exported": "raw_command_comment_exported: false" in source,
        "production_boundary": {
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate M3e-1 command consumer skeleton evidence."
    )
    parser.add_argument(
        "--source-archive",
        type=Path,
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("reports/m3e-command-consumer/m3e1-command-consumer-evidence.json"),
    )
    ...
    skeleton_ok = (
        ...
        and summary["command_consumer_skeleton_present"]
        and summary["ack_publication_present"]
        and summary["dlq_publication_present"]
        and summary["xack_after_publish_model_present"]
        and summary["endpoint_transport_absent"]
        and summary["raw_payload_not_exported"]
        and summary["raw_command_comment_not_exported"]
        and all(summary["production_boundary"].values())
    )
    evidence_ready = all_checks_ok and archive_ok and skeleton_ok
    evidence = {
        "m3e_step": "M3e-1",
        "command_consumer_skeleton_only": True,
        ...
        "command_consumer_skeleton_ok": skeleton_ok,
        "ack_dlq_xack_discipline_ok": summary["xack_after_publish_model_present"],
        "endpoint_transport_invoked": False,
        "external_order_endpoint_allowed": False,
