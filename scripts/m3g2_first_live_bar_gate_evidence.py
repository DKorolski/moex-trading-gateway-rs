#!/usr/bin/env python3
"""Generate M3g-2 first-live-bar gate evidence."""
...
DOC = Path("docs/m3g2-first-live-bar-gate.md")

CHECKS = [
    ["cargo", "fmt", "--all", "--check"],
    ["cargo", "test", "-p", "finam-gateway", "m3g2", "--", "--nocapture"],
    ["cargo", "test", "--all"],
    ["cargo", "clippy", "--workspace", "--all-targets", "--", "-D", "warnings"],
    ["bash", "scripts/forbidden_surface_scan.sh"],
    ["bash", "scripts/forbidden_surface_negative_harness.sh"],
    ["bash", "scripts/order_endpoint_scanner_transition_spec.sh"],
    ["python3", "-m", "py_compile", "scripts/m3g2_first_live_bar_gate_evidence.py"],
]
...
def evidence_summary(root: Path) -> dict[str, Any]:
    ...
    required_patterns = [
        ...
        "Duplicate",
        "NonMonotonic",
        "m3g_live_bar_dedupe_key",
        "m3g_bar_is_inside_session",
        "ReadinessPhase::Blocked",
        "live_ready_allowed: false",
        "runtime_live_attachment_allowed: false",
        "real_finam_order_endpoint_used: false",
        "external_order_endpoint_allowed: false",
        "m3g2_rejects_historical_or_stale_or_out_of_session_first_live_bar",
        "m3g2_live_bar_gate_dedupes_and_rejects_non_monotonic_bars",
        "m3g2_first_live_bar_missing_hard_blocks_readiness_but_live_ready_remains_forbidden",
    ]
    forbidden_patterns = [
        "m3g2_evaluate_first_live_bar_gate::reqwest",
        "m3g2_evaluate_first_live_bar_gate::POST",
        "m3g2_evaluate_first_live_bar_gate::DELETE",
        "api.finam.ru/order",
    ]
    return {
        ...
        "m3g2_first_live_bar_gate_ok": all(
            pattern in source for pattern in required_patterns
        ),
        "historical_readonly_rejected": "HistoricalOrReadOnlySource" in source,
        "timestamp_freshness_present": (
            "max_allowed_age" in source and "age.num_seconds()" in source
        ),
        "session_validation_present": "m3g_bar_is_inside_session" in source,
        "dedupe_monotonic_policy_present": (
            "accepted_bar_keys" in source and "last_accepted_open_ts" in source
        ),
        "hard_block_readiness_present": (
            "report.readiness.phase = ReadinessPhase::Blocked" in source
        ),
        "live_ready_not_allowed": (
            "live_ready_allowed: false" in source
            and "assert_ne!(readyish.readiness.phase, ReadinessPhase::LiveReady)" in source
        ),
        "closure_documented": "M3g-2 hardens the market-data side" in doc,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Generate M3g-2 first-live-bar evidence.")
    parser.add_argument(
        "--source-archive",
        type=Path,
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("reports/m3g-readiness/m3g2-first-live-bar-gate-evidence.json"),
    )
    ...
    policy_ok = (
        ...
        and summary["m3g2_first_live_bar_gate_ok"]
        and summary["historical_readonly_rejected"]
        and summary["timestamp_freshness_present"]
        and summary["session_validation_present"]
        and summary["dedupe_monotonic_policy_present"]
        and summary["hard_block_readiness_present"]
    ...
    evidence = {
        ...
        "m3g2_first_live_bar_gate_ok": summary["m3g2_first_live_bar_gate_ok"],
        "historical_readonly_rejected": summary["historical_readonly_rejected"],
        "timestamp_freshness_present": summary["timestamp_freshness_present"],
        "session_validation_present": summary["session_validation_present"],
        "dedupe_monotonic_policy_present": summary["dedupe_monotonic_policy_present"],
        "hard_block_readiness_present": summary["hard_block_readiness_present"],
