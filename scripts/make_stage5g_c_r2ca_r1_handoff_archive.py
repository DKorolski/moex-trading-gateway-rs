#!/usr/bin/env python3
"""Build the push-bound self-attesting R2-c-a R1 review archive."""
...
import stage5g_c_r2ca_r1_handoff_safety_check as safety

ROOT = Path(__file__).resolve().parents[1]
HANDOFF_DIR = ROOT / "reports/handoff"
CONTRACT_PATH = "docs/stage-5/stage5g-c-r2ca-r1-market-terminal-state-coherence.json"
...
def run_gate(temp: Path, label: str, command: list[str], source_ref: str) -> dict[str, Any]:
    ...
    if completed.returncode != 0:
        ...
        raise SystemExit(f"R2-c-a R1 gate failed: {label}")
...
def main() -> int:
    if run_text(["git", "status", "--porcelain", "--untracked-files=all"]):
        raise SystemExit("refusing R2-c-a R1 handoff: source tree is dirty")
    branch = run_text(["git", "branch", "--show-current"])
    if branch != safety.BRANCH:
        raise SystemExit(f"R1 handoff requires {safety.BRANCH}, got {branch}")
    if run_text(["git", "rev-parse", "--show-object-format"]) != "sha1":
        raise SystemExit("R1 handoff requires SHA-1 Git objects")
    ...
    if parent_ref != safety.BASE_REF:
        raise SystemExit(f"R1 must directly follow {safety.BASE_REF}")
    ...
    if changed_paths != safety.EXPECTED_CHANGED_PATHS:
        raise SystemExit(
            f"R1 changed-path scope drift: expected {safety.EXPECTED_CHANGED_PATHS!r}, "
    ...
    with tempfile.TemporaryDirectory(prefix="stage5g-r2ca-r1-handoff-") as raw_temp:
        temp = Path(raw_temp)
        results = {
            label: run_gate(temp, label, command, source_ref)
        ...
        preseal = subprocess.run(
            [
                "python3", "scripts/stage5g_c_r2ca_r1_handoff_safety_check.py",
                str(archive), "--allow-missing-final-safety", "--result-out",
        ...
        if preseal.returncode != 0:
            ...
            raise SystemExit("R2-c-a R1 preseal safety check failed")
        append_generated(
            archive, temp, [safety.SAFETY_RESULT, safety.SAFETY_STDOUT, safety.SAFETY_STDERR]
        )

    final = subprocess.run(
        ["python3", "scripts/stage5g_c_r2ca_r1_handoff_safety_check.py", str(archive)],
        cwd=ROOT,
        check=False,
    )
    if final.returncode != 0:
        raise SystemExit("R2-c-a R1 final safety check failed")
