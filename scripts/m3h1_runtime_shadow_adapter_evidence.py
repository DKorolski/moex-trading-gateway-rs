#!/usr/bin/env python3
"""Generate M3h-1 runtime shadow input adapter evidence."""
...
DOC = Path("docs/m3h1-runtime-shadow-input-adapter.md")

CHECKS = [
    ["cargo", "fmt", "--all", "--check"],
    ["cargo", "test", "-p", "finam-gateway", "m3h1", "--", "--nocapture"],
    ["cargo", "test", "--all"],
    ["cargo", "clippy", "--workspace", "--all-targets", "--", "-D", "warnings"],
    ["bash", "scripts/forbidden_surface_scan.sh"],
    ["bash", "scripts/forbidden_surface_negative_harness.sh"],
    ["bash", "scripts/order_endpoint_scanner_transition_spec.sh"],
    ["python3", "-m", "py_compile", "scripts/m3h1_runtime_shadow_adapter_evidence.py"],
]
...
def evidence_summary(root: Path) -> dict[str, Any]:
    ...
    required_patterns = [
        ...
        "m3h1_runtime_adapter_accepts_live_final_bar_without_live_unlock",
        "m3h1_runtime_adapter_rejects_historical_or_readonly_as_live_unlock",
        "m3h1_runtime_adapter_keeps_readiness_and_order_snapshot_broker_neutral",
        "m3h1_runtime_adapter_deadletters_wrong_stream_contract",
    ]
    forbidden_patterns = [
        "m3h1_adapt_runtime_shadow_entry::reqwest",
        "m3h1_adapt_runtime_shadow_entry::POST",
        "m3h1_adapt_runtime_shadow_entry::DELETE",
        "api.finam.ru/order",
    ]
    return {
        ...
        "runtime_shadow_adapter_present": (
            "M3hRuntimeShadowInput" in source
            and "m3h1_adapt_runtime_shadow_entry" in source
        ),
        "bar_source_classification_present": (
            "M3hRuntimeShadowBarSourceClass::LiveFinal" in source
            and "M3hRuntimeShadowBarSourceClass::HistoricalOrReadOnly" in source
        ),
        "historical_readonly_does_not_unlock_live": (
            "m3h1_runtime_adapter_rejects_historical_or_readonly_as_live_unlock"
            in source
        ),
        "broker_neutral_runtime_input_only": (
            "broker_neutral_payload_only: true" in source
            and "finam_dto_visible_to_runtime: false" in source
        ),
        "no_command_emission": "emits_broker_command: false" in source,
        "live_ready_not_allowed": (
            "live_ready_allowed: false" in source
            and "ReadinessPhase::LiveReady" in source
        ),
        "closure_documented": "M3h-1 starts runtime shadow integration" in doc,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate M3h-1 runtime shadow adapter evidence."
    )
    parser.add_argument(
        "--source-archive",
        type=Path,
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("reports/m3h-runtime-shadow/m3h1-runtime-shadow-adapter-evidence.json"),
    )
    ...
    policy_ok = (
        ...
        and summary["runtime_shadow_adapter_present"]
        and summary["bar_source_classification_present"]
        and summary["historical_readonly_does_not_unlock_live"]
        and summary["broker_neutral_runtime_input_only"]
    ...
    evidence = {
        ...
        "runtime_shadow_adapter_present": summary["runtime_shadow_adapter_present"],
        "bar_source_classification_present": summary[
            "bar_source_classification_present"
        ],
        "historical_readonly_does_not_unlock_live": summary[
            "historical_readonly_does_not_unlock_live"
        ],
        "broker_neutral_runtime_input_only": summary[
            "broker_neutral_runtime_input_only"
        ],
