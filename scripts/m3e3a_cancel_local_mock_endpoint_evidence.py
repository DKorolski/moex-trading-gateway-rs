#!/usr/bin/env python3
"""Generate M3e-3a cancel local-mock endpoint boundary evidence."""
...
CHECKS = [
    ...
    [
        ...
        "-m",
        "py_compile",
        "scripts/m3e3a_cancel_local_mock_endpoint_evidence.py",
    ],
]
...
def evidence_summary(root: Path) -> dict[str, Any]:
    ...
    required_patterns = [
        ...
        "load_by_broker_order_id",
        "simulate_cancel_order_endpoint_classified_transport",
        ...
        "m3e3a_cancel_command_reaches_local_mock_endpoint_after_request_cancel",
        "m3e3a_duplicate_cancel_request_does_not_call_endpoint_again",
        "m3e3a_cancel_preflight_rejects_before_endpoint_attempt",
        "m3e3a_cancel_404_409_410_require_reconciliation_not_blind_success",
    ]
    forbidden_patterns = [
    ...
    return {
        ...
        "m3e3a_cancel_local_mock_endpoint_boundary_ok": (
            "process_cancel_order" in source
            and "simulate_cancel_order_endpoint_classified_transport" in source
            and "FinamMockClassifiedEndpointTransport" in source
        ),
        "cancel_command_received_persisted_before_endpoint": (
            "insert_received(lifecycle_record.clone())" in source
            and "m3e3a_cancel_command_reaches_local_mock_endpoint_after_request_cancel" in source
        ),
        "cancel_preflight_local_reject_before_endpoint": (
            "m3e3a_cancel_preflight_rejects_before_endpoint_attempt" in source
            and "preflight_local_reject_before_endpoint" in source
        ),
        "request_cancel_persisted_before_endpoint": (
            "simulate_cancel_order_endpoint_classified_transport" in source
            and "begin_submit_or_request_cancel_persisted_before_endpoint" in source
        ),
        "cancel_accepted_not_terminal": (
            "EndpointResponseIntegrationOutcomeKind::CancelSubmitted" in source
            and "OrderPathState::CancelSubmitted" in source
        ),
        "cancel_404_409_410_reconciliation_not_success": (
            "m3e3a_cancel_404_409_410_require_reconciliation_not_blind_success" in source
            and "CommandAckReasonCode::ReconciliationRequired" in source
            and "assert_ne!(report.ack_status, Some(CommandAckStatus::Submitted))" in source
        ),
        "duplicate_cancel_request_no_second_endpoint_attempt": (
            "m3e3a_duplicate_cancel_request_does_not_call_endpoint_again" in source
            and "assert_eq!(transport.cancel_call_count, 1)" in source
        ),
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate M3e-3a cancel local-mock endpoint boundary evidence."
    )
    parser.add_argument(
        "--source-archive",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(
            "reports/m3e-command-consumer/m3e3a-cancel-local-mock-endpoint-evidence.json"
        ),
    )
    ...
    boundary_ok = (
        ...
        and summary["m3e3a_cancel_local_mock_endpoint_boundary_ok"]
        and summary["cancel_command_received_persisted_before_endpoint"]
        and summary["cancel_preflight_local_reject_before_endpoint"]
        and summary["request_cancel_persisted_before_endpoint"]
        and summary["cancel_accepted_not_terminal"]
        and summary["cancel_404_409_410_reconciliation_not_success"]
        and summary["duplicate_cancel_request_no_second_endpoint_attempt"]
    ...
    evidence = {
        ...
        "m3e3a_cancel_local_mock_endpoint_boundary_ok": summary[
            "m3e3a_cancel_local_mock_endpoint_boundary_ok"
        ],
        "cancel_command_received_persisted_before_endpoint": summary[
            "cancel_command_received_persisted_before_endpoint"
        ],
        "cancel_preflight_local_reject_before_endpoint": summary[
            "cancel_preflight_local_reject_before_endpoint"
        ],
        "request_cancel_persisted_before_endpoint": summary[
            "request_cancel_persisted_before_endpoint"
        ],
        "cancel_accepted_not_terminal": summary["cancel_accepted_not_terminal"],
        "cancel_404_409_410_reconciliation_not_success": summary[
            "cancel_404_409_410_reconciliation_not_success"
        ],
        "duplicate_cancel_request_no_second_endpoint_attempt": summary[
            "duplicate_cancel_request_no_second_endpoint_attempt"
        ],
