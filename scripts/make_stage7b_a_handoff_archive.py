#!/usr/bin/env python3
"""Create an immutable Stage 7B-a source and evidence handoff."""
...
def fail(message: str) -> None:
    raise SystemExit(f"stage7b-a-handoff: FAIL: {message}")
...
def main() -> None:
    ...
    merge_base = run(["git", "merge-base", "HEAD", checker.BASE])
    if merge_base != checker.BASE:
        fail("wrong accepted predecessor")
    ...
    with tempfile.TemporaryDirectory(prefix="stage7b-a-handoff-") as temp:
        artifact_dir = Path(temp)
        env = dict(os.environ)
        env["STAGE7B_A_ARTIFACT_DIR"] = str(artifact_dir)
        gate = subprocess.run(
            ["bash", "scripts/stage7b_a_gate.sh"],
            cwd=ROOT,
            text=True,
    ...
    manifest = json.dumps(
        {
            ...
            "accepted_predecessor": checker.BASE,
            "members": [
    ...
    marker = (
        ...
        f"accepted_predecessor={checker.BASE}\n"
    ).encode()
    evidence = json.dumps(
        {
            "schema_version": 1,
    ...
    members += [
        ...
        ("handoff-evidence/stage7b-a-full-gate.txt", gate_output, 0o644),
        ("handoff-evidence/stage7b-a-evidence.json", evidence, 0o644),
        ("handoff-evidence/stage7b-a-negative.txt", negative, 0o644),
        (
            "handoff-evidence/stage7b-a-toolchain.txt",
            f"{run(['rustc', '--version'])}\n{run(['cargo', '--version'])}\n".encode(),
            0o644,
        ),
    ] + artifact_members
