#!/usr/bin/env python3
"""Keep Stage 7B-b inside path/lock paper durability boundaries."""
from __future__ import annotations

import subprocess
from pathlib import Path

import stage7b_b_check
...
ALLOWED_EXACT = {
    ...
    "scripts/make_stage7b_b_handoff_archive.py",
}
FORBIDDEN_PREFIXES = (
    "crates/broker-finam/",
...
def main() -> None:
    ...
    changed = subprocess.check_output(
        ["git", "diff", "--name-only", stage7b_b_check.BASE], cwd=root, text=True
    ).splitlines()
    ...
    if bad or outside:
        raise SystemExit(f"stage7b-b-closed-surface: FAIL bad={bad} outside={outside}")
    production = "\n".join(
    ...
    for token in (
        ...
        if token in production:
            raise SystemExit(f"stage7b-b-closed-surface: FAIL forbidden token={token}")
    print(
        "stage7b-b-closed-surface: PASS "
        f"changed={len(touched)} redis_consumer=false finam_post_delete=false "
