#!/usr/bin/env python3
"""Build the push-bound self-attesting Stage 5G-c review archive."""
...
def run_stage5g_c_verifier(command, *args, **kwargs):
    command = [
...
inherited.subprocess.run = run_stage5g_c_verifier
...
def main() -> int:
    ...
    if head != origin:
        raise SystemExit(
            "refusing Stage 5G-c handoff: origin/stage5g-lifecycle does not equal HEAD"
        )
