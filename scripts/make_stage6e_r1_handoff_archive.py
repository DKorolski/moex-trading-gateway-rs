#!/usr/bin/env python3
"""Create immutable Stage 6E-R1 source plus evidence handoff."""
...
def fail(message: str) -> None:
    raise SystemExit(f"stage6e-r1-handoff: FAIL: {message}")
...
def main() -> None:
    ...
    negative = subprocess.check_output(["python3", "scripts/stage6e_r1_negative_harness.py"], cwd=ROOT)
    ...
    evidence = json.dumps(
        {
            ...
            "trusted_current_process_restore_epoch": True,
            "trusted_validation_time_from_host": True,
            "integration_fingerprint_schema_version": 3,
            "accepted_fresh_truth_schema_version": 2,
            "detached_stage6e_gate_passed": True,
            "detached_stage6d_and_stage6c_r1_inherited_gates_passed": True,
    ...
    members += [
        ...
        ("handoff-evidence/stage6e-r1-full-gate.txt", redacted, 0o644),
        ("handoff-evidence/stage6e-r1-evidence.json", evidence, 0o644),
        ("handoff-evidence/stage6e-r1-negative.txt", negative, 0o644),
        (
            "handoff-evidence/stage6e-r1-toolchain.txt",
            f"{run(['rustc', '--version'])}\n{run(['cargo', '--version'])}\n".encode(),
            0o644,
        ),
    ]
