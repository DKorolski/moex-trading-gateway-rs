#!/usr/bin/env python3
"""Validate an immutable Stage 8B-P R2A1 handoff."""
...
EVIDENCE = "handoff-evidence/stage8b-p-r2a1-evidence.json"
GATE = "handoff-evidence/stage8b-p-r2a1-gate.txt"
...
REQUIRED = GENERATED | {
    "docs/stage-8/stage8b-p-r2a1-corrective-implementation.md",
    "docs/stage-8/stage8b-p-r2a1-current-source-authority.json",
    "docs/stage-8/stage8b-p-r2a1-network-topology-authority.json",
    "docs/stage-8/stage8b-p-r2a1-query-policy-authority.json",
    "docs/stage-8/stage8b-p-r2a1-build-evidence.json",
    ...
    "scripts/stage8b_p_r2a1_readonly_preflight_check.py",
    "scripts/stage8b_p_r2a1_readonly_preflight_gate.sh",
    "scripts/stage8b_p_r2a1_readonly_preflight_negative_harness.py",
    "scripts/make_stage8b_p_r2a1_handoff.py",
    "scripts/stage8b_p_r2a1_handoff_safety_check.py",
}
...
def check(path: str) -> dict[str, object]:
    with zipfile.ZipFile(path) as archive:
        ...
        if evidence.get("stage") != "8B-P" or evidence.get("revision") != "R2A1":
        ...
        if evidence.get("new_negative_mutations") != 40 or evidence.get("inherited_negative_mutations") != 134:
        ...
        if b"stage8b-p-r2a1-gate: PASS" not in gate or sha(gate) != evidence.get("gate_sha256"):
            raise ValueError("gate evidence mismatch")
        binary = archive.read(BINARY)
        if sha(binary) != evidence.get("helper_executable_sha256"):
...
def main() -> None:
    if len(sys.argv) != 2:
        raise SystemExit("usage: stage8b_p_r2a1_handoff_safety_check.py ARCHIVE")
    try:
        result = check(sys.argv[1])
    except (OSError, ValueError, KeyError, zipfile.BadZipFile, json.JSONDecodeError) as error:
        print(f"stage8b-p-r2a1-handoff-safety: FAIL {error}", file=sys.stderr)
        raise SystemExit(1)
    print("stage8b-p-r2a1-handoff-safety: PASS " + json.dumps(result, sort_keys=True))
