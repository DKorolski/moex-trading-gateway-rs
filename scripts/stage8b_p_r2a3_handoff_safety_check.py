#!/usr/bin/env python3
"""Validate an immutable Stage 8B-P R2A3 source/Linux-build handoff."""
...
EVIDENCE = "handoff-evidence/stage8b-p-r2a3-evidence.json"
GATE = "handoff-evidence/stage8b-p-r2a3-gate.txt"
MANIFEST = "handoff-evidence/source-tree-manifest.json"
BINARIES = {
    "stage8b-readonly-preflight": "handoff-evidence/linux-amd64/stage8b-readonly-preflight",
    "stage8b-r2a3-authority-issuer": "handoff-evidence/linux-amd64/stage8b-r2a3-authority-issuer",
    "stage8b-r2a3-launcher": "handoff-evidence/linux-amd64/stage8b-r2a3-launcher",
}
GENERATED = {"handoff-commit.txt", EVIDENCE, GATE, MANIFEST, *BINARIES.values()}
REQUIRED = GENERATED | {
    "docs/stage-8/STAGE8B_P_R2A3_ACCEPTANCE_MATRIX_2026-08-25.csv",
    "docs/stage-8/STAGE8B_P_R2A3_NEGATIVE_INVENTORY_2026-08-25.md",
    "docs/stage-8/STAGE8B_P_R2A3_RUNNABLE_CONTRACT_PROVENANCE_2026-08-25.md",
    "docs/stage-8/stage8b-p-r2a3-authority.json",
    "docs/stage-8/stage8b-p-r2a3-build-evidence.json",
    "docs/stage-8/stage8b-p-r2a3-finam-read-contract-snapshot.json",
    "scripts/stage8b_p_r2a3_gate.sh",
    ...
    "tools/stage8b-readonly-preflight/src/bin/stage8b-r2a3-authority-issuer.rs",
    "tools/stage8b-readonly-preflight/src/bin/stage8b-r2a3-launcher.rs",
}
...
def check(path: str) -> dict[str, object]:
    with zipfile.ZipFile(path) as archive:
        ...
        build = json.loads(archive.read("docs/stage-8/stage8b-p-r2a3-build-evidence.json"))
        ...
        if evidence.get("stage") != "8B-P" or evidence.get("revision") != "R2A3":
        ...
        if evidence.get("r2a3_negative_mutations") != 47 or evidence.get("inherited_negative_mutations") != 134:
            raise ValueError("coverage mismatch")
        if evidence.get("controlled_tests") != 32:
        ...
        if b"stage8b-p-r2a3-gate: PASS" not in gate or sha(gate) != evidence.get("gate_sha256"):
            raise ValueError("gate evidence mismatch")
        for name, member in BINARIES.items():
            digest = sha(archive.read(member))
            if digest != build["linux_release_sha256"].get(name):
...
def main() -> None:
    if len(sys.argv) != 2:
        raise SystemExit("usage: stage8b_p_r2a3_handoff_safety_check.py ARCHIVE")
    try:
        result = check(sys.argv[1])
    except (OSError, ValueError, KeyError, zipfile.BadZipFile, json.JSONDecodeError) as error:
        print(f"stage8b-p-r2a3-handoff-safety: FAIL {error}", file=sys.stderr)
        raise SystemExit(1)
    print("stage8b-p-r2a3-handoff-safety: PASS " + json.dumps(result, sort_keys=True))
