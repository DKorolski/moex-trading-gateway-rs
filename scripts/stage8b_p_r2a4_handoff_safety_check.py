#!/usr/bin/env python3
"""Validate an immutable Stage 8B-P R2A4 source/Linux-build handoff."""
...
EVIDENCE = "handoff-evidence/stage8b-p-r2a4-evidence.json"
GATE = "handoff-evidence/stage8b-p-r2a4-gate.txt"
MANIFEST = "handoff-evidence/source-tree-manifest.json"
BINARIES = {
    name: f"handoff-evidence/linux-amd64/{name}"
    for name in (
        "stage8b-readonly-preflight",
        "stage8b-r2a3-authority-issuer",
        "stage8b-r2a3-launcher",
        "stage8b-r2a4-authority-producer",
        "stage8b-r2a4-authority-issuer",
        "stage8b-r2a4-package-issuer",
        "stage8b-r2a4-launcher",
        "stage8b-r2a4-key-ceremony",
        "stage8b-r2a4-controlled-layout",
        "stage8b-r2a4-controlled-server",
    )
}
GENERATED = {"handoff-commit.txt", EVIDENCE, GATE, MANIFEST, *BINARIES.values()}
REQUIRED = GENERATED | {
    "docs/stage-8/STAGE8B_P_R2A4_ACCEPTANCE_MATRIX_2026-08-26.csv",
    "docs/stage-8/STAGE8B_P_R2A4_REVIEW_CLOSURE_2026-08-26.md",
    ...
    "docs/stage-8/stage8b-p-r2a4-production-topology.md",
    "docs/stage-8/stage8b-p-r2a4-status.json",
    "deploy/stage8b-r2a4/stage8b-r2a4-producer@.service",
    "deploy/stage8b-r2a4/stage8b-r2a4-issuer@.service",
    "scripts/stage8b_p_r2a4_gate.sh",
    "scripts/stage8b_p_r2a4_linux_rehearsal.sh",
...
def check(path: str) -> dict[str, object]:
    with zipfile.ZipFile(path) as archive:
        ...
        build = json.loads(archive.read("docs/stage-8/stage8b-p-r2a4-build-evidence.json"))
        ...
        if evidence.get("revision") != "R2A4" or evidence.get("authorization_status") != "NOT_ISSUED":
            raise ValueError("stage or authorization mismatch")
        if evidence.get("r2a4_negative_mutations") != 32 or evidence.get("controlled_tests") != 36:
            raise ValueError("coverage mismatch")
        for key in (
            "credential_used",
            "real_auth_request_sent",
            "real_broker_get_sent",
            "operator_arm_issued",
            "dispatch_attempt_appended",
        ...
        if b"stage8b-p-r2a4-gate: PASS" not in gate or sha(gate) != evidence.get("gate_sha256"):
            raise ValueError("gate evidence mismatch")
        for name, member in BINARIES.items():
            if sha(archive.read(member)) != build["linux_release_sha256"].get(name):
...
def main() -> None:
    if len(sys.argv) != 2:
        raise SystemExit("usage: stage8b_p_r2a4_handoff_safety_check.py ARCHIVE")
    try:
        result = check(sys.argv[1])
    except (OSError, ValueError, KeyError, zipfile.BadZipFile, json.JSONDecodeError) as error:
        print(f"stage8b-p-r2a4-handoff-safety: FAIL {error}", file=sys.stderr)
        raise SystemExit(1)
    print("stage8b-p-r2a4-handoff-safety: PASS " + json.dumps(result, sort_keys=True))
