...
if [[ "$(id -u)" != "0" ]]; then
  echo "stage8b-r2b-r2-custody: must run as root" >&2
  exit 1
fi
...
for binary in "$HELPER" "$LAUNCHER" "$CONTROLLED_LAUNCHER" "$PRODUCER" "$ISSUER" "$PACKAGE_ISSUER" "$LAYOUT" "$ADAPTER" "$SEEDER" "$MANIFEST_ISSUER"; do
  test -x "$binary"
done
...
if "$LAUNCHER" >/tmp/stage8b-r2b-wrong-helper.log 2>&1; then
  echo "stage8b-r2b-r2-custody: wrong helper was accepted" >&2
  exit 1
fi
...
for operation in PLACE CANCEL; do
  ...
  setpriv --reuid 8095 --regid 8095 --clear-groups \
    ...
    >"/tmp/stage8b-r2b-r2-adapter-$operation_lower.json"
  ...
  install -d -o 0 -g 8301 -m 0730 /var/lib/moex-trading/stage8b/r2b-evidence
  if "$CONTROLLED_LAUNCHER" --controlled-custody >"/tmp/stage8b-r2b-${operation,,}.out" 2>&1; then
    echo "stage8b-r2b-r2-custody: network-disabled helper unexpectedly succeeded" >&2
    exit 1
  fi
  ...
  if [[ ! -f "$admission/$nonce.requested" ]]; then
    ...
    echo "stage8b-r2b-r2-custody: launcher failed before durable admission" >&2
    exit 1
  fi
  test -f "$admission/$nonce.requested"
  test -f "$admission/$nonce.marker-created"
  test -f "$admission/$nonce.durable"
  test -f "$admission/$nonce.helper-started"
  ...
  test -f "$terminal"
  grep -Fq '"terminal_outcome":"FAILURE"' "$terminal"
  grep -Fq '"order_post_sent":false' "$terminal"
  grep -Fq '"order_delete_sent":false' "$terminal"
  grep -Fq '"raw_body_exported":false' "$terminal"
  test "$(stat -c %u "$terminal")" = "8301"
  test "$(stat -c %g "$terminal")" = "8301"
  test "$(stat -c %a "$terminal")" = "640"
  if setpriv --reuid 8301 --regid 8301 --clear-groups \
    rm "/var/lib/moex-trading/stage8b/r2a5/used-run-nonces/$nonce" 2>/dev/null; then
    echo "stage8b-r2b-r2-custody: helper identity deleted nonce" >&2
    exit 1
  fi
  if "$CONTROLLED_LAUNCHER" --controlled-custody >/tmp/stage8b-r2b-replay.log 2>&1; then
    echo "stage8b-r2b-r2-custody: replay was accepted" >&2
    exit 1
  fi
  test "$(find /var/lib/moex-trading/stage8b/r2b-evidence -maxdepth 1 -type f -name 'r2b-terminal-*.json' | wc -l | tr -d ' ')" = "1"
  echo "stage8b-r2b-r2-production-custody-$operation: PASS"
done

echo "stage8b-r2b-r2-linux-custody: PASS helper_fd_bound=true root_nonce=true uid8301=true terminal=true replay=false external_network=false"
