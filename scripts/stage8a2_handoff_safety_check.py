#!/usr/bin/env python3
"""Verify a Stage 8A-2 immutable handoff archive."""
...
REQUIRED = {
    ...
    "handoff-evidence/stage8a2-evidence.json",
    "handoff-evidence/stage8a2-full-gate.txt",
    "handoff-evidence/gate-artifacts/stage8a2-gate-summary.json",
    "handoff-evidence/gate-artifacts/stage8a2-proof-map.json",
}
...
def verify(path: str) -> dict[str, object]:
    with zipfile.ZipFile(path) as archive:
        ...
        evidence = json.loads(archive.read("handoff-evidence/stage8a2-evidence.json"))
        ...
        if evidence["accepted_stage8a1_ref"] != "1ff04154ba4b7a5ee060a73b853ce89bd7442f44":
            raise ValueError("predecessor mismatch")
        if not evidence["all_required_gates_passed"] or evidence["transport_available"]:
            raise ValueError("gate/transport evidence mismatch")
...
def main() -> None:
    if len(sys.argv) != 2:
        raise SystemExit("usage: stage8a2_handoff_safety_check.py ARCHIVE")
    try:
        result = verify(sys.argv[1])
    except (ValueError, KeyError, json.JSONDecodeError, zipfile.BadZipFile) as error:
        print(f"stage8a2-handoff-safety: FAIL {error}", file=sys.stderr)
        raise SystemExit(1)
